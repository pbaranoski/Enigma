...
# DESC:   This python program extracts the MEDPAR Bay State RRB file which is a subset of the full SSN file
#
...
try:
   ...
   ########################################################################################################
   snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_MEDPARBAYST_STG/MEDPAR_BAYSTATE_RRB_{FNAME_SUFFIX}_{TMSTMP}.txt.gz
            FROM (
                WITH CTE_CNT AS (
                    SELECT 
                    ...
                    INNER JOIN
                        (SELECT DISTINCT BENE_XREF_EFCTV_SK
                         ...
                         WHERE BENE_SSN_NUM IN (SELECT SSN_NUM FROM BIA_{ENVNAME}.CMS_TARGET_XTR_{ENVNAME}.MEDPAR_BAYSTATE_RRB)) BTRK1
                    ON BTRK.BENE_XREF_EFCTV_SK = BTRK1.BENE_XREF_EFCTV_SK AND
                       BENE_ID_TYPE_CD IN ('R')
                    GROUP BY 1
                    HAVING HICN_COUNT >= 1
                ),
                CTE_SSN AS (
                ...
                INNER JOIN 
                    (SELECT BENE_XREF_EFCTV_SK,
                            BENE_SSN_NUM
                     FROM IDRC_{ENVNAME}.CMS_DIM_BENE_{ENVNAME}.BENE_ID_TRKNG
                     WHERE BENE_SSN_NUM IN (SELECT SSN_NUM FROM BIA_{ENVNAME}.CMS_TARGET_XTR_{ENVNAME}.MEDPAR_BAYSTATE_RRB)
                     GROUP BY 1,2
                    ) TRKNG
