#!/usr/bin/sh
############################################################################################################
# Script Name: BAYSTATE_LOAD_SSN_FNDR_FILE.sh
...
#
# Note: The expected filename for the finder file is MEDPAR_BAYSTATE_SSN_*.txt. It will probably need to be
...
LOGNAME=/app/IDRC/XTR/CMS/logs/BAYSTATE_LOAD_SSN_FNDR_FILE_${TMSTMP}.log
...
echo "################################### " >> ${LOGNAME}
echo "BAYSTATE_LOAD_SSN_FNDR_FILE.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="BAYSTATE_LOAD_SSN_FNDR_FILE.sh script - Failed (${ENVNAME})"
...
NOF_FILES=`wc -l ${DATADIR}temp_MEDPAR_BAYSTATE_SSN.txt  | awk '{print $1}' `	2>> ${LOGNAME}

echo "" >> ${LOGNAME}
echo "${NOF_FILES} MEDPAR Baystate SSN finder files found in S3." >> ${LOGNAME}
...
filename=`awk '{print $4}' ${DATADIR}temp_MEDPAR_BAYSTATE_SSN.txt` 

echo "" >> ${LOGNAME}
echo "MEDPAR Baystate SSN Finder file found: ${filename}" >> ${LOGNAME}

#################################################################################
# Copy S3 Finder File to Linux. 
#################################################################################
echo "" >> ${LOGNAME}
echo "Copy MEDPAR Baystate SSN finder file from s3 to linux " >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	echo "" >> ${LOGNAME}
	echo "Copying MEDPAR Baystate SSN finder file to Linux failed." >> ${LOGNAME}
	
	# Send Failure email	
	SUBJECT="BAYSTATE_LOAD_SSN_FNDR_FILE.sh  - Failed (${ENVNAME})"
...
BAYSTATE_SSN_FF=${filename}

#################################################################################
# Execute Python code to load Finder File
#################################################################################
echo "" >> ${LOGNAME}
echo "Start execution of BAYSTATE_LOAD_SSN_FNDR_FILE.py program"  >> ${LOGNAME}

# Export environment variables for Python code
export DATADIR
export BAYSTATE_SSN_FF

${PYTHON_COMMAND} ${RUNDIR}BAYSTATE_LOAD_SSN_FNDR_FILE.py >> ${LOGNAME} 2>&1
...
if [[ $RET_STATUS != 0 ]]; then
        echo "" >> ${LOGNAME}
        echo "Python script BAYSTATE_LOAD_SSN_FNDR_FILE.py failed" >> ${LOGNAME}
		
		# Send Failure email	
		SUBJECT="BAYSTATE_LOAD_SSN_FNDR_FILE.sh  - Failed (${ENVNAME})"
		MSG="MEDPAR Baystate loading SSN finder file has failed."
		${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
echo "" >> ${LOGNAME}
echo "Python script BAYSTATE_LOAD_SSN_FNDR_FILE.py completed successfully. " >> ${LOGNAME}

#################################################################################
# Remove SSN Finder File from the Linux server
#################################################################################
echo "" >> ${LOGNAME}
echo "Delete MEDPAR Baystate SSN finder file from linux data directory"  >> ${LOGNAME}
...
#################################################################################
echo "" >> ${LOGNAME}
echo "Move MEDPAR Baystate SSN finder file to archive folder after successful load into table"  >> ${LOGNAME}
...
#################################################################################
echo "" >> ${LOGNAME}
echo "BAYSTATE_LOAD_SSN_FNDR_FILE.sh completed successfully." >> ${LOGNAME}

SUBJECT="BAYSTATE_LOAD_SSN_FNDR_FILE.sh  - Completed (${ENVNAME})"
MSG="Loading MEDPAR Baystate SSN finder file to Snowflake has completed successfully."
...
if [[ $RET_STATUS != 0 ]]; then
		...
		# Send Failure email	
		SUBJECT="Sending Success email in BAYSTATE_LOAD_SSN_FNDR_FILE.sh - Failed (${ENVNAME})"
		MSG="Sending Success email in BAYSTATE_LOAD_SSN_FNDR_FILE.sh has failed."
		${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
