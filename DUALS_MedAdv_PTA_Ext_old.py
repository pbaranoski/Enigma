#!/usr/bin/env python
########################################################################################################
# Name:  DUALS_MedAdv_PTA_Ext.py
#
# Desc: Duals Quarterly PTA Medicare Advantage extract 
#
...
EXT_STATES=os.getenv('EXT_STATES')
...
def Execute_SQL(ST):
    ...
    print(f"Before Executing SQL DUALS_MedAdv AH-{ST} Extract")

    #**************************************
    #   Extract NYSPAP Bene Contract data  
    #**************************************                         
    snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_DUALS_MA_STG/DUALS_MedAdv_AH_{ST}_{CLM_PRD}_{TMSTMP}.txt.gz
                                                FROM (                                                               

                    SELECT  'AH' as record_type
                            ,C.CLM_TYPE_CD
                            ...
                            ,CASE WHEN CDS.CLM_ACTV_CARE_FROM_DT IS NULL THEN REPEAT(8,' ') ELSE TO_CHAR(CDS.CLM_ACTV_CARE_FROM_DT,'YYYYMMDD') END  AS CLM_ACTV_CARE_FROM_DT

                            ,CASE WHEN CDS.CLM_DSCHRG_DT IS NULL         THEN REPEAT(8,' ') ELSE TO_CHAR(CDS.CLM_DSCHRG_DT,'YYYYMMDD') END  AS CLM_DSCHRG_DT
                            ...
                            ,RPAD(COALESCE(EC.CLM_DRVD_DRG_CD,' '),4,' ')           AS CLM_DRVD_DRG_CD 
                            ,RPAD(COALESCE(CI.DGNS_DRG_CD,' '),4,' ')               AS DGNS_DRG_CD
                            
                            ,RPAD(CASE WHEN CI.CLM_DSCHRG_CD = '~' THEN ' ' ELSE COALESCE(CI.CLM_DSCHRG_CD,' ') END,2,' ') AS CLM_DSCHRG_CD  
                            
                            ,RPAD(COALESCE(CI.CLM_ADMSN_TYPE_CD,' '),2,' ')         AS CLM_ADMSN_TYPE_CD  
                            ,RPAD(COALESCE(CI.CLM_ADMSN_SRC_CD,' '),2,' ')          AS CLM_ADMSN_SRC_CD
                            ...
                            ,RPAD(COALESCE(C.CLM_ATNDG_PRVDR_NPI_NUM,' '),10,' ')   AS  CLM_ATNDG_PRVDR_NPI_NUM
                            ,RPAD(COALESCE(C.CLM_FAC_PRVDR_NPI_NUM,' '),10,' ')     AS  CLM_FAC_PRVDR_NPI_NUM
                            ,RPAD(COALESCE(C.CLM_OPRTG_PRVDR_NPI_NUM,' '),10,' ')   AS  CLM_OPRTG_PRVDR_NPI_NUM 
                    ...
                    AND C.CLM_TYPE_CD IN (  4012,4013,4014,4022,4023,4024,4034,4071,4072,                 
                                            4074,4075,4076,4077,4083,4085,4073,4079,4089,     
                                            4011,4041,4018,4021,4028)                                  
                    AND C.CLM_THRU_DT between '{CLM_FROM_DT}' AND '{CLM_THRU_DT}' 
                    AND ST.GEO_USPS_STATE_CD = '{ST}' 
    ...
    print(f"Before Executing SQL DUALS_MedAdv AL-{ST} Extract")

    #**************************************
    #   Extract NYSPAP Bene Contract data  
    #**************************************   
    snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_DUALS_MA_STG/DUALS_MedAdv_AL_{ST}_{CLM_PRD}_{TMSTMP}.txt.gz
                                                FROM (                                                               

                    SELECT 'AL' as record_type
                            ,C.CLM_TYPE_CD
                            ,RPAD(COALESCE(C.CLM_CNTL_NUM,' '),40,' ')         AS CLM_CNTL_NUM
...
try:
    ...
    #**************************************
    # Convert csv string of states into a python List
    #**************************************
    ST_LIST = EXT_STATES.split(sep=",")
    print(f"{ST_LIST=}")

    #**************************************
    # Execute SQL for each State
    #**************************************
    for ST in ST_LIST:
        Execute_SQL(ST) 
