...
# Paul Baranoski 2025-11-13 Modify SQL to only use C.CLM_BLG_PRVDR_NPI_NUM in where clause instead of all possible NPI numbers in the IDR.
# Paul Baranoski 2026-07-23 Add REQ192 to filename.
########################################################################################################
import os
import sys
import datetime
...
try:
   ...
   #**************************************   
   snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_DOJ_STG/DOJ_REQ192_ModernVascular_{NPI_NUM}_{OBL}_Y{EXT_YR}_{TMSTMP}.csv.gz
                                                FROM (
           ...
           SELECT DISTINCT
              ...
              ,RPAD(COALESCE(TO_CHAR(B.BENE_BRTH_DT, 'YYYYMMDD'),''),8,' ') AS CLM_PTNT_BIRTH_DT
			...
			WHERE C.CLM_TYPE_CD IN ('71','72') 
			  AND CL.CLM_LINE_FROM_DT BETWEEN TO_DATE('{ENUM_DT}','YYYY-MM-DD') AND TO_DATE('{EXT_YR}-12-31','YYYY-MM-DD')
			  AND C.CLM_FINL_ACTN_IND = 'Y'
			  AND CL.CLM_POS_CD = '11'
 
			  AND (
					 C.CLM_BLG_PRVDR_NPI_NUM         IN (SELECT FF.NPI_NUM FROM FF )   
