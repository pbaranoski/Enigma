#!/usr/bin/sh
############################################################################################################
# Script Name: OPMHI_LOAD_DGNS_FNDR_FILE.sh
...
LOGNAME=/app/IDRC/XTR/CMS/logs/OPMHI_LOAD_DGNS_FNDR_FILE_${TMSTMP}.log
...
echo "################################### " >> ${LOGNAME}
echo "OPMHI_LOAD_DGNS_FNDR_FILE.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
rm ${DATADIR}OPMHI_ICD10DG*  >> ${LOGNAME}  2>&1
...
aws s3 ls s3://${S3BUCKET}${PREFIX}  > ${DATADIR}temp_OPMHI_ICD10DGS_EXCL.txt  
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="OPMHI_LOAD_DGNS_FNDR_FILE.sh script - Failed"
...
NOF_FILES=`wc -l ${DATADIR}temp_OPMHI_ICD10DGS_EXCL.txt  | awk '{print $1}' `	2>> ${LOGNAME}

echo "" >> ${LOGNAME}
echo "${NOF_FILES} OPMHI ICD10DGS finder files found in S3." >> ${LOGNAME}
...
filename=`awk '{print $4}' ${DATADIR}temp_OPMHI_ICD10DGS_EXCL.txt` 

echo "" >> ${LOGNAME}
echo "OPMHI ICD10DGS Finder file found: ${filename}" >> ${LOGNAME}

#################################################################################
# Copy S3 Finder File to Linux. 
#################################################################################
echo "" >> ${LOGNAME}
echo "Copy OPMHI ICD10DGS finder file from s3 to linux " >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	echo "" >> ${LOGNAME}
	echo "Copying OPMHI ICD10DGS finder file to Linux failed." >> ${LOGNAME}
	
	# Send Failure email	
	SUBJECT="OPMHI_LOAD_DGNS_FNDR_FILE.sh  - Failed"
...
OPMHI_ICD10DGS_FF=${filename}

#################################################################################
# Execute Python code to load Finder File
#################################################################################
echo "" >> ${LOGNAME}
echo "Start execution of OPMHI_LOAD_DGNS_FNDR_FILE.py program"  >> ${LOGNAME}

# Export environment variables for Python code
export DATADIR
export OPMHI_ICD10DGS_FF

${PYTHON_COMMAND} ${RUNDIR}OPMHI_LOAD_DGNS_FNDR_FILE.py >> ${LOGNAME} 2>&1
...
if [[ $RET_STATUS != 0 ]]; then
        echo "" >> ${LOGNAME}
        echo "Python script OPMHI_LOAD_DGNS_FNDR_FILE.py failed" >> ${LOGNAME}
		
		# Send Failure email	
		SUBJECT="OPMHI_LOAD_DGNS_FNDR_FILE.sh  - Failed"
		MSG="OPMHI loading ICD-10 Diagnosis codes finder file has failed."
		${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${OPMHI_EMAIL_SENDER}" "${OPMHI_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
echo "" >> ${LOGNAME}
echo "Python script OPMHI_LOAD_DGNS_FNDR_FILE.py completed successfully. " >> ${LOGNAME}
...
#################################################################################
# echo "" >> ${LOGNAME}
# echo "Move OPMHI ICD10DGS finder file to archive folder after successful load into table"  >> ${LOGNAME}
...
#################################################################################
echo "" >> ${LOGNAME}
echo "OPMHI_LOAD_DGNS_FNDR_FILE.sh completed successfully." >> ${LOGNAME}

SUBJECT="OPMHI_LOAD_DGNS_FNDR_FILE.sh  - Completed"
MSG="Loading OPMHI ICD-10 diagnosis codes finder file to Snowflake has completed successfully."
