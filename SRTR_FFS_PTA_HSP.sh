#!/usr/bin/bash
############################################################################################################
# Name: SRTR_FFS_PTA_HSP.sh
#
# Desc: SRTR FFS Part A Extract for HSP
#
...
LOGNAME=/app/IDRC/XTR/CMS/logs/SRTR_FFS_PTA_HSP_${TMSTMP}.log
touch ${LOGNAME}
...
echo "################################### " >> ${LOGNAME}
echo "SRTR_FFS_PTA_HSP.sh started at ${TMSTMP} " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="SRTR FFS Part A HSP Extract - Failed"
...
do
	...
	if [[ $RET_STATUS != 0 ]]; then
		...
		# Send Failure email	
		SUBJECT="SRTR_FFS_PTA_HSP.sh  - Failed"
		MSG="SRTR FFS Part A HSP extract has failed in extract step."
		${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_FFS_EMAIL_SENDER}" "${SRTR_FFS_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
		exit 12
	fi

	echo "" >> ${LOGNAME}
	echo "Python script SRTR_FFS_PartA_Extract.py completed successfully for HSP. " >> ${LOGNAME}
done
...
###########################################################################################
echo " " >> ${LOGNAME}
echo "EFT SRTR FFS Part A HSP Extract Files " >> ${LOGNAME}
...
if [ $RET_STATUS != 0 ]; then
	...
	# Send Failure email	
	SUBJECT="SRTR_FFS_PTA_HSP.sh - Failed"
	MSG="EFT for SRTR FFS Part A HSP has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_FFS_EMAIL_SENDER}" "${SRTR_FFS_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
SUBJECT="SRTR FFS Part A HSP Extract" 
MSG="The SRTR FFS Part A HSP extract from Snowflake has completed.\n\nThe following file(s) were created:\n\n${S3Files}"
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="Sending Success email in SRTR_FFS_PTA_HSP.sh  - Failed"
	MSG="Sending Success email in SRTR_FFS_PTA_HSP.sh  has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_FFS_EMAIL_SENDER}" "${SRTR_FFS_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
###########################################################################################
echo "" >> ${LOGNAME}
echo "SRTR_FFS_PTA_HSP.sh completed successfully." >> ${LOGNAME}
