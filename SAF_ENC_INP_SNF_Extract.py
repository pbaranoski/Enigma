#!/usr/bin/env python
########################################################################################################
# Name:  SAF_ENC_INP_SNF_Extract.py
#
...
# Paul Baranoski 2023-06-08 Change SF extract filename for EFT processing rules.
# Paul Baranoski 2023-06-09 Add COALESCE for BENE_MBI_ID
# Paul Baranoski 2023-06-16 Modified WITH CLM_ENC_CLMS SELECT. Had hard-coded IP claim types instead of
#                           using variable CLM_TYPE_CDS.  
# Paul Baranoski 2023-06-16 Mistakenly defaulted blank HCPCS_CD to 'RRRRR'. Have no idea where I got that.
# Paul Baranoski 2023-06-22 Removed columns 26-30 in TMP_PROD_INP_VAL (not used).
...
# Paul Baranoski 2023-08-31 Updated code for No_OF_DAYS_STAYED to remove negative sign (keep absolute value). 
#                           Most likely a coding error.
# Paul Baranoski 2023-09-01 Added code to SELECT of CLM_ADMSN_SRC_CD to remove tildas.
# Paul Baranoski 2025-03-05 CI.DGNS_DRG_CD incorrectly had a minus sign which is a Claim coding issue (not us). Added Replace to remove '-' sign.
########################################################################################################
import os
import sys
import datetime
...
CLM_TYPE_CDS=os.getenv('CLM_TYPE_CDS')
YYYY=os.getenv('YYYY')
...
try:
   ...
   snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_SAFENC{CLM_TYPE_LIT}_STG/SAFENC_{CLM_TYPE_LIT}_{STUS_LIT}_{RNG_LIT}_{TMSTMP}.txt.gz
                                                FROM (
...
WITH CLM_ENC_CLMS AS (
	...
	WHERE C.CLM_TYPE_CD IN ({CLM_TYPE_CDS})
...
,TMP_RLT_OCRNC_SGNTR_MBR_INP_TMP AS (
...
SELECT DISTINCT
	...
	,RPAD(CASE WHEN TRIM(C.CLM_BLG_PRVDR_OSCAR_NUM) = '~' THEN ' ' ELSE COALESCE(C.CLM_BLG_PRVDR_OSCAR_NUM,' ') END,20,' ') AS CLM_BLG_PRVDR_OSCAR_NUM
	...
	,RPAD(COALESCE(CI.CLM_ADMSN_TYPE_CD,' '),2,' ')   AS CLM_ADMSN_TYPE_CD 
	,RPAD(REPLACE(COALESCE(CI.CLM_ADMSN_SRC_CD,' '),'~',''),2,' ')    AS CLM_ADMSN_SRC_CD

	,RPAD(CASE WHEN PROD_MT.CLM_DGNS_PRCDR_ICD_IND = '~' THEN ' ' ELSE COALESCE(PROD_MT.CLM_DGNS_PRCDR_ICD_IND,' ') END,1,' ') AS CLM_DGNS_PRCDR_ICD_IND
	,RPAD(CASE WHEN PROD_MT.CLM_ADMTG_DGNS_CD      = '~' THEN ' ' ELSE COALESCE(PROD_MT.CLM_ADMTG_DGNS_CD,' ') END,7,' ')      AS CLM_ADMTG_DGNS_CD
	
	,REPLACE(TO_CHAR( (C.CLM_THRU_DT - C.CLM_FROM_DT) + 1,'FM000'),'-','') AS  NO_OF_DAYS_STAYED
	,CASE WHEN DT.CLM_DSCHRG_DT IS NULL THEN '10000101' ELSE TO_CHAR(DT.CLM_DSCHRG_DT,'YYYYMMDD') END   AS  CLM_DSCHRG_DT
	,REPLACE(TO_CHAR(CI.DGNS_DRG_CD,'FM0000'),'-','')
	...
	,TO_CHAR(CE.CLM_DRVD_DRG_CD,'FM0000')  AS CLM_DRVD_DRG_CD
	...
	--- POA-TYPE-CD **********
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_1,' '),1,' ')  AS  POA_DGNS_IND_1
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_2,' '),1,' ')  AS  POA_DGNS_IND_2
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_3,' '),1,' ')  AS  POA_DGNS_IND_3
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_4,' '),1,' ')  AS  POA_DGNS_IND_4
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_5,' '),1,' ')  AS  POA_DGNS_IND_5
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_6,' '),1,' ')  AS  POA_DGNS_IND_6
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_7,' '),1,' ')  AS  POA_DGNS_IND_7
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_8,' '),1,' ')  AS  POA_DGNS_IND_8
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_9,' '),1,' ')  AS  POA_DGNS_IND_9
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_10,' '),1,' ') AS  POA_DGNS_IND_10
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_11,' '),1,' ') AS  POA_DGNS_IND_11
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_12,' '),1,' ') AS  POA_DGNS_IND_12
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_13,' '),1,' ') AS  POA_DGNS_IND_13
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_14,' '),1,' ') AS  POA_DGNS_IND_14
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_15,' '),1,' ') AS  POA_DGNS_IND_15
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_16,' '),1,' ') AS  POA_DGNS_IND_16
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_17,' '),1,' ') AS  POA_DGNS_IND_17
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_18,' '),1,' ') AS  POA_DGNS_IND_18
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_19,' '),1,' ') AS  POA_DGNS_IND_19
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_20,' '),1,' ') AS  POA_DGNS_IND_20
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_21,' '),1,' ') AS  POA_DGNS_IND_21
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_22,' '),1,' ') AS  POA_DGNS_IND_22
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_23,' '),1,' ') AS  POA_DGNS_IND_23
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_24,' '),1,' ') AS  POA_DGNS_IND_24
	,RPAD(COALESCE(PDIIT.POA_DGNS_IND_25,' '),1,' ') AS  POA_DGNS_IND_25
	
	--- POA-TYPE-CD-E **********
	,RPAD(COALESCE(PDEIIT.POA_DGNS_E_IND_1,' '),1,' ')  AS POA_DGNS_E_IND_1
	,RPAD(COALESCE(PDEIIT.POA_DGNS_E_IND_2,' '),1,' ')  AS POA_DGNS_E_IND_2
	,RPAD(COALESCE(PDEIIT.POA_DGNS_E_IND_3,' '),1,' ')  AS POA_DGNS_E_IND_3
	,RPAD(COALESCE(PDEIIT.POA_DGNS_E_IND_4,' '),1,' ')  AS POA_DGNS_E_IND_4
	,RPAD(COALESCE(PDEIIT.POA_DGNS_E_IND_5,' '),1,' ')  AS POA_DGNS_E_IND_5
	,RPAD(COALESCE(PDEIIT.POA_DGNS_E_IND_6,' '),1,' ')  AS POA_DGNS_E_IND_6
	,RPAD(COALESCE(PDEIIT.POA_DGNS_E_IND_7,' '),1,' ')  AS POA_DGNS_E_IND_7
	,RPAD(COALESCE(PDEIIT.POA_DGNS_E_IND_8,' '),1,' ')  AS POA_DGNS_E_IND_8
	,RPAD(COALESCE(PDEIIT.POA_DGNS_E_IND_9,' '),1,' ')  AS POA_DGNS_E_IND_9	
	,RPAD(COALESCE(PDEIIT.POA_DGNS_E_IND_10,' '),1,' ') AS POA_DGNS_E_IND_10	
	,RPAD(COALESCE(PDEIIT.POA_DGNS_E_IND_11,' '),1,' ') AS POA_DGNS_E_IND_11
	,RPAD(COALESCE(PDEIIT.POA_DGNS_E_IND_12,' '),1,' ') AS POA_DGNS_E_IND_12
	...
	--- CLM-DGNS-E **********
	,RPAD(COALESCE(DECIT.CLM_DGNS_PRCDR_ICD_IND,' '),1,' ') AS  CLM_DGNS_PRCDR_ICD_IND
	...
	,RPAD(CASE WHEN PROD_MT.CLM_PRCDR_7_CD = '~'   THEN ' ' ELSE COALESCE(PROD_MT.CLM_PRCDR_7_CD,' ' )   END,7,' ') AS CLM_PRCDR_7_CD 
	,RPAD(CASE WHEN PROD_MT.CLM_PRCDR_8_CD = '~'   THEN ' ' ELSE COALESCE(PROD_MT.CLM_PRCDR_8_CD,' ' )   END,7,' ') AS CLM_PRCDR_8_CD 
	,RPAD(CASE WHEN PROD_MT.CLM_PRCDR_9_CD = '~'   THEN ' ' ELSE COALESCE(PROD_MT.CLM_PRCDR_9_CD,' ' )   END,7,' ') AS CLM_PRCDR_9_CD 
	,RPAD(CASE WHEN PROD_MT.CLM_PRCDR_10_CD = '~'  THEN ' ' ELSE COALESCE(PROD_MT.CLM_PRCDR_10_CD,' ' )  END,7,' ') AS CLM_PRCDR_10_CD 
	,RPAD(CASE WHEN PROD_MT.CLM_PRCDR_11_CD = '~'  THEN ' ' ELSE COALESCE(PROD_MT.CLM_PRCDR_11_CD,' ' )  END,7,' ') AS CLM_PRCDR_11_CD 
	,RPAD(CASE WHEN PROD_MT.CLM_PRCDR_12_CD = '~'  THEN ' ' ELSE COALESCE(PROD_MT.CLM_PRCDR_12_CD,' ' )  END,7,' ') AS CLM_PRCDR_12_CD 
	,RPAD(CASE WHEN PROD_MT.CLM_PRCDR_13_CD = '~'  THEN ' ' ELSE COALESCE(PROD_MT.CLM_PRCDR_13_CD,' ' )  END,7,' ') AS CLM_PRCDR_13_CD 
	,RPAD(CASE WHEN PROD_MT.CLM_PRCDR_14_CD = '~'  THEN ' ' ELSE COALESCE(PROD_MT.CLM_PRCDR_14_CD,' ' )  END,7,' ') AS CLM_PRCDR_14_CD 
	,RPAD(CASE WHEN PROD_MT.CLM_PRCDR_15_CD = '~'  THEN ' ' ELSE COALESCE(PROD_MT.CLM_PRCDR_15_CD,' ' )  END,7,' ') AS CLM_PRCDR_15_CD 
	,RPAD(CASE WHEN PROD_MT.CLM_PRCDR_16_CD = '~'  THEN ' ' ELSE COALESCE(PROD_MT.CLM_PRCDR_16_CD,' ' )  END,7,' ') AS CLM_PRCDR_16_CD 
	,RPAD(CASE WHEN PROD_MT.CLM_PRCDR_17_CD = '~'  THEN ' ' ELSE COALESCE(PROD_MT.CLM_PRCDR_17_CD,' ' )  END,7,' ') AS CLM_PRCDR_17_CD 
	,RPAD(CASE WHEN PROD_MT.CLM_PRCDR_18_CD = '~'  THEN ' ' ELSE COALESCE(PROD_MT.CLM_PRCDR_18_CD,' ' )  END,7,' ') AS CLM_PRCDR_18_CD 
	,RPAD(CASE WHEN PROD_MT.CLM_PRCDR_19_CD = '~'  THEN ' ' ELSE COALESCE(PROD_MT.CLM_PRCDR_19_CD,' ' )  END,7,' ') AS CLM_PRCDR_19_CD 	
	,RPAD(CASE WHEN PROD_MT.CLM_PRCDR_20_CD = '~'  THEN ' ' ELSE COALESCE(PROD_MT.CLM_PRCDR_20_CD,' ' )  END,7,' ') AS CLM_PRCDR_20_CD 
	,RPAD(CASE WHEN PROD_MT.CLM_PRCDR_21_CD = '~'  THEN ' ' ELSE COALESCE(PROD_MT.CLM_PRCDR_21_CD,' ' )  END,7,' ') AS CLM_PRCDR_21_CD 
	,RPAD(CASE WHEN PROD_MT.CLM_PRCDR_22_CD = '~'  THEN ' ' ELSE COALESCE(PROD_MT.CLM_PRCDR_22_CD,' ' )  END,7,' ') AS CLM_PRCDR_22_CD 
	,RPAD(CASE WHEN PROD_MT.CLM_PRCDR_23_CD = '~'  THEN ' ' ELSE COALESCE(PROD_MT.CLM_PRCDR_23_CD,' ' )  END,7,' ') AS CLM_PRCDR_23_CD 
	,RPAD(CASE WHEN PROD_MT.CLM_PRCDR_24_CD = '~'  THEN ' ' ELSE COALESCE(PROD_MT.CLM_PRCDR_24_CD,' ' )  END,7,' ') AS CLM_PRCDR_24_CD 
	,RPAD(CASE WHEN PROD_MT.CLM_PRCDR_25_CD = '~'  THEN ' ' ELSE COALESCE(PROD_MT.CLM_PRCDR_25_CD,' ' )  END,7,' ') AS CLM_PRCDR_25_CD 	
	
	,CASE WHEN PROD_MT.CLM_PRCDR_1_DT  IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_1_DT,'YYYYMMDD')  END AS CLM_PRCDR_1_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_2_DT  IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_2_DT,'YYYYMMDD')  END AS CLM_PRCDR_2_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_3_DT  IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_3_DT,'YYYYMMDD')  END AS CLM_PRCDR_3_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_4_DT  IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_4_DT,'YYYYMMDD')  END AS CLM_PRCDR_4_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_5_DT  IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_5_DT,'YYYYMMDD')  END AS CLM_PRCDR_5_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_6_DT  IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_6_DT,'YYYYMMDD')  END AS CLM_PRCDR_6_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_7_DT  IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_7_DT,'YYYYMMDD')  END AS CLM_PRCDR_7_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_8_DT  IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_8_DT,'YYYYMMDD')  END AS CLM_PRCDR_8_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_9_DT  IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_9_DT,'YYYYMMDD')  END AS CLM_PRCDR_9_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_10_DT IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_10_DT,'YYYYMMDD') END AS CLM_PRCDR_10_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_11_DT IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_11_DT,'YYYYMMDD') END AS CLM_PRCDR_11_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_12_DT IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_12_DT,'YYYYMMDD') END AS CLM_PRCDR_12_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_13_DT IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_13_DT,'YYYYMMDD') END AS CLM_PRCDR_13_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_14_DT IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_14_DT,'YYYYMMDD') END AS CLM_PRCDR_14_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_15_DT IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_15_DT,'YYYYMMDD') END AS CLM_PRCDR_15_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_16_DT IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_16_DT,'YYYYMMDD') END AS CLM_PRCDR_16_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_17_DT IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_17_DT,'YYYYMMDD') END AS CLM_PRCDR_17_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_18_DT IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_18_DT,'YYYYMMDD') END AS CLM_PRCDR_18_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_19_DT IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_19_DT,'YYYYMMDD') END AS CLM_PRCDR_19_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_20_DT IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_20_DT,'YYYYMMDD') END AS CLM_PRCDR_20_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_21_DT IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_21_DT,'YYYYMMDD') END AS CLM_PRCDR_21_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_22_DT IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_22_DT,'YYYYMMDD') END AS CLM_PRCDR_22_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_23_DT IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_23_DT,'YYYYMMDD') END AS CLM_PRCDR_23_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_24_DT IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_24_DT,'YYYYMMDD') END AS CLM_PRCDR_24_DT
	,CASE WHEN PROD_MT.CLM_PRCDR_25_DT IS NULL THEN '10000101' ELSE TO_CHAR(PROD_MT.CLM_PRCDR_25_DT,'YYYYMMDD') END AS CLM_PRCDR_25_DT
	...
	,RPAD(COALESCE(PIVT.VAL_CD_1, ' '),2,' ')  AS  VAL_CD_1
	,RPAD(COALESCE(PIVT.VAL_CD_2, ' '),2,' ')  AS  VAL_CD_2
	,RPAD(COALESCE(PIVT.VAL_CD_3, ' '),2,' ')  AS  VAL_CD_3
	,RPAD(COALESCE(PIVT.VAL_CD_4, ' '),2,' ')  AS  VAL_CD_4
	,RPAD(COALESCE(PIVT.VAL_CD_5, ' '),2,' ')  AS  VAL_CD_5
	,RPAD(COALESCE(PIVT.VAL_CD_6, ' '),2,' ')  AS  VAL_CD_6
	,RPAD(COALESCE(PIVT.VAL_CD_7, ' '),2,' ')  AS  VAL_CD_7
	,RPAD(COALESCE(PIVT.VAL_CD_8, ' '),2,' ')  AS  VAL_CD_8
	,RPAD(COALESCE(PIVT.VAL_CD_9, ' '),2,' ')  AS  VAL_CD_9
	,RPAD(COALESCE(PIVT.VAL_CD_10, ' '),2,' ') AS  VAL_CD_10
	,RPAD(COALESCE(PIVT.VAL_CD_11, ' '),2,' ') AS  VAL_CD_11
	,RPAD(COALESCE(PIVT.VAL_CD_12, ' '),2,' ') AS  VAL_CD_12
	,RPAD(COALESCE(PIVT.VAL_CD_13, ' '),2,' ') AS  VAL_CD_13
	,RPAD(COALESCE(PIVT.VAL_CD_14, ' '),2,' ') AS  VAL_CD_14
	,RPAD(COALESCE(PIVT.VAL_CD_15, ' '),2,' ') AS  VAL_CD_15
	,RPAD(COALESCE(PIVT.VAL_CD_16, ' '),2,' ') AS  VAL_CD_16
	,RPAD(COALESCE(PIVT.VAL_CD_17, ' '),2,' ') AS  VAL_CD_17
	,RPAD(COALESCE(PIVT.VAL_CD_18, ' '),2,' ') AS  VAL_CD_18
	,RPAD(COALESCE(PIVT.VAL_CD_19, ' '),2,' ') AS  VAL_CD_19
	,RPAD(COALESCE(PIVT.VAL_CD_20, ' '),2,' ') AS  VAL_CD_20
	,RPAD(COALESCE(PIVT.VAL_CD_21, ' '),2,' ') AS  VAL_CD_21
	,RPAD(COALESCE(PIVT.VAL_CD_22, ' '),2,' ') AS  VAL_CD_22
	,RPAD(COALESCE(PIVT.VAL_CD_23, ' '),2,' ') AS  VAL_CD_23
	,RPAD(COALESCE(PIVT.VAL_CD_24, ' '),2,' ') AS  VAL_CD_24
	,RPAD(COALESCE(PIVT.VAL_CD_25, ' '),2,' ') AS  VAL_CD_25

	-- WARNING!! (FORMAT '9(13)V9(2)S') (CHAR(15)) Truncation of sign
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_1,0), '000000000000.00S'),'.','')  AS  VAL_AMT_1 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_2,0), '000000000000.00S'),'.','')  AS  VAL_AMT_2 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_3,0), '000000000000.00S'),'.','')  AS  VAL_AMT_3 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_4,0), '000000000000.00S'),'.','')  AS  VAL_AMT_4 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_5,0), '000000000000.00S'),'.','')  AS  VAL_AMT_5 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_6,0), '000000000000.00S'),'.','')  AS  VAL_AMT_6 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_7,0), '000000000000.00S'),'.','')  AS  VAL_AMT_7 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_8,0), '000000000000.00S'),'.','')  AS  VAL_AMT_8 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_9,0), '000000000000.00S'),'.','')  AS  VAL_AMT_9 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_10,0),'000000000000.00S'),'.','') AS  VAL_AMT_10 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_11,0),'000000000000.00S'),'.','') AS  VAL_AMT_11 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_12,0),'000000000000.00S'),'.','') AS  VAL_AMT_12 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_13,0),'000000000000.00S'),'.','') AS  VAL_AMT_13 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_14,0),'000000000000.00S'),'.','') AS  VAL_AMT_14 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_15,0),'000000000000.00S'),'.','') AS  VAL_AMT_15 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_16,0),'000000000000.00S'),'.','') AS  VAL_AMT_16 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_17,0),'000000000000.00S'),'.','') AS  VAL_AMT_17 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_18,0),'000000000000.00S'),'.','') AS  VAL_AMT_18 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_19,0),'000000000000.00S'),'.','') AS  VAL_AMT_19 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_20,0),'000000000000.00S'),'.','') AS  VAL_AMT_20 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_21,0),'000000000000.00S'),'.','') AS  VAL_AMT_21 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_22,0),'000000000000.00S'),'.','') AS  VAL_AMT_22 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_23,0),'000000000000.00S'),'.','') AS  VAL_AMT_23 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_24,0),'000000000000.00S'),'.','') AS  VAL_AMT_24 
	,REPLACE(TO_CHAR(COALESCE(PIVT.VAL_AMT_25,0),'000000000000.00S'),'.','') AS  VAL_AMT_25 
...
LEFT OUTER JOIN TMP_POA_DGNS_IND_INP PDIIT
...
LEFT OUTER JOIN TMP_POA_DGNS_E_IND_INP PDEIIT
...
LEFT OUTER JOIN TMP_DGNS_E_CD_INP DECIT
...
LEFT OUTER JOIN TMP_OCRNC_SGNTR_MBR_INP COSM
ON COSM.CLM_OCRNC_SGNTR_SK  = C.CLM_OCRNC_SGNTR_SK

LEFT OUTER JOIN TMP_RLT_OCRNC_SGNTR_MBR_INP_TMP CROSM
ON CROSM.CLM_RLT_OCRNC_SGNTR_SK = C.CLM_RLT_OCRNC_SGNTR_SK

LEFT OUTER JOIN TMP_RLT_COND_SGNTR_MBR_INP CRCSM
ON C.CLM_RLT_COND_SGNTR_SK  = CRCSM.CLM_RLT_COND_SGNTR_SK

LEFT OUTER JOIN TMP_PROD_INP_VAL PIVT
ON  C.GEO_BENE_SK      = PIVT.GEO_BENE_SK
AND C.CLM_DT_SGNTR_SK  = PIVT.CLM_DT_SGNTR_SK
AND C.CLM_TYPE_CD      = PIVT.CLM_TYPE_CD
AND C.CLM_NUM_SK       = PIVT.CLM_NUM_SK
...
--WHERE C.CLM_TYPE_CD  IN (4018,4021,4028) /* SNF */
--WHERE C.CLM_TYPE_CD  IN (4011,4041) /* IP */
WHERE C.CLM_TYPE_CD  IN ({CLM_TYPE_CDS})
--and C.CLM_FINL_ACTN_IND = 'Y'
and CE.CLM_EDPS_STUS_CD = 'A'
AND CLE.CLM_LINE_ENCTR_STUS_CD <> '84'
