#!/usr/bin/bash
############################################################################################################
# Name: RAND_FFS_PTA_INP.sh
# Desc: RAND FFS Part A Extract for INP
#
...
LOGNAME=/app/IDRC/XTR/CMS/logs/${TESTLOG}RAND_FFS_PTA_INP_${TMSTMP}.log
touch ${LOGNAME}
chmod 666 ${LOGNAME} 2>> ${LOGNAME} 

echo "################################### " >> ${LOGNAME}
echo "RAND_FFS_PTA_INP.sh started at ${TMSTMP} " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
echo "" >> ${LOGNAME}
echo "Python script RAND_FFS_PartA_Extract.py completed successfully for INP. " >> ${LOGNAME}
...
do
	...
	if [[ $RET_STATUS != 0 ]]; then
		echo "" >> ${LOGNAME}
		echo "RAND_FFS_PTA_INP.sh failed during the combine step" >> ${LOGNAME}

		#Send Failure email	
		SUBJECT="RAND FFS Part A INP Extract - Failed (${ENVNAME}${TESTEMAIL})"
		MSG="RAND_FFS_PTA_INP.sh failed while combining INP S3 files."
...
SUBJECT="RAND FFS Part A INP Extract - COMPLETE (${ENVNAME}${TESTEMAIL})"
MSG="RAND_FFS_PTA_INP.sh completed successfully.\n\nThe following file(s) were created:\n\n${FILE_LIST}"
${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${RAND_FFS_EMAIL_SENDER}" "${RAND_FFS_EMAIL_SUCCESS_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1

if [[ $RET_STATUS != 0 ]]; then
	echo "" >> ${LOGNAME}
	echo "RAND_FFS_PTA_INP.sh failed - Error in calling sendEmail.py" >> ${LOGNAME}
	
	# Send Failure email	
	SUBJECT="RAND_FFS_PTA_INP.sh failed - Error in calling sendEmail.py (${ENVNAME}${TESTEMAIL})"
	MSG="Sending Success email in RAND_FFS_PTA_INP.sh  has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${RAND_FFS_EMAIL_SENDER}" "${RAND_FFS_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
if [[ $RET_STATUS != 0 ]]; then
		...
		# Send Failure email	
		SUBJECT="Create Manifest file in RAND_FFS_PTA_INP.sh - Failed (${ENVNAME}${TESTEMAIL})"
		MSG="Create Manifest file in RAND_FFS_PTA_INP.sh has failed."
...
###########################################################################################
echo "" >> ${LOGNAME}
echo "RAND_FFS_PTA_INP.sh completed successfully." >> ${LOGNAME}
