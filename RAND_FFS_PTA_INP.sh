...
LOGNAME=/app/IDRC/XTR/CMS/logs/RAND_FFS_PTA_INP_${TMSTMP}.log
touch ${LOGNAME}
...
do
	...
	if [[ $RET_STATUS != 0 ]]; then
		...
		#Send Failure email	
		SUBJECT="RAND FFS Part A INP Extract - Failed (${ENVNAME})"
...
SUBJECT="RAND FFS Part A INP Extract - COMPLETE (${ENVNAME})"
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="RAND_FFS_PTA_INP.sh failed - Error in calling sendEmail.py (${ENVNAME})"
...
if [[ $RET_STATUS != 0 ]]; then
		...
		# Send Failure email	
		SUBJECT="Create Manifest file in RAND_FFS_PTA_INP.sh - Failed (${ENVNAME})"
