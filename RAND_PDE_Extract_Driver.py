#!/usr/bin/bash
############################################################################################################
# Name: RAND_PDE_Extract_Driver.py
#
...
# Paul Baranoski    2026-04-30 Convert from bash to python. Move "driver" logic out of SQL module into "driver module. 
#                              Called new SQL module RAND_PartB_Extract_v2.py. 
############################################################################################################
########################################################################################################
# Set TESTING status 
########################################################################################################
...
def main_processing_loop():
    ...
    try:    
        ...
        LOGNAME = f"{LOG_DIR}{TESTLOG}RAND_PTD_Extract_{TMSTMP}.log"
        ...
        rootLogger.info(f"\nRAND_PDE_Extract_Driver.py started at {TMSTMP}")
        ...
        #############################################################
        rootLogger.info("")
        rootLogger.info("RAND FFS PTB DME Processing will use the following parameters:")
        rootLogger.info(f"{EXT_YYYY=}")
        rootLogger.info(f"{CURR_YEAR=}")
        ...
        ext_months = [
            {"ext_month":"JAN",
             "start_dates": ["-01-01","-01-16"],
             "end_date": ["-01-15","-01-31"]
            },
            {"ext_month":"FEB",
             "start_dates": ["-02-01","-02-16"],
             "end_date": ["-02-15","-02-28"]
            },
            {"ext_month":"MAR",
             "start_dates": ["-03-01","-03-16"],
             "end_date": ["-03-15","-03-31"]
            },
            {"ext_month":"APR",
             "start_dates": ["-04-01","-04-16"],
             "end_date": ["-04-15","-04-30"]
            },
            {"ext_month":"MAY",
             "start_dates": ["-05-01","-05-16"],
             "end_date": ["-05-15","-05-31"]
            },
            {"ext_month":"JUN",
             "start_dates": ["-06-01","-06-16"],
             "end_date": ["-06-15","-06-30"]
            },
            {"ext_month":"JUL",
             "start_dates": ["-07-01","-07-16"],
             "end_date": ["-07-15","-07-31"]
            },
            {"ext_month":"AUG",
             "start_dates": ["-08-01","-08-16"],
             "end_date": ["-08-15","-08-31"]
            },
            {"ext_month":"SEP",
             "start_dates": ["-09-01","-09-16"],
             "end_date": ["-09-15","-09-30"]
            },
            {"ext_month":"OCT",
             "start_dates": ["-10-01","-10-16"],
             "end_date": ["-10-15","-10-31"]
            },
            {"ext_month":"NOV",
             "start_dates": ["-11-01","-11-16"],
             "end_date": ["-11-15","-11-30"]
            },
            {"ext_month":"DEC",
             "start_dates": ["-12-01","-12-16"],
             "end_date": ["-12-15","-12-31"]
            }  
        ]
            
        for i in range(len(ext_months)):
            
            MONTH_DESC = ext_months[i]['ext_month']
            rootLogger.info(f"{MONTH_DESC=}")
                            
            for j in range(len(ext_months[i]["start_dates"])):

                rootLogger.info(f"******************************")
                rootLogger.info(f"Process next Extract file")

                RNG = j + 1
                rootLogger.info(f"{RNG=}")
            
                # Ex. 2026-01-01, 2026-03-31
                START_DATE = f"{EXT_YYYY}{ext_months[i]['start_dates'][j]}"
                END_DATE = f"{EXT_YYYY}{ext_months[i]['end_date'][j]}"

                # Is it a leap year and last week on Feb --> overide END_DATE
                if ext_months[i]['ext_month'] == "FEB" and RNG == 4:
                    is_leap_year = calendar.isleap(int(EXT_YYYY))
                    if is_leap_year == True:
                        END_DATE = f"{EXT_YYYY}-02-29"

                XTR_FILE_NAME = f"RAND_FFS_PDE_Y{EXT_YYYY}_{MONTH_DESC}P{RNG}_{TMSTMP}.csv.gz"
                ...
                #############################################################
                rootLogger.info("")
                rootLogger.info("Start execution of RAND_PDE_Extract_v2.py program")

                try:
                    sp_info = subprocess.run(['python3', 'RAND_PDE_Extract_v2.py'], capture_output=True, text=True, check=True)

                    write_sp_info_2_log(sp_info) 
                    
                except subprocess.CalledProcessError as e:
                    rootLogger.error(f"Calling RAND_PDE_Extract_v2.py failed with return code {e.returncode}")
                    rootLogger.error(e.output)
                    
                    ## Send Failure email	
                    SUBJECT=f"RAND_PDE_Extract_Driver.py extract - Failed ({ENVNAME}{TESTEMAIL})"
                    MSG=f"Calling RAND_PDE_Extract_v2.py failed with return code {e} "
                ...
                # call was successful
                rootLogger.info("Python script RAND_PDE_Extract_v2.py completed successfully. ")
                ...
                RAND_FFSPTAB_BUCKET = f"{XTR_BUCKET}/{RAND_PDE_BUCKET_FLDR}"
                ...
                try:
                    ...
                    ## Send Failure email	
                    SUBJECT=f"Combining S3 files in RAND_PDE_Extract_Driver.py - Failed ({ENVNAME}{TESTEMAIL})"
                    MSG=f"Combining S3 files in RAND_PDE_Extract_Driver.py has failed."
        ...
        SUBJECT=f"RAND PDE extract ({ENVNAME}{TESTEMAIL})" 
        MSG=f"The RAND PDE extract from Snowflake has completed.\n\nThe following file(s) were created:\n\n{S3Files}"
        
        try:
            sp_info = subprocess.run(['python3', 'sendEmail.py', CMS_EMAIL_SENDER, RAND_PDE_EMAIL_SUCCESS_RECIPIENT, SUBJECT, MSG], capture_output=True, text=True, check=True)
            write_sp_info_2_log(sp_info)
        ...
        ######################################################################
        rootLogger.info("")
        rootLogger.info(f"Creating manifest files for RAND FFS PDE Extract. ")
        ...
        #####################################################
        try:
            CreManDr.createManifestFile(bucket=XTR_BUCKET, s3folder=RAND_PDE_BUCKET_FLDR, runToken=TMSTMP, BoxEmails=RAND_FFS_BOX_RECIPIENTS, Manifest_folder=MANIFEST_HOLD_BUCKET_FLDR )

        except Exception as e:

            SUBJECT=f"Create Manifest file in RAND_PDE_Extract_Driver.py - Failed ({ENVNAME}{TESTEMAIL})"
            MSG=f"Create Manifest file in RAND_PDE_Extract_Driver.py  has failed. {e}"
        ...
        rootLogger.info("RAND FFS PDE Extract completed successfully.")
        rootLogger.info(f"\nEnded at {TMSTMP}" )


    except Exception as e:
        print (f"Exception occured in RAND_PDE_Extract_Driver.py\n {e}")

        rootLogger.error("Exception occured in RAND_PDE_Extract_Driver.py.")
        rootLogger.error("\n%s", str(e))

        ## Send Failure email	
        SUBJECT=f"RAND_PDE_Extract_Driver.py  - Failed ({ENVNAME}{TESTEMAIL})"
        MSG=f"Exception occured in RAND_PDE_Extract_Driver.py {e}. Process failed. "
