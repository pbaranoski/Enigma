   ...
   snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_DOJ_STG/DOJ_ModernVascular_{NPI_NUM}_{OBL}_Y{EXT_YR}_{TMSTMP}.csv.gz
			...
			WHERE C.CLM_TYPE_CD IN ('71','72') 
			  ...
			  AND CL.CLM_POS_CD = '11'
                 ...
                 -- These NPIs are in results-set
                 --     C.CLM_BLG_PRVDR_NPI_NUM
                 ----   C.PRVDR_BLG_PRVDR_NPI_NUM
                 --     C.CLM_FAC_PRVDR_NPI_NUM
                 ----   C.PRVDR_RFRG_PRVDR_NPI_NUM
                 --     C.CLM_RFRG_PRVDR_NPI_NUM
                 --     CL.CLM_RNDRG_PRVDR_NPI_NUM
                 ----   CL.PRVDR_RNDRNG_PRVDR_NPI_NUM	
                 --     CL.CLM_RNDRG_PRVDR_GRP_NPI_NUM	
                 ----   CL.PRVDR_RNDRNG_PRVDR_GRP_NPI_NUM

			  AND (
					 CLM_ATNDG_PRVDR_NPI_NUM         IN (SELECT FF.NPI_NUM FROM FF ) OR
				     C.CLM_BLG_PRVDR_NPI_NUM         IN (SELECT FF.NPI_NUM FROM FF ) OR
				     C.CLM_OPRTG_PRVDR_NPI_NUM       IN (SELECT FF.NPI_NUM FROM FF ) OR
				     C.CLM_OTHR_PRVDR_NPI_NUM        IN (SELECT FF.NPI_NUM FROM FF ) OR
				     C.CLM_FAC_PRVDR_NPI_NUM         IN (SELECT FF.NPI_NUM FROM FF ) OR
				     C.CLM_SRVC_PRVDR_NPI_NUM        IN (SELECT FF.NPI_NUM FROM FF ) OR
                     C.CLM_RFRG_PRVDR_NPI_NUM        IN (SELECT FF.NPI_NUM FROM FF ) OR
                     C.PRVDR_CPO_FAC_NPI_NUM         IN (SELECT FF.NPI_NUM FROM FF ) OR
				     CL.CLM_RNDRG_PRVDR_NPI_NUM      IN (SELECT FF.NPI_NUM FROM FF ) OR
                     CL.CLM_RNDRG_PRVDR_GRP_NPI_NUM	 IN (SELECT FF.NPI_NUM FROM FF ) 
			  )
