...
LOGNAME=/app/IDRC/XTR/CMS/logs/PSA_Outpatient_${TMSTMP}.log
RUNDIR=/app/IDRC/XTR/CMS/scripts/run/
DATADIR=/app/IDRC/XTR/CMS/data/

PSA_FF_OPT_PREFIX=PSA_FINDER_FILE_APC_Categories
...
echo "################################### " >> ${LOGNAME}
echo "PSA_Outpatient.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
rm ${DATADIR}tempPSAOPT.txt  >> ${LOGNAME} 2>&1
...
echo "List ${PSA_FF_OPT_PREFIX}_* finder files in S3 " >> ${LOGNAME}

S3_FINDER_FILES=`aws s3 ls s3://${FINDER_FILE_BUCKET}${PSA_FF_OPT_PREFIX}`  >> ${LOGNAME} 
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="PSA_Outpatient.sh - Failed (${ENVNAME})"
	MSG="Listing S3 finder files from ${FINDER_FILE_BUCKET}${PSA_FF_OPT_PREFIX} failed."
...
if [[ $RET_STATUS != 0 ]]; then
	echo "" >> ${LOGNAME}
	echo "Shell script PSA_Outpatient.sh failed." >> ${LOGNAME}
...
# Ex. PIV_COL_DESC_LIST = 'Drugs, Vaccines','Special OPPS Payment Status'
#     PIV_COL_NAME_LIST = "Drugs, Vaccines","Special OPPS Payment Status"
#################################################################################
echo "" >> ${LOGNAME}
...
# 1) Skip header; 2) Finder file has 3 fields
tail -n +2 ${DATADIR}${FINDER_FILE} | cut -d, -f3- | sort -u > ${DATADIR}tempPSAOPT.txt  
...
#############################################################
echo "" >> ${LOGNAME}
echo "Start execution of PSA_Outpatient.py program"  >> ${LOGNAME}
...
${PYTHON_COMMAND} ${RUNDIR}PSA_Outpatient.py >> ${LOGNAME} 2>&1
...
if [[ $RET_STATUS != 0 ]]; then
	echo "" >> ${LOGNAME}
	echo "Python script PSA_Outpatient.py failed" >> ${LOGNAME}
	
	# Send Failure email	
	SUBJECT="PSA_Outpatient.sh  - Failed ${ENVNAME}"
	MSG="PSA Outpatient Extract has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${PSA_EMAIL_SENDER}" "${PSA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
echo "" >> ${LOGNAME}
echo "Python script PSA_Outpatient.py completed successfully. " >> ${LOGNAME}
...
SUBJECT="PSA Outpatient extract (${ENVNAME})" 
MSG="The Extract for the creation of the PSA Outpatient extract file from Snowflake has completed.\n\nThe following file(s) were created:\n\n${S3Files}"
...
if [[ $RET_STATUS != 0 ]]; then
		...
		# Send Failure email	
		SUBJECT="Sending Success email in PSA_Outpatient.sh  - Failed ${ENVNAME}"
		MSG="Sending Success email in PSA_Outpatient.sh  has failed."
		${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${PSA_EMAIL_SENDER}" "${PSA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#################################################################################
echo "" >> ${LOGNAME}
echo "Move processed S3 PSA Outpatient Finder File to archive directory in S3." >> ${LOGNAME}
...
#############################################################
echo "" >> ${LOGNAME} 
echo "Remove tempPSAOPT.txt from data directory" >> ${LOGNAME} 

rm ${DATADIR}tempPSAOPT.txt >> ${LOGNAME} 2>&1
rm ${DATADIR}${PSA_FF_OPT_PREFIX}*  >> ${LOGNAME} 2>&1
...
#############################################################
echo "" >> ${LOGNAME}
echo "PSA_Outpatient.sh completed successfully." >> ${LOGNAME}
