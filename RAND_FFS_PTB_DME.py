...
def main_processing_loop():
    ...
    try:    
        ...
        LOGNAME = f"{LOG_DIR}{TESTLOG}RAND_FFS_PTB_DME_{TMSTMP}.log"
        ...
        rootLogger.info(f"\nRAND_FFS_PTB_DME.py started at {TMSTMP}")
        ...
        for i in range(len(start_date_parms)):
            # Ex. 2026-01-01, 2026-06-30
            START_DATE = f"{EXT_YYYY}{start_date_parms[i]}"
            END_DATE = f"{EXT_YYYY}{end_date_parms[i]}"
            RNG = i + 1

            XTR_FILE_NAME = f"RAND_FFS_DME_Y{EXT_YYYY}_P{RNG}_{TMSTMP}.csv.gz"
            ...
            try:
                ...
                ## Send Failure email	
                SUBJECT=f"RAND_FFS_PTB_DME.py extract - Failed ({ENVNAME}{TESTEMAIL})"
            ...
            try:
                ...
                ## Send Failure email	
                SUBJECT=f"Combining S3 files in RAND_FFS_PTB_DME.py - Failed ({ENVNAME}{TESTEMAIL})"
                MSG=f"Combining S3 files in RAND_FFS_PTB_DME.py has failed."
        ...
        SUBJECT=f"RAND FFS PTB DME Extract Complete ({ENVNAME}{TESTEMAIL})" 
        MSG=f"RAND FFS PTB DME Extract completed successfully.\n\nThe following file(s) were created:\n\n{S3Files}"
        ...
        ######################################################################
        rootLogger.info("")
        rootLogger.info(f"Creating manifest files for RAND FFS PTB DME Extract. ")
        ...
        try:
            ...
            SUBJECT=f"Create Manifest file in RAND_FFS_PTB_DME.py - Failed ({ENVNAME}{TESTEMAIL})"
            MSG=f"Create Manifest file in RAND_FFS_PTB_DME.py  has failed. {e}"
        ...
        rootLogger.info("RAND FFS PTB DME Extract completed successfully.")
        rootLogger.info(f"\nEnded at {TMSTMP}" )


    except Exception as e:
        print (f"Exception occured in RAND_FFS_PTB_DME.py\n {e}")

        rootLogger.error("Exception occured in RAND_FFS_PTB_DME.py.")
        rootLogger.error("\n%s", str(e))

        ## Send Failure email	
        SUBJECT=f"RAND_FFS_PTB_DME.py  - Failed ({ENVNAME}{TESTEMAIL})"
        MSG=f"Exception occured in RAND_FFS_PTB_DME.py {e}. Process failed. "
