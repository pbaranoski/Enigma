#!/usr/bin/env python
########################################################################################################
# Name:  PAC_SNF_CLMS.py
#
# Desc: Script to Extract Skilled Nursing Facility(SNF) Claims Data
#
...
try:
   ...
   #**************************************
   #   Extract Skilled Nursing Facility(SNF) Claims Data
   #**************************************   
   snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_PAC_STG/PAC_SNF_CLMS_Y{FYQ}_{TMSTMP}.csv.gz
                                                FROM (
SELECT
 DISTINCT       
...
,ABS(LPAD(TO_CHAR(CI.CLM_HIPPS_VBP_AMT,'FM999999990.00'),12))       AS CLM_HIPPS_VBP_AMT
