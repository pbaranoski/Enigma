#!/usr/bin/bash
############################################################################################################
# Script Name: BAYSTATE_Extract.sh
# Description: This script executes the full SNN extraction python script for MEDPAR Baystate
#
...
CUR_DT=`date +%Y%m%d`
LOGNAME=/app/IDRC/XTR/CMS/logs/BAYSTATE_Extract_${TMSTMP}.log
...
echo "################################### " >> ${LOGNAME}
echo "BAYSTATE_Extract.sh started at `date` " >> ${LOGNAME}
...
#################################################################################
echo "" >> ${LOGNAME}
echo "Start execution of BAYSTATE_Extract.py program"  >> ${LOGNAME}

${PYTHON_COMMAND} ${RUNDIR}BAYSTATE_Extract.py >> ${LOGNAME} 2>&1
...
if [[ $RET_STATUS != 0 ]]; then
        echo "" >> ${LOGNAME}
        echo "Python script BAYSTATE_Extract.py failed" >> ${LOGNAME}
		
        # Send Failure email	
        SUBJECT="BAYSTATE_Extract.sh  - Failed (${ENVNAME})"
        MSG="MEDPBAR Baystate SSN extract has failed."
...
echo "" >> ${LOGNAME}
echo "Python script BAYSTATE_Extract.py completed successfully. " >> ${LOGNAME}
...
SUBJECT="BAYSTATE_Extract.sh  - Completed (${ENVNAME})"
MSG="MEDPAR Baystate SSN Extract has completed successfully.\n\nThe following file(s) were created:\n\n${S3Files}"
...
if [[ $RET_STATUS != 0 ]]; then
		...
		# Send Failure email	
		SUBJECT="Sending Success email in BAYSTATE_Extract.sh - Failed (${ENVNAME})"
		MSG="Sending Success email in BAYSTATE_Extract.sh  has failed."
		${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#############################################################
echo "" >> ${LOGNAME}
echo "EFT MEDPAR Baystate SSN Extract.  " >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="EFT file step in BAYSTATE_Extract.sh - Failed (${ENVNAME})"
	MSG="EFT file step in BAYSTATE_Extract.sh has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#################################################################################
echo "" >> ${LOGNAME}
echo "BAYSTATE_Extract.sh completed successfully." >> ${LOGNAME}
