...
# Paul Baranoski 2025-01-23 Update ranges for Home Health (Category 4) 9000-9799 Continuation of Home Health Agencies (Update range)
#	                        Other Services (Category 5) 6500-6989 Outpatient Physical Therapy Services (Update range) 
#                           (Removed as this is mapped to SNF  - category 3)
#	                         Continuation of Home Health Agencies (Category 4) 7000-8499 (to be added)
########################################################################################################
# IMPORTS
########################################################################################################
import os
...
try:
   ...
   snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_STS_PTA_BPYMTS_STG/STS_PTA_BPYMTS_RPT_A-1AB_{EXT_TO_YYYY}_{RUN_PRD}_{TMSTMP}.csv.gz
                    FROM (
                ...
                WITH RPT_CLM_TYPE_CATEGORIES AS (
                    ...
                    UNION 
                    SELECT '5' AS RPT_CLM_TYPE_CD, 'Other Services'           AS RPT_CLM_TYPE_DESC  FROM DUAL
                    UNION
                ...
                --SELECT * FROM PARTA_SUM_DATA_BY_CLM_TYPE;
                ...
                --SELECT *
                --FROM ALL_RPT_DATA R
                --WHERE RPT_CLM_TYPE_CD IN ('1','2')
                --ORDER BY R.CAL_YEAR, R.GEO_STATE_NAME, R.RPT_CLM_TYPE_CD, R.RPT_PPS_CD
                --;

                ,RPT_DATA_FORMATTED  AS  (

                    SELECT GEO_STATE_NAME, ST_SORT_ORD, CAL_YEAR 
                ...
                ,RPT_DATA_ALL_AREAS  AS  (
                    ...
                    SELECT 
                         ...
                         ,'6'           AS ST_SORT_ORD   
                         ,CAL_YEAR
                         ,SUM(TOT_NOF_BILLS) AS TOT_NOF_BILLS
                         ,SUM(TOT_CHRG_AMT)  AS TOT_CHRG_AMT
                         ,SUM(TOT_PMT_AMT)   AS TOT_PMT_AMT
                    
                         ,SUM(INP_NOF_BILLS) AS INP_NOF_BILLS
                         ,SUM(INP_CHRG_AMT) AS INP_CHRG_AMT
                         ,SUM(INP_PMT_AMT) AS INP_PMT_AMT
                     
                         ,SUM(SNF_NOF_BILLS) AS SNF_NOF_BILLS
                         ,SUM(SNF_CHRG_AMT) AS SNF_CHRG_AMT
                         ,SUM(SNF_PMT_AMT) AS SNF_PMT_AMT
                    
                         ,SUM(HHA_NOF_BILLS) AS HHA_NOF_BILLS
                         ,SUM(HHA_CHRG_AMT) AS HHA_CHRG_AMT
                         ,SUM(HHA_PMT_AMT) AS HHA_PMT_AMT
                    
                         ,SUM(HSP_NOF_BILLS) AS HSP_NOF_BILLS
                         ,SUM(HSP_CHRG_AMT) AS HSP_CHRG_AMT
                         ,SUM(HSP_PMT_AMT) AS HSP_PMT_AMT	
