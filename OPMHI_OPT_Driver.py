...
OPMHI_OPT_BUCKET = rf"{XTR_BUCKET}/{OPMHI_OPT_BUCKET_FLDR}"
...
def main_processing_loop():
    ...
    try:    
        ...
        LOGNAME = f"{LOG_DIR}{TESTLOG}OPMHI_OPT_Driver_{TMSTMP}.log"
        ...
        rootLogger.info(f"\nOPMHI_OPT_Driver.py started at {TMSTMP}")
        ...
        deleteFilesFromLinuxUsingPrefix(DATA_DIR, "FEHB_CMS_OPT")
        ...
        #############################################################
        rootLogger.info(f"Remove s3 file {OPMHI_OPT_BUCKET_FLDR}{EXT_FILENAME} if it already exists in s3.")
        s3_client.delete_object(Bucket=XTR_BUCKET, Key=f"{OPMHI_OPT_BUCKET_FLDR}{EXT_FILENAME}")
        rootLogger.info(f"Remove s3 file {OPMHI_OPT_BUCKET_FLDR}{EXT_FILENAME}.gpg if it already exists in s3.")
        s3_client.delete_object(Bucket=XTR_BUCKET, Key=f"{OPMHI_OPT_BUCKET_FLDR}{EXT_FILENAME}.gpg")
        ...
        rootLogger.info(f"{OPMHI_OPT_BUCKET=} ")

        sConcatFilename = f"FEHB_CMS_{CTYP}_{START_DT}_{END_DT}_{CUR_DATE}.txt.gz"
        rootLogger.info(f"{sConcatFilename=}")

        try:
            sp_info = subprocess.run(['bash', 'CombineS3Files.sh', OPMHI_OPT_BUCKET, sConcatFilename ], capture_output=True, text=True, check=True)
            write_sp_info_2_log(sp_info) 
            ...
            ## Send Failure email	
            SUBJECT=f"Combining S3 files in OPMHI_OPT_Driver.py - Failed ({ENVNAME})"
            MSG=f"Combining S3 files in OPMHI_OPT_Driver.py has failed."
        ...
        # Send Success email	
        SUBJECT=f" OPMHI_OPT_Driver- completed ({ENVNAME}{TESTEMAIL})"
        MSG=f" OPMHI_OPT_Driver completed. \n\nThe following extract files were created:\n\n{S3Files}"
        ...
        #############################################################
        downloadFileFromS3(s3_client, XTR_BUCKET, f"{OPMHI_OPT_BUCKET_FLDR}{EXT_FILENAME}", f"{DATA_DIR}{EXT_FILENAME}" )
        ...
        #############################################################
        s3UploadFile(s3_client, f"{DATA_DIR}{EXT_FILENAME}.gpg", XTR_BUCKET, f"{OPMHI_OPT_BUCKET_FLDR}{EXT_FILENAME}.gpg")

        #############################################################
        # Move extract .gz file to archive folder so that it is not
        # included in the manifest file.
        #############################################################
        s3MoveFile2NewFolder(s3_client, XTR_BUCKET, f"{OPMHI_OPT_BUCKET_FLDR}{EXT_FILENAME}", f"{OPMHI_OPT_BUCKET_FLDR}archive/{EXT_FILENAME}")
        ...
        MANIFEST_FILE_HLQ = "OPMHI_OPT"
        # Ex. CLAIMS_YYYYMM
        DEST_FLDR = f"CLAIMS_{TMSTMP[:6]}"

        try:
            sp_info = subprocess.run(['bash', 'CreateManifestSFTPFile.sh', OPMHI_OPT_BUCKET, CUR_DATE, OPMHI_BOX_RECIPIENT, MANIFEST_BUCKET_TO_USE, MANIFEST_FILE_HLQ, DEST_FLDR ], capture_output=True, text=True, check=True)
            write_sp_info_2_log(sp_info)
            ...
            SUBJECT=f"Create Manifest file in OPMHI_OPT_Driver.py - Failed ({ENVNAME})"
            MSG=f"Create Manifest file in OPMHI_OPT_Driver.py  has failed."
        ...
        rootLogger.info("Script OPMHI_OPT_Driver.py completed successfully.")
        rootLogger.info(f"\nEnded at {datetime.now().strftime('%Y%m%d.%H%M%S')}" )


    except Exception as e:
        print (f"Exception occured in OPMHI_OPT_Driver.py\n {e}")

        rootLogger.error("Exception occured in OPMHI_OPT_Driver.py.")
        rootLogger.error("\n%s", str(e))

        ## Send Failure email	
        SUBJECT = f"Exception occured in OPMHI_OPT_Driver.py - Failed ({ENVNAME}{TESTEMAIL})"
        MSG= f"Exception occured in OPMHI_OPT_Driver.py {e}.  Process failed"
