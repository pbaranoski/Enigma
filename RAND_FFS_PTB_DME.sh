...
LOGNAME=/app/IDRC/XTR/CMS/logs/${TESTLOG}RAND_FFS_PTB_DME_${TMSTMP}.log
touch ${LOGNAME}
...
do
	...
	if [[ $RET_STATUS != 0 ]]; then
		...
		#Send Failure email	
		SUBJECT="RAND FFS Part B DME Extract - Failed (${ENVNAME}${TESTEMAIL})"
...
#Send Failure email	
SUBJECT="RAND FFS Part B DME Extract - COMPLETE (${ENVNAME}${TESTEMAIL})"
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="RAND_FFS_PTB_DME.sh failed - Error in calling sendEmail.py (${ENVNAME}${TESTEMAIL})"
...
if [[ $RET_STATUS != 0 ]]; then
		...
		# Send Failure email	
		SUBJECT="Create Manifest file in RAND_FFS_PTA_DME.sh - Failed (${ENVNAME}${TESTEMAIL})"
