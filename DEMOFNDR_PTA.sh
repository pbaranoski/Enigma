...
#
######################################################################################
# Name:  DEMOFNDR_PTA.sh
#
# Desc: Performs Extract of PartA data. 
#
...
LOGNAME=/app/IDRC/XTR/CMS/logs/DEMOFNDR_PTA_Extract_${TMSTMP}.log
...
echo "################################### " >> ${LOGNAME}
echo "DEMOFNDR_PTA.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
${PYTHON_COMMAND} ${RUNDIR}DEMOFNDR_PTA.py  >> ${LOGNAME} 2>&1
...
if [[ $RET_STATUS != 0 ]]; then
        echo "" >> ${LOGNAME}
        echo "Python script DEMOFNDR_PTA.py failed." >> ${LOGNAME}
