...
LOGNAME=/app/IDRC/XTR/CMS/logs/RAND_FFS_PTA_HOS_${TMSTMP}.log
touch ${LOGNAME}
...
do
	...
	if [[ $RET_STATUS != 0 ]]; then
		...
		#Send Failure email	
		SUBJECT="RAND FFS Part A HOS Extract - Failed (${ENVNAME})"
...
SUBJECT="RAND FFS Part A HOS Extract - COMPLETE (${ENVNAME})"
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="RAND_FFS_PTA_HOS.sh failed - Error in calling sendEmail.py (${ENVNAME})"
...
if [[ $RET_STATUS != 0 ]]; then
		...
		# Send Failure email	
		SUBJECT="Create Manifest file in RAND_FFS_PTA_HOS.sh - Failed (${ENVNAME})"
