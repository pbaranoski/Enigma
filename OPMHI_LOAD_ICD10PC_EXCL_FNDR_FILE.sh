#!/usr/bin/sh
############################################################################################################
# Script Name: OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE.sh
...
LOGNAME=/app/IDRC/XTR/CMS/logs/OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE_${TMSTMP}.log
...
rm ${DATADIR}OPMHI_ICD10PC*  >> ${LOGNAME}  2>&1

#################################################################################
# Get ICD10PCS EXCL finder files from the finder file bucket
#################################################################################
...
aws s3 ls s3://${S3BUCKET}${PREFIX}  > ${DATADIR}temp_OPMHI_ICD10PCS_EXCL.txt  
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE.sh script - Failed"
...
NOF_FILES=`wc -l ${DATADIR}temp_OPMHI_ICD10PCS_EXCL.txt  | awk '{print $1}' `	2>> ${LOGNAME}

echo "" >> ${LOGNAME}
echo "${NOF_FILES} OPMHI ICD10PCS EXCL finder files found in S3." >> ${LOGNAME}
...
filename=`awk '{print $4}' ${DATADIR}temp_OPMHI_ICD10PCS_EXCL.txt` 

echo "" >> ${LOGNAME}
echo "OPMHI ICD10PCS EXCL Finder file found: ${filename}" >> ${LOGNAME}

#################################################################################
# Copy S3 Finder File to Linux. 
#################################################################################
echo "" >> ${LOGNAME}
echo "Copy OPMHI ICD10PCS EXCL finder file from s3 to linux " >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	echo "" >> ${LOGNAME}
	echo "Copying OPMHI ICD10PCS EXCL finder file to Linux failed." >> ${LOGNAME}
	
	# Send Failure email	
	SUBJECT="OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE.sh  - Failed"
...
OPMHI_ICD10PCS_EXCL_FF=${filename}

#################################################################################
# Execute Python code to load Finder File
#################################################################################
echo "" >> ${LOGNAME}
echo "Start execution of OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE.py program"  >> ${LOGNAME}

# Export environment variables for Python code
export DATADIR
export OPMHI_ICD10PCS_EXCL_FF

${PYTHON_COMMAND} ${RUNDIR}OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE.py >> ${LOGNAME} 2>&1
...
if [[ $RET_STATUS != 0 ]]; then
        echo "" >> ${LOGNAME}
        echo "Python script OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE.py failed" >> ${LOGNAME}
...
echo "" >> ${LOGNAME}
echo "Python script OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE.py completed successfully. " >> ${LOGNAME}
...
#################################################################################
# echo "" >> ${LOGNAME}
# echo "Move OPMHI ICD10PCS EXCL finder file to archive folder after successful load into table"  >> ${LOGNAME}
...
#############################################################
echo "" >> ${LOGNAME}
echo "OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE.sh completed successfully." >> ${LOGNAME}

SUBJECT="OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE.sh  - Completed"
MSG="Loading OPMHI ICD10PCS EXCL Finder File to Snowflake has completed successfully."
