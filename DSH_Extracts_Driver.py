...
def main_processing_loop():
  ...
  try:    
        ...
        LOGNAME = f"{LOG_DIR}{TESTLOG}DSH_Extracts_{TMSTMP}.log" 
        ...
        rootLogger.info(f"\nDSH_Extracts_Driver.py started at {TMSTMP}")
        ...
        for sFF in lstFilenames:
            ...
            if sFF_Ext != "csv":
                ...
                archiveFinderFile(s3_client, XTR_BUCKET, FINDER_FILE_BUCKET_FLDR, sFF)
            ...
            with open(f"{DATADIR}{sFF}", "r", encoding="UTF-8") as f:
                ...
                for sExtRecord in f:
                    ...
                    sp_info = subprocess.run(['python3', 'DSH_Extracts.py'], capture_output=True, text=True, check=True)
                    write_sp_info_2_log(sp_info)  
            ...
            if len(lstFiles4ManifestFile) == 0:
                ...
                try:
                    ...
                    SUBJECT=f"Create Manifest file in DSH_Extracts_Driver.py - Failed ({ENVNAME}{TESTEMAIL})"
                    MSG=f"Create Manifest file in DSH_Extracts_Driver.py has failed."
        ...
        ####################################################################     -
        rootLogger.info("")
        rootLogger.info("DSH_Extracts_Driver.py completed successfully.")
        rootLogger.info(f"\nEnded at {TMSTMP}" )
        sys.exit(0)


  except Exception as e:
        print (f"Exception occured in DSH_Extracts_Driver.py\n {e}")

        rootLogger.error("Exception occured in DSH_Extracts_Driver.py.")
        rootLogger.error("\n%s", str(e))
        
        ## Send Failure email	
        SUBJECT=f"DSH_Extracts_Driver.py  - Failed ({ENVNAME}{TESTEMAIL})"
        MSG=f"Exception occured in DSH_Extracts_Driver.py {e}. Process failed. "
