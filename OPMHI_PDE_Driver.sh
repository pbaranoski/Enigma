#!/usr/bin/bash
############################################################################################################
# Script Name: OPMHI_PDE_Driver.sh
# Description: This script executes the SQL python script for OPM-HI PTD 
#
# Created: Paul Baranoski (cloned from OPMHI_PDE_Driver.sh
# Modified: 06/13/2023
#
...
# 12/27/2023   Joshua Turner        Added ENV to email notifications
...
# 2026-02-23   Viren Khanna         Modify to Add "TESTING" functionality.
############################################################################################################
set +x
...
CUR_DT=`date +%Y%m%d`
LOGNAME=/app/IDRC/XTR/CMS/logs/${TESTLOG}_OPMHI_PDE_Driver_${TMSTMP}.log
...
echo "################################### " >> ${LOGNAME}
echo "OPMHI_PDE_Driver.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
#################################################################################
echo "" >> ${LOGNAME}
echo "Remove any residual FEHB_CMS_PTD_* files on linux data directory" >> ${LOGNAME}
rm ${DATADIR}FEHB_CMS_PTD_*   2>> ${LOGNAME}
...
echo "" >> ${LOGNAME}
echo "OPM-HI PTD Extract is starting with the following parameters:" >> ${LOGNAME}
...
#################################################################################
# Execute Python code to produce PTD file
#################################################################################
echo "" >> ${LOGNAME}
echo "Start execution of OPMHI_PDE_Extract.py program"  >> ${LOGNAME}

${PYTHON_COMMAND} ${RUNDIR}OPMHI_PDE_Extract.py >> ${LOGNAME} 2>&1
...
if [[ $RET_STATUS != 0 ]]; then
        echo "" >> ${LOGNAME}
        echo "Python script OPMHI_PDE_Extract.py failed" >> ${LOGNAME}
		
        # Send Failure email	
        SUBJECT="OPMHI_PDE_Driver.sh  - Failed (${ENVNAME})"
        MSG="OPM-HI PTD extract has failed."
...
echo "" >> ${LOGNAME}
echo "Python script OPMHI_PDE_Extract.py completed successfully. " >> ${LOGNAME}
...
if [ $RET_STATUS != 0 ]; then
         ...
         # Send Failure email	
         SUBJECT="OPMHI_PDE_Driver.sh - Failed (${ENVNAME})"
         MSG="CombineS3Files.sh for OPM-HI PTB PTD has failed."
         ${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
SUBJECT="OPMHI_PDE_Driver.sh  - Completed (${ENVNAME}${TESTEMAIL})"
MSG="OPM-HI PTD has completed successfully.\n\nThe following file(s) were created:\n\n${S3Files}"
...
if [[ $RET_STATUS != 0 ]]; then
		...
		# Send Failure email	
		SUBJECT="OPMHI_PDE_Driver.sh - Failed (${ENVNAME})"
		MSG="Sending Success email in OPMHI_PDE_Driver.sh  has failed."
		${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#############################################################
echo "" >> ${LOGNAME}
echo "Create Manifest file for OPMHI PTD Extract.  " >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="Create Manifest file in OPMHI_PDE_Driver.sh - Failed (${ENVNAME})"
	MSG="Create Manifest file in OPMHI_PDE_Driver.sh has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#################################################################################
echo "" >> ${LOGNAME}
echo "OPMHI_PDE_Driver.sh completed successfully." >> ${LOGNAME}
