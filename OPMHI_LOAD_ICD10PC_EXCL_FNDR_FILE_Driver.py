#!/usr/bin/env python
########################################################################################################
# Name: OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE_Driver.py
# Description: This script uploads the OPM-HI CPT exclusion procedure code finder files to 
#              BIA_{ENV}.CMS_TARGET_XTR_{ENV}.OPMHI_ICD10PCS_EXCL
#
#
...
def getConfigFile(s3_client, S3BUCKET, s3ConfigFolder_n_filename):
    ...
    if  s3ConfigFile == None:
        ## Send Failure email
        SUBJECT = f"OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE_Driver.py - Failed ({ENVNAME})"
...
def main_processing_loop():
    ...
    try:    
        ...
        LOGNAME = f"{LOG_DIR}{TESTLOG}OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE_Driver_{TMSTMP}.log"
        ...
        rootLogger.info(f"OPMHI ICD10PC finder file Finder filer File bucket={FINDER_FILE_BUCKET_FLDR}")
        ...
        #################################################################
        # Section 1: OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE_Driver.py logic
        #################################################################    

        rootLogger.info("--- Starting OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE_Driver.py logic ---")
        PREFIX= "OPMHI_ICD10PC"
        ...
        if len(lstKeys) == 0:
            ...
            ## Send Failure email	
            SUBJECT=f"OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE_Driver.py script - Failed ({ENVNAME})"
        ...
        OPMHI_ICD10PCS_FF=  lstKeys[0].split("/") [-1] 
        rootLogger.info("Start execution of OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE.py program")
        ...
        #############################################################
        os.environ["TMSTMP"] = TMSTMP
        os.environ["OPMHI_ICD10PCS_FF"] = OPMHI_ICD10PCS_FF
        ...
        try:
            sp_info = subprocess.run(['python3', 'OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE.py'], capture_output=True, text=True, check=True)
            write_sp_info_2_log(sp_info) 


        except subprocess.CalledProcessError as e:
            rootLogger.error(f"Calling OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE.py failed with return code {e.returncode}")
            rootLogger.error("\n%s", e.output)
            
            ## Send Failure email	
            SUBJECT=f"OPMHI loading CPT EXCL finder file has failed ({ENVNAME})"
            MSG=f"OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE.py failed."
        ...
        rootLogger.info("")
        rootLogger.info("Python script OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE.py completed successfully.")
        ...
        # Send Success email	
        SUBJECT=f"OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE_Driver.py script - completed ({ENVNAME}{TESTEMAIL})"
        MSG=f"OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE_Driver.py script completed."
        ...
        rootLogger.info(f"Move finder file in S3 to archive folder: s3://{FINDER_FILE_BUCKET_FLDR}archive/{OPMHI_ICD10PCS_FF}")
        
        s3ExtSourceKey = FINDER_FILE_BUCKET_FLDR + OPMHI_ICD10PCS_FF
        s3ExtDestinationKey = FINDER_FILE_BUCKET_FLDR + "archive/" + OPMHI_ICD10PCS_FF
        ...
        rootLogger.info("Script OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE_Driver.py completed successfully.")
        rootLogger.info(f"\nEnded at {TMSTMP}" )


    except Exception as e:
        print (f"Exception occured in OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE_Driver.py\n {e}")

        rootLogger.error("Exception occured in OPMHI_LOAD_ICD10PC_EXCL_FNDR_FILE_Driver.py.")
        rootLogger.error("\n%s", str(e))
