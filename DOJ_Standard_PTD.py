#!/usr/bin/env python
########################################################################################################
# Name:  DOJ_PartD_Standard.py
#
# DESC:  This python script extracts PTD data.
#
# Created: Paul Baranoski
# Modified: 10/24/2024	Natalya Tinovsky	Changed column length for "DDPS 2025 Part D IRA Claims layout changes"
#						Added column CLM_PTD_MODEL_CD  
# 
########################################################################################################
# IMPORTS
########################################################################################################
...
try:
    ...
    ########################################################################################################
    snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_DOJ_STG/{OUTPUT_FILE}
                FROM (
...
SELECT DISTINCT
    ...
    FROM IDRC_{ENVNAME}.CMS_FCT_CLM_{ENVNAME}.CLM C
                ...
                WHERE C.CLM_TYPE_CD IN (1,2,4)
                  ...
                  -- Sovaldi      
                  AND CL.CLM_LINE_FROM_DT BETWEEN to_date('2022-02-06','YYYY-MM-DD') AND to_date('2022-12-31','YYYY-MM-DD')
