...
# Paul Baranoski 2026-07-14 Changed call to function s3MoveFile2NewFolder to s3MoveLargeFile2NewFolder because file is 8 GB.
########################################################################################################
import os
...
OPMHI_CAR_BUCKET = rf"{XTR_BUCKET}/{OPMHI_CAR_BUCKET_FLDR}"
...
def main_processing_loop():
    ...
    try:    
        ...
        LOGNAME = f"{LOG_DIR}{TESTLOG}OPMHI_CAR_Driver_{TMSTMP}.log"
        ...
        rootLogger.info(f"\nOPMHI_CAR_Driver.py started at {TMSTMP}")
        ...
        deleteFilesFromLinuxUsingPrefix(DATA_DIR, "FEHB_CMS_CAR")
        ...
        rootLogger.info(f"{CLM_TYPE_CD1=}")
        rootLogger.info(f"{CLM_TYPE_CD2=}")
        ...
        #############################################################
        rootLogger.info(f"Remove s3 file {OPMHI_CAR_BUCKET_FLDR}{EXT_FILENAME} if it already exists in s3.")
        s3_client.delete_object(Bucket=XTR_BUCKET, Key=f"{OPMHI_CAR_BUCKET_FLDR}{EXT_FILENAME}")
        rootLogger.info(f"Remove s3 file {OPMHI_CAR_BUCKET_FLDR}{EXT_FILENAME}.gpg if it already exists in s3.")
        s3_client.delete_object(Bucket=XTR_BUCKET, Key=f"{OPMHI_CAR_BUCKET_FLDR}{EXT_FILENAME}.gpg")
        ...
        os.environ["CLM_TYPE_CD1"] = CLM_TYPE_CD1
        os.environ["CLM_TYPE_CD2"] = CLM_TYPE_CD2
        ...
        #############################################################
        rootLogger.info("")
        rootLogger.info("Start execution of OPMHI_PTB_Extract.py program")

        try:
            sp_info = subprocess.run(['python3', 'OPMHI_PTB_Extract.py'], capture_output=True, text=True, check=True)
            write_sp_info_2_log(sp_info) 
            
        except subprocess.CalledProcessError as e:
            rootLogger.error(f"Calling OPMHI_PTB_Extract.py failed with return code {e.returncode}")
            rootLogger.error("\n%s", e.output)
            
            ## Send Failure email	
            SUBJECT=f"OPMHI_PTB_Extract- Failed ({ENVNAME})"
            MSG=f"Python script OPMHI_PTB_Extract failed."
        ...
        rootLogger.info("")
        rootLogger.info("Python script OPMHI_PTB_Extract.py completed successfully.")
        ...
        rootLogger.info(f"{OPMHI_CAR_BUCKET=} ")

        sConcatFilename = f"FEHB_CMS_{CTYP}_{START_DT}_{END_DT}_{CUR_DATE}.txt.gz"
        rootLogger.info(f"{sConcatFilename=}")

        try:
            sp_info = subprocess.run(['bash', 'CombineS3Files.sh', OPMHI_CAR_BUCKET, sConcatFilename ], capture_output=True, text=True, check=True)
            write_sp_info_2_log(sp_info) 
            ...
            ## Send Failure email	
            SUBJECT=f"Combining S3 files in OPMHI_CAR_Driver.py - Failed ({ENVNAME})"
            MSG=f"Combining S3 files in OPMHI_CAR_Driver.py has failed."
        ...
        # Send Success email	
        SUBJECT=f" OPMHI_CAR_Driver- completed ({ENVNAME}{TESTEMAIL})"
        MSG=f" OPMHI_CAR_Driver completed. \n\nThe following extract files were created:\n\n{S3Files}"
        ...
        #############################################################
        downloadFileFromS3(s3_client, XTR_BUCKET, f"{OPMHI_CAR_BUCKET_FLDR}{EXT_FILENAME}", f"{DATA_DIR}{EXT_FILENAME}" )
        ...
        #############################################################
        s3UploadFile(s3_client, f"{DATA_DIR}{EXT_FILENAME}.gpg", XTR_BUCKET, f"{OPMHI_CAR_BUCKET_FLDR}{EXT_FILENAME}.gpg")

        #############################################################
        # Move extract .gz file to archive folder so that it is not
        # included in the manifest file.
        #############################################################
        s3MoveLargeFile2NewFolder(s3_client, XTR_BUCKET, f"{OPMHI_CAR_BUCKET_FLDR}{EXT_FILENAME}", f"{OPMHI_CAR_BUCKET_FLDR}archive/{EXT_FILENAME}")
        ...
        MANIFEST_FILE_HLQ = "OPMHI_CAR"
        # Ex. CLAIMS_YYYYMM
        DEST_FLDR = f"CLAIMS_{TMSTMP[:6]}"

        try:
            sp_info = subprocess.run(['bash', 'CreateManifestSFTPFile.sh', OPMHI_CAR_BUCKET, CUR_DATE, OPMHI_BOX_RECIPIENT, MANIFEST_BUCKET_TO_USE, MANIFEST_FILE_HLQ, DEST_FLDR ], capture_output=True, text=True, check=True)
            write_sp_info_2_log(sp_info)
            ...
            SUBJECT=f"Create Manifest file in OPMHI_CAR_Driver.py - Failed ({ENVNAME})"
            MSG=f"Create Manifest file in OPMHI_CAR_Driver.py  has failed."
        ...
        rootLogger.info("Script OPMHI_CAR_Driver.py completed successfully.")
        rootLogger.info(f"\nEnded at {datetime.now().strftime('%Y%m%d.%H%M%S')}" )


    except Exception as e:
        print (f"Exception occured in OPMHI_CAR_Driver.py\n {e}")

        rootLogger.error("Exception occured in OPMHI_CAR_Driver.py.")
        rootLogger.error("\n%s", str(e))

        ## Send Failure email	
        SUBJECT = f"Exception occured in OPMHI_CAR_Driver.py - Failed ({ENVNAME}{TESTEMAIL})"
        MSG= f"Exception occured in OPMHI_CAR_Driver.py {e}.  Process failed"
