...
# Name:  PAC_HOSP_RC.py
#
# Desc: Script to Extract Hospice Revenue Center Data
#
# Created: Viren Khanna  12/02/2022  
#
########################################################################################################
import os
import sys
...
try:
   ...
   #**************************************
   #   Extract Hospice Revenue Center Data
   #**************************************   
   snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_PAC_STG/PAC_HOSP_RC_Y{FYQ}_{TMSTMP}.csv.gz
                                                FROM (
SELECT   
DISTINCT    
...
TO_CHAR(C.CLM_FROM_DT::DATE, 'YYYYMMDD') AS CLM_FROM_DT,   
