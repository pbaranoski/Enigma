...
def main_processing_loop():
    ...
    try:    
        ...
        LOGNAME = f"{LOG_DIR}{TESTLOG}OPMHI_ENRLMNT_Driver_{TMSTMP}.log"
        ...
        rootLogger.info(f"\nOPMHI_ENRLMNT_Driver.py started at {TMSTMP}")
        ...
        deleteFilesFromLinuxUsingPrefix(DATA_DIR, "FEHB_CMS_ENR")
        ...
        #############################################################
        rootLogger.info(f"Remove s3 file {OPMHI_ENRLMNT_BUCKET_FLDR}{EXT_FILENAME} if it already exists in s3.")
        s3_client.delete_object(Bucket=XTR_BUCKET, Key=f"{OPMHI_ENRLMNT_BUCKET_FLDR}{EXT_FILENAME}")
        rootLogger.info(f"Remove s3 file {OPMHI_ENRLMNT_BUCKET_FLDR}{EXT_FILENAME}.gpg if it already exists in s3.")
        s3_client.delete_object(Bucket=XTR_BUCKET, Key=f"{OPMHI_ENRLMNT_BUCKET_FLDR}{EXT_FILENAME}.gpg")
        ...
        #############################################################
        rootLogger.info("")
        rootLogger.info("Start execution of OPMHI_ENRLMNT_Extract.py program")

        try:
            sp_info = subprocess.run(['python3', 'OPMHI_ENRLMNT_Extract.py'], capture_output=True, text=True, check=True)
            write_sp_info_2_log(sp_info) 
            
        except subprocess.CalledProcessError as e:
            rootLogger.error(f"Calling OPMHI_ENRLMNT_Extract.py failed with return code {e.returncode}")
            rootLogger.error("\n%s", e.output)
            
            ## Send Failure email	
            SUBJECT=f"OPMHI_ENRLMNT_Extract- Failed ({ENVNAME})"
            MSG=f"Python script OPMHI_ENRLMNT_Extract failed."
        ...
        rootLogger.info("")
        rootLogger.info("Python script OPMHI_ENRLMNT_Extract.py completed successfully.")
        ...
        rootLogger.info(f"{OPMHI_ENRLMNT_BUCKET=} ")

        sConcatFilename = f"FEHB_CMS_{CTYP}_{START_DT}_{END_DT}_{CUR_DATE}.txt.gz"
        rootLogger.info(f"{sConcatFilename=}")

        try:
            sp_info = subprocess.run(['bash', 'CombineS3Files.sh', OPMHI_ENRLMNT_BUCKET, sConcatFilename ], capture_output=True, text=True, check=True)
            write_sp_info_2_log(sp_info) 
            ...
            ## Send Failure email	
            SUBJECT=f"Combining S3 files in OPMHI_ENRLMNT_Driver.py - Failed ({ENVNAME})"
            MSG=f"Combining S3 files in OPMHI_ENRLMNT_Driver.py has failed."
        ...
        # Send Success email	
        SUBJECT=f" OPMHI_ENRLMNT_Driver- completed ({ENVNAME}{TESTEMAIL})"
        MSG=f" OPMHI_ENRLMNT_Driver completed. \n\nThe following extract files were created:\n\n{S3Files}"
        ...
        #############################################################
        downloadFileFromS3(s3_client, XTR_BUCKET, f"{OPMHI_ENRLMNT_BUCKET_FLDR}{EXT_FILENAME}", f"{DATA_DIR}{EXT_FILENAME}" )

        #############################################################
        # GPG encrypt file
        #############################################################
        rootLogger.info("Get Secret Key. ")
        EncryptKey = GPGFunctions.get_secret(OPMHI_ENROLL_ENCRYPT_KEY_SECRET_NAME, REGION)
        ...
        #############################################################
        s3MoveFile2NewFolder(s3_client, XTR_BUCKET, f"{OPMHI_ENRLMNT_BUCKET_FLDR}{EXT_FILENAME}", f"{OPMHI_ENRLMNT_BUCKET_FLDR}archive/{EXT_FILENAME}")
        ...
        if os.environ["TESTING"] == "Y":
            ...
            MANIFEST_BUCKET_TO_USE = f"{XTR_BUCKET}/{MANIFEST_OPMHI_ENRLMNT_BUCKET_FLDR}"
        ...
        try:
            ...
            SUBJECT=f"Create Manifest file in OPMHI_ENRLMNT_Driver.py - Failed ({ENVNAME})"
            MSG=f"Create Manifest file in OPMHI_ENRLMNT_Driver.py  has failed."
        ...
        rootLogger.info("Script OPMHI_ENRLMNT_Driver.py completed successfully.")
        rootLogger.info(f"\nEnded at {TMSTMP}" )


    except Exception as e:
        print (f"Exception occured in OPMHI_ENRLMNT_Driver.py\n {e}")

        rootLogger.error("Exception occured in OPMHI_ENRLMNT_Driver.py.")
        rootLogger.error("\n%s", str(e))
