#!/usr/bin/bash
############################################################################################################
# Script Name: OPMHI_INP_Driver.sh
# Description: This script executes the Part A SQL python script for OPM-HI INP 
#
...
CUR_DT=`date +%Y%m%d`
LOGNAME=/app/IDRC/XTR/CMS/logs/${TESTLOG}_OPMHI_INP_Driver_${TMSTMP}.log
...
echo "################################### " >> ${LOGNAME}
echo "OPMHI_INP_Driver.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
S3BUCKET=${OPMHI_INP_BUCKET}

#################################################################################
# Clean linux data directory of any residual files
#################################################################################
echo "" >> ${LOGNAME}
echo "Remove any residual FEHB_CMS_INP_* files on linux data directory" >> ${LOGNAME}
rm ${DATADIR}FEHB_CMS_INP_*   2>> ${LOGNAME}
...
echo "" >> ${LOGNAME}
echo "OPM-HI INP Extract is starting with the following parameters:" >> ${LOGNAME}
...
#################################################################################
# Execute Python code to produce INP file
#################################################################################
echo "" >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
        ...
        # Send Failure email	
        SUBJECT="OPMHI_INP_Driver.sh - Failed (${ENVNAME})"
        MSG="OPM-HI INP extract has failed."
...
echo "" >> ${LOGNAME}
echo "Python script OPMHI_PTA_Extract.py completed successfully. " >> ${LOGNAME}
...
if [ $RET_STATUS != 0 ]; then
         ...
         # Send Failure email	
         SUBJECT="OPMHI_INP_Driver.sh - Failed (${ENVNAME})"
         MSG="CombineS3Files.sh for OPM-HI PTB INP has failed."
         ${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
SUBJECT="OPMHI_INP_Driver.sh  - Completed (${ENVNAME}${TESTEMAIL})"
MSG="OPM-HI PTA INP has completed successfully.\n\nThe following file(s) were created:\n\n${S3Files}"
...
if [[ $RET_STATUS != 0 ]]; then
		...
		# Send Failure email	
		SUBJECT="Sending Success email in OPMHI_INP_Driver.sh - Failed (${ENVNAME})"
		MSG="Sending Success email in OPMHI_INP_Driver.sh  has failed."
		${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#############################################################
echo "" >> ${LOGNAME}
echo "Create Manifest file for OPMHI INP Extract.  " >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="Create Manifest file in OPMHI_INP_Driver.sh - Failed (${ENVNAME})"
	MSG="Create Manifest file in OPMHI_INP_Driver.sh has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#################################################################################
echo "" >> ${LOGNAME}
echo "OPMHI_INP_Driver.sh completed successfully." >> ${LOGNAME}
