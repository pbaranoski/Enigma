...
LOGNAME=/app/IDRC/XTR/CMS/logs/STS_HOS_Facility_Rpt_${TMSTMP}.log
...
echo "################################### " >> ${LOGNAME}
echo "STS_HOS_Facility_Rpt.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
S3BUCKET=${STS_HOS_FACILITY_BUCKET} 
echo "In STS_HOS_Facility_Rpt.sh SS3BUCKET=${S3BUCKET}" >> ${LOGNAME}
...
if [ "${CUR_MM}" = "04" ];then
	...
	# Send Failure email	
	SUBJECT="STS HOS Facility Report - Failed (${ENVNAME})"
...
#############################################################
echo "" >> ${LOGNAME}
echo "Run STS_HOS_Facility_Rpt.py program" >> ${LOGNAME}

${PYTHON_COMMAND} ${RUNDIR}STS_HOS_Facility_Rpt.py  >> ${LOGNAME} 2>&1
...
if [[ $RET_STATUS != 0 ]]; then
	echo "" >> ${LOGNAME}
	echo "Python script STS_HOS_Facility_Rpt.py failed" >> ${LOGNAME}
	
	# Send Failure email	
	SUBJECT="STS HOS Facility Report - Failed (${ENVNAME})"
	MSG="Python script STS_HOS_Facility_Rpt.py failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
echo "" >> ${LOGNAME}
echo "Python script STS_HOS_Facility_Rpt.py completed successfully." >> ${LOGNAME}
...
#############################################################
# Send success email of STS HOS Facility Rpt files
#############################################################
echo "" >> ${LOGNAME}
echo "Send success email." >> ${LOGNAME}

# Send Success email	
SUBJECT="STS HOS Facility Report - completed ($ENVNAME)"
MSG="STS HOS Facility Report completed. \n\nThe following extract files were created:\n\n${S3Files}"
${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${STS_HOS_FACILITY_EMAIL_SUCCESS_RECIPIENT}" "${SUBJECT}" "${MSG}"  >> ${LOGNAME} 2>&1
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="Sending Success email in STS_HOS_Facility_Rpt.sh  - Failed (${ENVNAME})"
	MSG="Sending Success email in STS_HOS_Facility_Rpt.sh has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#############################################################
echo "" >> ${LOGNAME}
echo "Create Manifest file for STS HOS Facility Report.  " >> ${LOGNAME}

${RUNDIR}CreateManifestFile.sh ${S3BUCKET} ${TMSTMP} ${STS_HOS_FACILITY_BOX_RECIPIENT} 
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="Create Manifest file in STS_HOS_Facility_Rpt.sh - Failed (${ENVNAME})"
	MSG="Create Manifest file in STS_HOS_Facility_Rpt.sh  has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#############################################################
echo "" >> ${LOGNAME}
echo "STS_HOS_Facility_Rpt.sh completed successfully." >> ${LOGNAME}
