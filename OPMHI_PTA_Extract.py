#!/usr/bin/env python
########################################################################################################
# Name:  OPMHI_PTA_Extract.py
# DESC:  This python script executes the Part A SQL for OPM-HI
#
# Created: Joshua Turner
# Modified: 06/12/2023
#
# 10/02/2023   Paul Baranoski       Modified extract filename to use imported EXT_FILENAME.
# 01/14/2025   Sean Whitelock	    Modified PRVDR_LGL_NAME column length from 70 to 100.
########################################################################################################
# IMPORTS
########################################################################################################
import os
...
try:
    ...
    ########################################################################################################
    # Extract OPM-HI Part A data and write to S3
    ########################################################################################################
    snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_{STAGE_NAME}_STG/{EXT_FILENAME}
		...
		--***\/***
            ...
            INNER JOIN "BIA_{ENVNAME}"."CMS_TARGET_XTR_{ENVNAME}"."OPMHI_SSN" FNDR
                ON FNDR.SSN_NUM = BENE.BENE_SSN_NUM
            ...
            WHERE C.CLM_TYPE_CD IN ({CLM_TYPE_CD})
              ...
              AND (
                ...
                OR CL.CLM_LINE_DGNS_CD NOT IN
                    (SELECT OPMHI_ICD10DG_EXCL FROM "BIA_{ENVNAME}"."CMS_TARGET_XTR_{ENVNAME}"."OPMHI_ICD10DGS_EXCL")
                OR CP.CLM_PRCDR_CD NOT IN
                    (SELECT OPMHI_ICD10PC_EXCL FROM "BIA_{ENVNAME}"."CMS_TARGET_XTR_{ENVNAME}"."OPMHI_ICD10PCS_EXCL")
              )
