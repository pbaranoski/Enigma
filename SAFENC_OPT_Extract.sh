#!/usr/bin/bash
######################################################################################
# Name:  SAFENC_OPT_Extract.sh
#
# Desc: SAF Encounter OPT Extract script. 
#
#  Execute as ./SAFENC_OPT_Extract.sh  
#
...
#
# Created: Paul Baranoski  06/22/2023
#
# Modified:
#
# Paul Baranoski 2023-06-22 Created script. 
# Paul Baranoski 2023-07-06 Force GitHub to pick up script.
...
LOGNAME=/app/IDRC/XTR/CMS/logs/SAFENC_OPT_Extract_${TMSTMP}.log
...
echo "################################### " >> ${LOGNAME}
echo "SAFENC_OPT_Extract.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
SAF_ENC_PARM_FILE=SAF_ENC_OPT_PARM_FILE.txt
S3BUCKET=${SAFENC_OPT_BUCKET} 
CLM_TYPE_LIT=OPT
CLM_TYPE_CDS="4012,4013,4014,4022,4023,4034,4071,4072,4073,4074,4075,4076,4077,4079,4083,4085,4089"


echo "" >> ${LOGNAME}
echo "SAF ENC OPT bucket=${S3BUCKET}" >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="SAF Encounter OPT Extract - Failed (${ENVNAME})"
...
#################################################################################
# Find SAF Extract Years Parameter file in data directory.
#################################################################################
ParmFile2Process=`ls ${DATADIR}${SAF_ENC_PARM_FILE}` 1>> ${LOGNAME}  2>&1

echo "" >> ${LOGNAME}
echo "SAF Encounter OPT Extract Parameter file on linux: ${ParmFile2Process}" >> ${LOGNAME}


#################################################################################
# Loop thru SAF Encounter OPT Date Ranges in parameter file.
# 
...
do
		...
		#############################################################
		echo "" >> ${LOGNAME}
		echo "Start execution of SAFENC_OPT_Extract.py program" >> ${LOGNAME}

		${PYTHON_COMMAND} ${RUNDIR}SAFENC_OPT_Extract.py  >> ${LOGNAME} 2>&1
		...
		if [[ $RET_STATUS != 0 ]]; then
				echo "" >> ${LOGNAME}
				echo "Python script SAFENC_OPT_Extract.py failed" >> ${LOGNAME}
				
				# Send Failure email	
				SUBJECT="SAF Encounter OPT Extract - Failed (${ENVNAME})"
				MSG="SAF Encounter OPT Extract has failed."
				${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SAFENC_EMAIL_SENDER}" "${SAFENC_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
		...
		echo "" >> ${LOGNAME}
		echo "Python script SAFENC_OPT_Extract.py completed successfully." >> ${LOGNAME}
		...
		if [[ $RET_STATUS != 0 ]]; then
				...
				# Send Failure email	
				SUBJECT="Combining S3 files in SAFENC_OPT_Extract.sh - Failed (${ENVNAME})"
				MSG="Combining S3 files in SAFENC_OPT_Extract.sh has failed."
				${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SAFENC_EMAIL_SENDER}" "${SAFENC_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
SUBJECT="SAF Encounter OPT extract (${ENVNAME})" 
MSG="The SAF Encounter OPT extract has completed.\n\nThe following file(s) were created:\n\n${S3Files}"
...
if [[ $RET_STATUS != 0 ]]; then
		...
		# Send Failure email	
		SUBJECT="Sending Success email in SAFENC_OPT_Extract.sh - Failed (${ENVNAME})"
		MSG="Sending Success email in SAFENC_OPT_Extract.sh has failed."
		${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SAFENC_EMAIL_SENDER}" "${SAFENC_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#############################################################
echo " " >> ${LOGNAME}
echo "EFT SAF Encounter Outpatient Extract Files " >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="SAF Encounter OPT EFT process - Failed (${ENVNAME})"
	MSG="SAF Encounter OPT EFT process has failed."
...
#############################################################
echo "" >> ${LOGNAME}
echo "SAFENC_OPT_Extract.sh completed successfully." >> ${LOGNAME}
