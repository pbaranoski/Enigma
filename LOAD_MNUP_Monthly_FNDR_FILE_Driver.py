#!/usr/bin/sh
############################################################################################################
# Script Name: LOAD_MNUP_Monthly_FNDR_FILE_Driver
...
# Viren Khanna 2026-07-02 Convert bash script to python.
############################################################################################################
...
def main_processing_loop():
    ...
    try:    
        ...
        rootLogger.info(f"\nLOAD_MNUP_Monthly_FNDR_FILE_Driver started at {TMSTMP}")
        ...
        if NOF_FILES == 0:
            ...
            # Send Failure email	
            SUBJECT = f"LOAD_MNUP_Monthly_FNDR_FILE_Driver script - Failed ({ENVNAME})"
        ...
        LOAD_MNUP_Monthly_FINDER_FILE = lstFinderFilenames[0]

        rootLogger.info("")
        rootLogger.info(f"MNUP Finder file found: {LOAD_MNUP_Monthly_FINDER_FILE}")

        # Export environment variables for Python code
        os.environ["LOAD_MNUP_Monthly_FINDER_FILE"] = LOAD_MNUP_Monthly_FINDER_FILE
        ...
        #############################################################
        rootLogger.info("")
        rootLogger.info("Start execution of LOAD_MNUP_Monthly_FNDR_FILE.py program")

        try:
            sp_info = subprocess.run(['python3', 'LOAD_MNUP_Monthly_FNDR_FILE.py'], capture_output=True, text=True, check=True)
            write_sp_info_2_log(sp_info) 
            ...
            ## Send Failure email	
            SUBJECT=f"LOAD_MNUP_Monthly_FNDR_FILE_Driver - Failed ({ENVNAME})"
            MSG=f"Python script LOAD_MNUP_Monthly_FNDR_FILE_Driver failed."
        ...
        rootLogger.info("")
        rootLogger.info("Python script LOAD_MNUP_Monthly_FNDR_FILE_Driver completed successfully.")
        ...
        #################################################################################
        rootLogger.info("")
        rootLogger.info(f"Move processed finder file {LOAD_MNUP_Monthly_FINDER_FILE} to S3 Finder File SSA archive folder.")

        archiveFinderFile(s3_client, XTR_BUCKET, FINDER_FILE_SSA_BUCKET_FLDR, LOAD_MNUP_Monthly_FINDER_FILE)
        ...
        rootLogger.info("Python program LOAD_MNUP_Monthly_FNDR_FILE_Driver completed successfully.")
        rootLogger.info(f"\nEnded at {datetime.now().strftime('%Y%m%d.%H%M%S')}" )


    except Exception as e:
        print (f"Exception occured in LOAD_MNUP_Monthly_FNDR_FILE_Driver\n {e}")

        rootLogger.error("Exception occured in LOAD_MNUP_Monthly_FNDR_FILE_Driver.")
        rootLogger.error("\n%s", str(e))

        ## Send Failure email	
        SUBJECT = f"Exception occured in LOAD_MNUP_Monthly_FNDR_FILE_Driver - Failed ({ENVNAME}{TESTEMAIL})"
        MSG= f"Exception occured in LOAD_MNUP_Monthly_FNDR_FILE_Driver {e}.  Process failed"
