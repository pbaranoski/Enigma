...
#
######################################################################################
# Name:  STS_SNF_Rpts.sh
#        
# DESC:   This script extracts data for STS SNF table report - SNF Rev Cntr UNITS/CHRGS	  
#         by period expense (legacy AA8 report)
#
# Created: Viren Khanna  08/27/2024
# Modified: 
#
# Viren Khanna 2024-08-27 Create scripts.
######################################################################################
set +x
...
LOGNAME=/app/IDRC/XTR/CMS/logs/STS_SNF_RPTS_${TMSTMP}.log
...
echo "################################### " >> ${LOGNAME}
echo "STS_SNF_RPTs.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
if [ "${CUR_MM}" = "04" ];then
	...
	# Send Failure email	
	SUBJECT="STS SNF Report - Failed (${ENVNAME})"
...
#############################################################
echo "" >> ${LOGNAME}
echo "Run STS_SNF_RPT.py program" >> ${LOGNAME}

${PYTHON_COMMAND} ${RUNDIR}STS_SNF_RPT.py  >> ${LOGNAME} 2>&1
...
if [[ $RET_STATUS != 0 ]]; then
	echo "" >> ${LOGNAME}
	echo "Python script STS_SNF_RPT.py failed" >> ${LOGNAME}
	
	# Send Failure email	
	SUBJECT="STS HHA RevCtr Report - Failed (${ENVNAME})"
	MSG="Python script STS_SNF_RPT.py failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
echo "" >> ${LOGNAME}
echo "Python script STS_SNF_RPT.py completed successfully." >> ${LOGNAME}
...
#############################################################
# Send success email of STS SNF Tbl Rpt files
#############################################################
echo "" >> ${LOGNAME}
echo "Send success email." >> ${LOGNAME}

# Send Success email	
SUBJECT="STS SNF Report - completed ($ENVNAME)"
MSG="STS SNF Report completed. \n\nThe following extract files were created:\n\n${S3Files}"
${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${STS_SNF_EMAIL_SUCCESS_RECIPIENT}" "${SUBJECT}" "${MSG}"  >> ${LOGNAME} 2>&1
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="Sending Success email in STS_SNF_RPTs.sh  - Failed (${ENVNAME})"
	MSG="Sending Success email in STS_SNF_RPTs.sh has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#############################################################
echo "" >> ${LOGNAME}
echo "Create Manifest file for STS SNF Report.  " >> ${LOGNAME}

${RUNDIR}CreateManifestFile.sh ${S3BUCKET} ${TMSTMP} ${STS_SNF_BOX_RECIPIENT} 
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="Create Manifest file in STS_SNF_RPT.sh - Failed (${ENVNAME})"
	MSG="Create Manifest file in STS_SNF_RPT.sh  has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#############################################################
echo "" >> ${LOGNAME}
echo "STS_SNF_RPTs.sh completed successfully." >> ${LOGNAME}
