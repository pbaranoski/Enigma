#!/usr/bin/env python
########################################################################################################
# Name:  STS_HHA_A4A_Driver.py
#
# DESC:   This script extracts data for STS HHA by facility type  and state
#         (legacy A4A report). Is run biannually in Jan and July  
#
# Execute as python3 STS_HHA_A4A_Driver.py     (processing without override date) 
# Execute as python3 STS_HHA_A4A_Driver.py $1  (processing with override date --> YYYY-MM-DD format) 
#
# 			$1 = From_dt (YYYY-MM-DD)
#
#   
# Paul Baranoski   2025-10-17 Create Module.
# Paul Baranoski   2026-01-09 Modified 'SNF' to HHA A4A' verbiage in Subject/Message text in email.  
########################################################################################################
...
S3BUCKET = rf"{XTR_BUCKET}/{STS_SNF_BUCKET_FLDR}"
...
def validate_dt(sDate2Validate):
    ...
    try:
        ...
        ## Send Failure email	
        SUBJECT=f"STS_HHA_A4A_Driver.py - Failed ({ENVNAME})"
...
def main_processing_loop():
    ...
    try:    
        ...
        LOGNAME = f"{LOG_DIR}STS_HHA_A4A_{TMSTMP}.log"
        ...
        rootLogger.info(f"\nSTS_HHA_A4A_Driver.py started at {TMSTMP}")
        ...
        if CUR_MM == "07":
            ...
            # Send Failure email	
            SUBJECT=f"STS HHA A4A Report - Failed ({ENVNAME})"
        ...
        #############################################################
        rootLogger.info("")
        rootLogger.info("Start execution of STS_HHA_A4A.py program")

        try:
            sp_info = subprocess.run(['python3', 'STS_HHA_A4A.py'], capture_output=True, text=True, check=True)
            write_sp_info_2_log(sp_info) 
            
        except subprocess.CalledProcessError as e:
            rootLogger.error(f"Calling STS_HHA_A4A.py failed with return code {e.returncode}")
            rootLogger.error("\n%s", e.output)
            
            ## Send Failure email	
            SUBJECT=f"STS HHA A4A Report - Failed ({ENVNAME})"
            MSG=f"Python script STS_HHA_A4A.py failed."
        ...
        rootLogger.info("")
        rootLogger.info("Python script STS_HHA_A4A_Driver.py completed successfully.")
        ...
        # Send Success email	
        SUBJECT=f"STS HHA A4A Report - completed ({ENVNAME})"
        MSG=f"STS HHA A4A Report completed. \n\nThe following extract files were created:\n\n{S3Files}"
        
        try:
            sp_info = subprocess.run(['python3', 'sendEmail.py', CMS_EMAIL_SENDER, STS_HHA_EMAIL_SUCCESS_RECIPIENT, SUBJECT, MSG], capture_output=True, text=True, check=True)
            write_sp_info_2_log(sp_info)
        ...
        #############################################################
        rootLogger.info("")
        rootLogger.info("Create Manifest file for STS HHA A4A Report. ")
        ...
        #####################################################
        try:
            CreManDr.createManifestFile(bucket=XTR_BUCKET, s3folder=STS_HHA_BUCKET_FLDR, runToken=TMSTMP, BoxEmails=STS_HHA_BOX_RECIPIENT )

        except Exception as e:

            SUBJECT=f"Create Manifest file in STS_HHA_A4A_Driver.py - Failed ({ENVNAME})"
            MSG=f"Create Manifest file in STS_HHA_A4A_Driver.py  has failed."
        ...
        rootLogger.info("Script STS_HHA_A4A_Driver.py completed successfully.")
        rootLogger.info(f"\nEnded at {TMSTMP}" )


    except Exception as e:
        print (f"Exception occured in STS_HHA_A4A_Driver.py\n {e}")

        rootLogger.error("Exception occured in STS_HHA_A4A_Driver.py.")
        rootLogger.error("\n%s", str(e))
