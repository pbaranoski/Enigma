...
# Name:  PAC_LTCH_PC.py
#
# Desc: Script to Extract Long Term Care Hospital(LTCH) Procedure Code Data
#
...
try:
   ...
   #**************************************
   #   Extract Long Term Care Hospital(LTCH) Procedure Code Data
   #**************************************   
   snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_PAC_STG/PAC_LTCH_PC_Y{FYQ}_{TMSTMP}.csv.gz
                                                FROM (
SELECT 
DISTINCT 
...
RPAD(' ',6) AS SQNC_NUM,
RPAD(CP.CLM_PRCDR_CD,7), 
TO_CHAR(CP.CLM_PRCDR_PRFRM_DT::DATE, 'YYYYMMDD') AS CLM_PRCDR_PRFRM_DT, 
...
ORDER BY CLAIM_YEAR_MONTH,CLAIM_UID,CLM_PRCDR_PRFRM_DT
