...
LOGNAME=/app/IDRC/XTR/CMS/logs/PSA_Physicians_ext_${TMSTMP}.log
RUNDIR=/app/IDRC/XTR/CMS/scripts/run/
DATADIR=/app/IDRC/XTR/CMS/data/

PSA_FF_PHYS_PREFIX=PSA_FINDER_FILE_PRVDR_SPCLTY_CDS

touch ${LOGNAME}
chmod 666 ${LOGNAME} 2>> ${LOGNAME} 

echo "################################### " >> ${LOGNAME}
echo "PSA_Physicians.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
rm ${DATADIR}tempPSAPhys.txt  >> ${LOGNAME} 2>&1
rm ${DATADIR}${PSA_FF_PHYS_PREFIX}*  >> ${LOGNAME} 2>&1
...
echo "List ${PSA_FF_PHYS_PREFIX}_* finder files in S3 " >> ${LOGNAME}

S3_FINDER_FILES=`aws s3 ls s3://${FINDER_FILE_BUCKET}${PSA_FF_PHYS_PREFIX}`  >> ${LOGNAME} 
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="PSA_Physicians.sh - Failed (${ENVNAME})"
	MSG="Listing S3 finder files from ${FINDER_FILE_BUCKET}${PSA_FF_PHYS_PREFIX} failed."
...
if [[ $RET_STATUS != 0 ]]; then
	echo "" >> ${LOGNAME}
	echo "Shell script PSA_Physicians.sh failed." >> ${LOGNAME}
...
# Ex. PIV_COL_DESC_LIST = 'Addiction Medicine','Anesthesiology','Cardiac Surgery'
#     PIV_COL_NAME_LIST = "Addiction Medicine","Anesthesiology","Cardiac Surgery"
#################################################################################
echo "" >> ${LOGNAME}
echo "Build Pivot table column list" >> ${LOGNAME}

COL2MOVE="'PRIMARY_CARE'"

PIV_COL_DESC_LIST=""
PIV_COL_NAME_LIST=""

# 1) skip header; 2) finder file has 2 fields
tail -n +2 ${DATADIR}${FINDER_FILE} | cut -d, -f2- | sort -u > ${DATADIR}tempPSAPhys.txt  
...
#############################################################
echo "" >> ${LOGNAME}
echo "Start execution of PSA_Physicians_ext.py program"  >> ${LOGNAME}
...
${PYTHON_COMMAND} ${RUNDIR}PSA_Physicians.py >> ${LOGNAME} 2>&1
...
if [[ $RET_STATUS != 0 ]]; then
	echo "" >> ${LOGNAME}
	echo "Python script PSA_Physicians.py failed" >> ${LOGNAME}
	
	# Send Failure email	
	SUBJECT="PSA_Physicians.sh  - Failed ${ENVNAME}"
	MSG="PSA Physicians Extract has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${PSA_EMAIL_SENDER}" "${PSA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
echo "" >> ${LOGNAME}
echo "Python script PSA_Physicians.py completed successfully. " >> ${LOGNAME}
...
SUBJECT="PSA Physicians extract (${ENVNAME})" 
MSG="The Extract for the creation of the PSA Physicians extract file from Snowflake has completed.\n\nThe following file(s) were created:\n\n${S3Files}"
...
if [[ $RET_STATUS != 0 ]]; then
		...
		# Send Failure email	
		SUBJECT="Sending Success email in PSA_Physicians.sh  - Failed ${ENVNAME}"
		MSG="Sending Success email in PSA_Physicians.sh  has failed."
		${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${PSA_EMAIL_SENDER}" "${PSA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#################################################################################
echo "" >> ${LOGNAME}
echo "Move processed S3 PSA Physicians Finder File to archive directory in S3." >> ${LOGNAME}
...
#############################################################
echo "" >> ${LOGNAME} 
echo "Remove tempPSAPhys.txt from data directory" >> ${LOGNAME} 
...
#############################################################
echo "" >> ${LOGNAME}
echo "PSA_Physicians.sh completed successfully." >> ${LOGNAME}
