#!/usr/bin/env python
########################################################################################################
# Name:  SAFENC_CAR_Extract.py
# DESC:  This python script extracts data from IDRC for the SAF ENC CAR extracts
##
...
# Viren Khanna   2024-06-07 Updated code to add 2 fields CLM_ENCTR_ADJSTMT_GRP_CD,CLM_ADJSTMT_GRP_CD
...
#**************************************
   ...
   #**************************************  
   # P#IDR.XTR.ENC.SAF.EARLY.CAR.Y2022.DECWK1
   # SAF_ENC_EARLY_CAR_Y2022_DECWK1.txt.gz  
        ########################################################################################################
        # Extract SAF ENC CAR data and write to S3 as a flat file
        ########################################################################################################
   snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_SAFENCCAR_STG/SAFENC_CAR_{STUS_LIT}_{RNG_LIT}_{TMSTMP}.txt.gz
                FROM (
   WITH TMP_CAR_ADR AS (
...
SELECT DISTINCT
		...
		,REPLACE(TO_CHAR(COALESCE(CASE WHEN CEOPD.CLM_OTHR_PYR_RSPNSBLTY_SQNC_CD = 'P' THEN CEOPD.CLM_ENCTR_OTHR_PYR_PD_AMT ELSE 0 END,0),'FM000000000000.00S'),'.','') 
		...
		,RPAD(CASE WHEN CED.CLM_RMTNC_ADVC_RMRK_1_CD = '~' THEN ' '  ELSE COALESCE(CED.CLM_RMTNC_ADVC_RMRK_1_CD,' ') END,15,' ') AS CLM_RMTNC_ADVC_RMRK_1_CD
	        ,RPAD(CASE WHEN C.CLM_RFRG_PRVDR_PIN_NUM = '~'      THEN ' ' ELSE COALESCE(C.CLM_RFRG_PRVDR_PIN_NUM,' ')     END,14,' ')  AS  CLM_RFRG_PRVDR_PIN_NUM
		,REPLACE(TO_CHAR(COALESCE(C.CLM_BLOOD_PT_FRNSH_QTY,0),'FM000'),' ') AS CLM_BLOOD_PT_FRNSH_QTY
                ,REPLACE(TO_CHAR(COALESCE(CDN.CLM_BLOOD_DDCTBL_PT_QTY,0),'FM0000'),' ') AS CLM_BLOOD_DDCTBL_PT_QTY
		...
		--** RNDRG_PRVDR	 
                ,RPAD(CASE WHEN CL.CLM_RNDRG_PRVDR_PIN_NUM = '~' THEN ' ' ELSE COALESCE(CL.CLM_RNDRG_PRVDR_PIN_NUM,' ') END,10,' ')  AS CLM_RNDRG_PRVDR_PIN_NUM
		,RPAD(CASE WHEN CL.CLM_RNDRG_PRVDR_UPIN_NUM = '~' THEN ' ' ELSE COALESCE(CL.CLM_RNDRG_PRVDR_UPIN_NUM,' ') END,10,' ')  AS CLM_RNDRG_PRVDR_UPIN_NUM
		...
		,RPAD(CASE WHEN CL.CLM_RNDRG_PRVDR_GRP_NPI_NUM = '~' THEN ' ' ELSE COALESCE(CL.CLM_RNDRG_PRVDR_GRP_NPI_NUM,' ') END,10,' ')  AS CLM_RNDRG_PRVDR_GRP_NPI_NUM
                ,RPAD(CASE WHEN CL.CLM_RNDRG_PRVDR_TYPE_CD = '~' THEN ' ' ELSE COALESCE(CL.CLM_RNDRG_PRVDR_TYPE_CD,' ') END,2,' ')  AS CLM_RNDRG_PRVDR_TYPE_CD
		...
		,RPAD(CASE WHEN CL.CLM_POS_CD = '~' THEN ' ' ELSE COALESCE(CL.CLM_POS_CD,' ') END,2,' ')  AS CLM_POS_CD
		...
		,REPLACE(TO_CHAR(COALESCE(CL.CLM_LINE_PTB_BLOOD_DDCTBL_QTY,0),'FM000'),' ') AS CLM_LINE_PTB_BLOOD_DDCTBL_QTY
		
                ,REPLACE(TO_CHAR(COALESCE(CL.CLM_LINE_SBMT_CHRG_AMT,0),'000000000000.00S'),'.','') AS CLM_LINE_SBMT_CHRG_AMT 
		,RPAD(CASE WHEN CLEDD.CLM_LINE_DGNS_CD = '~'   THEN ' ' ELSE COALESCE(CLEDD.CLM_LINE_DGNS_CD,' ')	END,7,' ') AS CLM_LINE_DGNS_CD
                ,REPLACE(TO_CHAR(COALESCE( CL.CLM_LINE_ANSTHSA_UNIT_CNT,0),'FM0000.000'),'.','')  AS CLM_LINE_ANSTHSA_UNIT_CNT			   
                -- WARNING !!  (FORMAT '9(13)V9(2)S') (CHAR(15)) TRUNCATION OF SIGN 
	        ,REPLACE(TO_CHAR(CASE WHEN HCPCS.HCPCS_CD = ' '  THEN COALESCE(CLEOPD.CLM_LINE_OTHR_PYR_PD_AMT,0)  ELSE 0 END,'000000000000.00S'),'.','') AS CLM_LINE_OTHR_PYR_PD_AMT
	        ,RPAD(CASE WHEN CL.CLM_LINE_RX_NUM ='~' THEN ' ' ELSE COALESCE(CL.CLM_LINE_RX_NUM,' ')  END,30,' ') AS CLM_LINE_RX_NUM
		...
		-- ADDED CNTRCT_PBP_PTC_NUM FROM BENE_FACT TABLE ON 03/12/15
		-- UPDATED THE LOGIC FOR 2014 EXTRACT

		,RPAD(CASE WHEN C.CLM_SBMTR_CNTRCT_NUM = BF.CNTRCT_PTC_NUM THEN BF.CNTRCT_PBP_PTC_NUM ELSE ' ' END,3,' ')  AS CNTRCT_PBP_PTC_NUM
		-- ADDED COLUMNS CLM_CNTRCT_TYPE_CD, CLM_LINE_CNTRCT_TYPE_CD
		-- AND CLM_CHRT_RVW_SW ON 03/2016
		...
		-- FOLLOWING COLUMNS ADDED AS PART OF 2014 RERUN 08/16
		...
		--  FOLLOWING COLUMN  ADDED AS PART OF 2015 RERUN 05/18
		 ...
		 -- FOLLOWING COLUMNS ADDED AS PART OF 2016 RUN 07/19
...
/*
LEFT OUTER JOIN CMS_VIEW_BENE_MTRLZD_PRD.V1_BENE_FCT BF
 ON C.BENE_SK = BF.BENE_SK
AND BF.BENE_FCT_EFCTV_CD = '1'
...
-- AND ZIP5.GEO_FIPS_CNTY_CD = ZIP9.GEO_FIPS_CNTY_CD
 AND ZIP9.GEO_ZIP4_CD = BENE.GEO_ZIP4_CD
...
LEFT OUTER JOIN BIA_{ENVNAME}.CMS_TARGET_XTR_{ENVNAME}.SAFENC_DMEPOS_HCPCS HCPCS
 ON HCPCS.HCPCS_CD = CL.CLM_LINE_HCPCS_CD

--CMS_LOAD_COMM_PRD.MK_CAR_ADR_TEMP  ADDED AS PART OF 2014 RERUN 09/16
LEFT OUTER JOIN TMP_CAR_ADR ADRTEMP 
...
WHERE C.CLM_TYPE_CD = '4700'  /* CARRIER */
--AND C.CLM_FINL_ACTN_IND = 'Y'
AND CE.CLM_EDPS_STUS_CD = 'A'
AND CLE.CLM_LINE_ENCTR_STUS_CD <> '84'
