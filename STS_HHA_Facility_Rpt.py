...
try:
   ...
   snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_STS_HHA_FACILITY_STG/STS_HHA_FACILITY_RPT_AA4_{EXT_TO_YYYY}_{RUN_PRD}_{TMSTMP}.csv.gz
                    FROM (

                      WITH RPT_CLM_TYPE_CATEGORIES AS (
    
    SELECT '1' AS RPT_CLM_TYPE_CD, 'REHAB FACILITY BASED'      AS RPT_CLM_TYPE_DESC, '1' AS SORT_ORD_IND FROM DUAL
    UNION
    SELECT '2' AS RPT_CLM_TYPE_CD, 'HOSPITAL BASED'  AS RPT_CLM_TYPE_DESC, '2' AS SORT_ORD_IND FROM DUAL
    UNION
    SELECT '3' AS RPT_CLM_TYPE_CD, 'SNF BASED'      AS RPT_CLM_TYPE_DESC, '3' AS SORT_ORD_IND FROM DUAL
...
,PARTA_DTL_CLMS  AS (
            ...
            SELECT C.GEO_BENE_SK
                ...
                ,CASE WHEN C.CLM_QUERY_CD = '0' 
                  ...
                  , CASE WHEN FF.HHA_TYPE_CD = '4' THEN '1'
                  WHEN FF.HHA_TYPE_CD = '5' THEN '2'
                  WHEN FF.HHA_TYPE_CD = '6' THEN '3' 
                ELSE '4' END AS RPT_CLM_TYPE_CD 
...
SELECT CAL_YEAR
     ...
     ,SUM(CASE WHEN RPT_CLM_TYPE_CD = '1' THEN CHRG_AMT  ELSE 0 END) AS REHAB_CHRG_AMT
     ,SUM(CASE WHEN RPT_CLM_TYPE_CD = '1' THEN PMT_AMT   ELSE 0 END) AS REHAB_PMT_AMT
     ,SUM(CASE WHEN RPT_CLM_TYPE_CD = '2' THEN CHRG_AMT  ELSE 0 END) AS HOSPITAL_CHRG_AMT
     ,SUM(CASE WHEN RPT_CLM_TYPE_CD = '2' THEN PMT_AMT   ELSE 0 END) AS HOSPITAL_PMT_AMT
    
     ,SUM(CASE WHEN RPT_CLM_TYPE_CD = '3' THEN CHRG_AMT  ELSE 0 END) AS SNF_CHRG_AMT
     ,SUM(CASE WHEN RPT_CLM_TYPE_CD = '3' THEN PMT_AMT   ELSE 0 END) AS SNF_PMT_AMT
