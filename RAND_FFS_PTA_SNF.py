...
def main_processing_loop():
    ...
    try:    
        ...
        LOGNAME = f"{LOG_DIR}{TESTLOG}RAND_FFS_PTA_SNF_{TMSTMP}.log"
        ...
        rootLogger.info(f"\nRAND_FFS_PTA_SNF.py started at {TMSTMP}")
        ...
        #############################################################
        rootLogger.info("")
        rootLogger.info("RAND FFS PTA SNF Processing will use the following parameters:")
        ...
        for i in range(len(start_date_parms)):
            ...
            XTR_FILE_NAME = f"RAND_FFS_SNF_Y{EXT_YYYY}_P{RNG}_{TMSTMP}.csv.gz"
            ...
            try:
                ...
                ## Send Failure email	
                SUBJECT=f"RAND_FFS_PTA_SNF.py extract - Failed ({ENVNAME}{TESTEMAIL})"
            ...
            try:
                ...
                ## Send Failure email	
                SUBJECT=f"Combining S3 files in RAND_FFS_PTA_SNF.py - Failed ({ENVNAME}{TESTEMAIL})"
                MSG=f"Combining S3 files in RAND_FFS_PTA_SNF.py has failed."
        ...
        SUBJECT=f"RAND FFS PTA SNF Extract Complete ({ENVNAME}{TESTEMAIL})" 
        MSG=f"RAND FFS PTA SNF Extract completed successfully.\n\nThe following file(s) were created:\n\n{S3Files}"
        ...
        ######################################################################
        rootLogger.info("")
        rootLogger.info(f"Creating manifest files for RAND FFS PTA SNF Extract. ")
        ...
        try:
            ...
            SUBJECT=f"Create Manifest file in RAND_FFS_PTA_SNF.py - Failed ({ENVNAME}{TESTEMAIL})"
            MSG=f"Create Manifest file in RAND_FFS_PTA_SNF.py  has failed. {e}"
        ...
        rootLogger.info("RAND FFS PTA SNF Extract completed successfully.")
        rootLogger.info(f"\nEnded at {TMSTMP}" )


    except Exception as e:
        print (f"Exception occured in RAND_FFS_PTA_SNF.py\n {e}")

        rootLogger.error("Exception occured in RAND_FFS_PTA_SNF.py.")
        rootLogger.error("\n%s", str(e))

        ## Send Failure email	
        SUBJECT=f"RAND_FFS_PTA_SNF.py  - Failed ({ENVNAME}{TESTEMAIL})"
        MSG=f"Exception occured in RAND_FFS_PTA_SNF.py {e}. Process failed. "
