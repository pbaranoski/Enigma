...
LOGNAME=/app/IDRC/XTR/CMS/logs/RAND_FFS_PTA_SNF_${TMSTMP}.log
touch ${LOGNAME}
...
do
	...
	if [[ $RET_STATUS != 0 ]]; then
		...
		#Send Failure email	
		SUBJECT="RAND FFS Part A SNF Extract - Failed (${ENVNAME})"
...
SUBJECT="RAND FFS Part A SNF Extract - COMPLETE (${ENVNAME})"
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="RAND_FFS_PTA_SNF.sh failed - Error in calling sendEmail.py (${ENVNAME})"
...
if [[ $RET_STATUS != 0 ]]; then
		...
		# Send Failure email	
		SUBJECT="Create Manifest file in RAND_FFS_PTA_SNF.sh - Failed (${ENVNAME})"
