#!/usr/bin/sh
############################################################################################################
# Name: RAND_FFS_PTB_Driver.sh
#
# Desc: RAND FFS Part B Driver. This script will execute all RAND FFS Part B extracts (CAR, DME)
...
#
# Modified:
# Joshua Turner 	2023-03-08 	New script.
...
LOGNAME=/app/IDRC/XTR/CMS/logs/RAND_FFS_PTB_Driver_${TMSTMP}.log
touch ${LOGNAME}
...
echo "################################################" >> ${LOGNAME}
echo "RAND_FFS_PTB_Driver.sh started at ${TMSTMP}" >> {LOGNAME}
echo "################################################" >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
###########################################################################################
echo "========================================================" >> ${LOGNAME}
echo "Starting DME extract script - RAND_FFS_PTB_DME.sh " >> ${LOGNAME}
bash ${RUNDIR}RAND_FFS_PTB_DME.sh >> ${LOGNAME} 2>&1
...
###########################################################################################
echo "========================================================" >> ${LOGNAME}
echo "Starting CAR extract script - RAND_FFS_PTB_CAR.sh " >> ${LOGNAME}
bash ${RUNDIR}RAND_FFS_PTB_CAR.sh >> ${LOGNAME} 2>&1
...
if [[ $ERROR = "TRUE" ]]; then
	# Send Failure email	
	SUBJECT="RAND_FFS_PTB_Driver.sh - Failed (${ENVNAME})"
	MSG="An error was encountered within one or more of the RAND FFS Part B extracts. Please refer to the logs for next steps."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${RAND_FFS_EMAIL_SENDER}" "${RAND_FFS_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1

	exit 12
fi

SUBJECT="RAND FFS Part B Completed (${ENVNAME})" 
MSG="The RAND FFS Part B extracts from Snowflake have completed."
...
###########################################################################################
echo "" >> ${LOGNAME}
echo "RAND_FFS_PTB_Driver.sh completed successfully." >> ${LOGNAME}
