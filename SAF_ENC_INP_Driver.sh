#!/usr/bin/bash
############################################################################################################
# Name:  SAF_ENC_INP_Driver.sh
#
# Desc: SAF Encounter Inpatient Driver script 
#
# Execute as ./SAF_ENC_INP_Driver.sh 
#
...
LOGNAME=/app/IDRC/XTR/CMS/logs/SAF_ENC_INP_Driver_${TMSTMP}.log
...
echo "################################### " >> ${LOGNAME}
echo "SAF_ENC_INP_Driver.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
#############################################################
# Execute script for INP
#############################################################
echo "" >> ${LOGNAME}
echo "Execute script SAF_ENC_INP_SNF_Extract.sh"  >> ${LOGNAME}
${RUNDIR}SAF_ENC_INP_SNF_Extract.sh INP  2>> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
		...
		# Send Failure email	
		SUBJECT="SAF_ENC_INP_SNF_Extract.sh  - Failed"
...
#############################################################
echo "" >> ${LOGNAME}
echo "SAF_ENC_INP_Driver.sh completed successfully." >> ${LOGNAME}
