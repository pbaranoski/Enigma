#!/usr/bin/bash
############################################################################################################
# Name:  SRTR_ENC_DME_Extract.sh
#
# Desc: SRTR Encounter DME Extract 
#
# Execute as ./SRTR_ENC_DME_Extract.sh 
#
...
LOGNAME=/app/IDRC/XTR/CMS/logs/SRTR_ENC_DME_Extract_${TMSTMP}.log
...
echo "################################### " >> ${LOGNAME}
echo "SRTR_ENC_DME_Extract.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="SRTR Encounter DME Extract - Failed"
...
do
		...
		#############################################################
		echo "" >> ${LOGNAME}
		echo "Start execution of SRTR_ENC_DME_Extract.py program"  >> ${LOGNAME}
		...
		if [[ $RET_STATUS != 0 ]]; then
				echo "" >> ${LOGNAME}
				echo "Python script SRTR_ENC_DME_Extract.py failed" >> ${LOGNAME}
				
				# Send Failure email	
				SUBJECT="SRTR_ENC_DME_Extract.sh  - Failed"
				MSG="SRTR Encounter DME extract has failed."
				${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_ENC_EMAIL_SENDER}" "${SRTR_ENC_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
		...
		echo "" >> ${LOGNAME}
		echo "Python script SRTR_ENC_DME_Extract.py completed successfully. " >> ${LOGNAME}
...
SUBJECT="SRTR Encounter DME Extract" 
MSG="The SRTR Encounter DME Extract from Snowflake has completed.\n\nThe following file(s) were created:\n\n${S3Files}"
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="Sending Success email in SRTR_ENC_DME_Extract.sh  - Failed"
	MSG="Sending Success email in SRTR_ENC_DME_Extract.sh  has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_ENC_EMAIL_SENDER}" "${SRTR_ENC_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#############################################################
echo " " >> ${LOGNAME}
echo "EFT SRTR Encounter DME Extract Files " >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="SRTR Encounter DME EFT process  - Failed"
	MSG="SRTR Encounter DME EFT process has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_ENC_EMAIL_SENDER}" "${SRTR_ENC_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
