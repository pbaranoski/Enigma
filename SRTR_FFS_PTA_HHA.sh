#!/usr/bin/bash
############################################################################################################
# Name: SRTR_FFS_PTA_HHA.sh
#
# Desc: SRTR FFS Part A Extract for HHA
#
...
LOGNAME=/app/IDRC/XTR/CMS/logs/SRTR_FFS_PTA_HHA_${TMSTMP}.log
touch ${LOGNAME}
...
echo "################################### " >> ${LOGNAME}
echo "SRTR_FFS_PTA_HHA.sh started at ${TMSTMP} " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="SRTR FFS Part A HHA Extract - Failed"
...
do
	...
	if [[ $RET_STATUS != 0 ]]; then
		...
		# Send Failure email	
		SUBJECT="SRTR_FFS_PTA_HHA.sh  - Failed"
		MSG="SRTR FFS Part A HHA extract has failed in extract step."
		${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_FFS_EMAIL_SENDER}" "${SRTR_FFS_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
		exit 12
	fi

	echo "" >> ${LOGNAME}
	echo "Python script SRTR_FFS_PartA_Extract.py completed successfully for HHA. " >> ${LOGNAME}
done
...
###########################################################################################
echo " " >> ${LOGNAME}
echo "EFT SRTR FFS Part A HHA Extract Files " >> ${LOGNAME}
...
if [ $RET_STATUS != 0 ]; then
	...
	# Send Failure email	
	SUBJECT="SRTR_FFS_PTA_HHA.sh - Failed"
	MSG="EFT for SRTR FFS Part A HHA has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_FFS_EMAIL_SENDER}" "${SRTR_FFS_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
SUBJECT="SRTR FFS Part A HHA Extract" 
MSG="The SRTR FFS Part A HHA extract from Snowflake has completed.\n\nThe following file(s) were created:\n\n${S3Files}"
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="Sending Success email in SRTR_FFS_PTA_HHA.sh - Failed"
	MSG="Sending Success email in SRTR_FFS_PTA_HHA.sh has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_FFS_EMAIL_SENDER}" "${SRTR_FFS_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
###########################################################################################
echo "" >> ${LOGNAME}
echo "SRTR_FFS_PTA_HHA.sh completed successfully." >> ${LOGNAME}
