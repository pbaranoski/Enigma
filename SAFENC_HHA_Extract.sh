#!/usr/bin/bash
############################################################################################################
# Name: SAFENC_HHA_Extract.sh
#
# Desc: SAF ENC HHA Extract
##
#
#  $1 = HHA
#
...
#
# Modified:
# Viren Khanna 	 2023-07-06 New script.
...
LOGNAME=/app/IDRC/XTR/CMS/logs/SAFENC_HHA_Extract_${TMSTMP}.log
...
echo "################################### " >> ${LOGNAME}
echo "SAFENC_HHA_Extract.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
S3BUCKET=${SAFENC_HHA_BUCKET} 
echo "SAF ENC HHA bucket=${S3BUCKET}" >> ${LOGNAME}
...
aws s3 rm s3://${S3BUCKET} --recursive --exclude "*" --include "SAFENC_HHA_*"  2>> ${LOGNAME}
...
SAF_ENC_PARM_FILE=SAF_ENC_HHA_PARM_FILE.txt
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="SAF Encounter HHA Extract - Failed (${ENVNAME})"
...
echo "" >> ${LOGNAME}
echo "SAF Encounter HHA Extract Parameter file on linux: ${ParmFile2Process}" >> ${LOGNAME}



#################################################################################
# Loop thru SAF Encounter HHA Date Ranges in parameter file.
# 
...
do
		...
		export YYYY
                ...
                #############################################################
		echo "" >> ${LOGNAME}
		echo "Start execution of SAFENC_HHA_Extract.py program"  >> ${LOGNAME}
		${PYTHON_COMMAND} ${RUNDIR}SAFENC_HHA_Extract.py >> ${LOGNAME} 2>&1
		...
		if [[ $RET_STATUS != 0 ]]; then
				echo "" >> ${LOGNAME}
				echo "Python script SAFENC_HHA_Extract.py failed" >> ${LOGNAME}
				
				# Send Failure email	
				SUBJECT="SAFENC_HHA_Extract.sh - Failed (${ENVNAME})"
				MSG="SAF ENC HHA extract has failed."
				${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SAFENC_EMAIL_SENDER}" "${SAFENC_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
		...
		echo "" >> ${LOGNAME}
		echo "Python script SAFENC_HHA_Extract.py completed successfully. " >> ${LOGNAME}
		...
		concatFilename=SAFENC_HHA_${STUS_LIT}_${RNG_LIT}_${TMSTMP}.txt.gz
		...
		if [[ $RET_STATUS != 0 ]]; then
				...
				# Send Failure email	
				SUBJECT="Combining S3 files in SAF_ENC_HHA_Extract.sh - Failed (${ENVNAME})"
				MSG="Combining S3 files in SAF_ENC_HHA_Extract.sh has failed."
				${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SAFENC_EMAIL_SENDER}" "${SAFENC_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
SUBJECT="SAF ENC HHA extract (${ENVNAME})" 
MSG="The SAF ENC HHA extract from Snowflake has completed.\n\nThe following file(s) were created:\n\n${S3Files}"
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="Sending Success email in SAFENC_HHA_Extract.sh - Failed (${ENVNAME})"
	MSG="Sending Success email in SAFENC_HHA_Extract.sh  has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SAFENC_EMAIL_SENDER}" "${SAFENC_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#############################################################
echo " " >> ${LOGNAME}
echo "EFT SAF Encounter HHA Extract Files " >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="SAF Encounter HHA EFT process - Failed (${ENVNAME})"
	MSG="SAF Encounter HHA EFT process has failed."
...
#############################################################
echo "" >> ${LOGNAME}
echo "SAFENC_HHA_Extract.sh completed successfully." >> ${LOGNAME}
