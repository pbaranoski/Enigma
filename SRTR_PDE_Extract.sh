#!/usr/bin/sh
############################################################################################################
# Name: SRTR_PDE_Extract.sh
#
# Desc: SRTR PDE Extract
#
# Author     : Viren Khanna
# Created    : 2/13/2023
#
# Modified:
# Viren Khanna 	2023-02-13 	New script.
# 06/08/23    - Sumathi Gayam  -- Added EFT Functionality
############################################################################################################
...
LOGNAME=/app/IDRC/XTR/CMS/logs/SRTR_PDE_Extract_${TMSTMP}.log
...
echo "################################### " >> ${LOGNAME}
echo "SRTR_PDE_Extract.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
S3BUCKET=${SRTR_PDE_BUCKET} 
echo "SRTR PDE bucket=${S3BUCKET}" >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="SRTR Encounter HHA Extract - Failed"
	MSG="Copying S3 files from ${CONFIG_BUCKET} failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_PDE_EMAIL_SENDER}" "${SRTR_PDE_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
do
		...
		echo "CLM_TYPE=${CLM_TYPE}" >> ${LOGNAME}
		echo "CLM_TYPE_CD=${CLM_TYPE_CD}" >> ${LOGNAME}
		...
		#############################################################
		echo "" >> ${LOGNAME}
		echo "Start execution of SRTR_PDE_Extract.py program"  >> ${LOGNAME}
		${PYTHON_COMMAND} ${RUNDIR}SRTR_PDE_Extract.py >> ${LOGNAME} 2>&1
		...
		if [[ $RET_STATUS != 0 ]]; then
				echo "" >> ${LOGNAME}
				echo "Python script SRTR_PDE_Extract.py failed" >> ${LOGNAME}
				
				# Send Failure email	
				SUBJECT="SRTR_PDE_Extract.sh  - Failed"
				MSG="SRTR PDE extract has failed."
				${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_PDE_EMAIL_SENDER}" "${SRTR_PDE_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
		...
		echo "" >> ${LOGNAME}
		echo "Python script SRTR_PDE_Extract.py completed successfully. " >> ${LOGNAME}
...
if [ $RET_STATUS != 0 ]; then
		...
		# Send Failure email	
		SUBJECT="Get S3 Extract file list in SRTR_PDE_Extract.sh - Failed"
		MSG="S3 Extract file list in SRTR_PDE_Extract.sh has failed."
		${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_PDE_EMAIL_SENDER}" "${SRTR_PDE_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
SUBJECT="SRTR PDE extract" 
MSG="The SRTR PDE extract from Snowflake has completed.\n\nThe following file(s) were created:\n\n${S3Files}"

${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_PDE_EMAIL_SENDER}" "${SRTR_PDE_EMAIL_SUCCESS_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="Sending Success email in SRTR_PDE_Extract.sh  - Failed"
	MSG="Sending Success email in SRTR_PDE_Extract.sh  has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_PDE_EMAIL_SENDER}" "${SRTR_PDE_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#############################################################
echo " " >> ${LOGNAME}
echo "EFT SRTR PART D Extract Files " >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="SRTR PART D EFT process  - Failed"
	MSG="SRTR EPART DEFT process has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_PDE_EMAIL_SENDER}" "${SRTR_PDE_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#############################################################
echo "" >> ${LOGNAME}
echo "SRTR_PDE_Extract.sh completed successfully." >> ${LOGNAME}
