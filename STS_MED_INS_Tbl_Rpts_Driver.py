#!/usr/bin/env python
########################################################################################################
# Name:  STS_MED_INS_Tbl_Rpts_Driver.py
#
...
#
# Execute as python3 STS_MED_INS_Tbl_Rpts_Driver.py     (processing without override date) 
# Execute as python3 STS_MED_INS_Tbl_Rpts_Driver.py $1  (processing with override date --> YYYY-MM-DD format) 
#
...
# Vijay Mandavilli 2026-05-20 Updated code with common functions and test logic.
...
S3BUCKET = rf"{XTR_BUCKET}/{STS_MED_INS_BUCKET_FLDR}"
...
def main_processing_loop():
    ...
    try:    
        ...
        LOGNAME = f"{LOG_DIR}{TESTLOG}STS_MED_INS_Tbl_Rpts_{TMSTMP}.log"
        ...
        rootLogger.info(f"\nSTS_MED_INS_Tbl_Rpts_Driver.py started at {TMSTMP}")
        ...
        #############################################################
        rootLogger.info("")
        rootLogger.info("Start execution of STS_MED_INS_Tbl_Rpts.py program")

        try:
            sp_info = subprocess.run(['python3', 'STS_MED_INS_Tbl_Rpts.py'], capture_output=True, text=True, check=True)
            write_sp_info_2_log(sp_info) 
            
        except subprocess.CalledProcessError as e:
            rootLogger.error(f"Calling STS_MED_INS_Tbl_Rpts.py failed with return code {e.returncode}")
            rootLogger.error("\n%s", e.stdout)
            rootLogger.error("\n%s", e.stderr)
            
            ## Send Failure email	
            SUBJECT=f"STS Medical Insurance Table Report - Failed ({ENVNAME})"
            MSG=f"Python script STS_MED_INS_Tbl_Rpts.py failed."
        ...
        rootLogger.info("")
        rootLogger.info("Python script STS_MED_INS_Tbl_Rpts_Driver.py completed successfully.")
        ...
        # Send Success email	
        SUBJECT=f"STS MED INS Tbl Report - completed ({ENVNAME}{TESTEMAIL})"
        MSG=f"STS Medical Insurance Table Report completed. \n\nThe following extract files were created:\n\n{S3Files}"
        ...
        #####################################################
        try:
            CreManDr.createManifestFile(bucket=XTR_BUCKET, s3folder=STS_MED_INS_BUCKET_FLDR, runToken=TMSTMP, BoxEmails=STS_MED_INS_BOX_RECIPIENT )

        except Exception as e:

            SUBJECT=f"Create Manifest file in STS_MED_INS_Tbl_Rpts_Driver.py - Failed ({ENVNAME})"
            MSG=f"Create Manifest file in STS_MED_INS_Tbl_Rpts_Driver.py  has failed."
        ...
        rootLogger.info("Script STS_MED_INS_Tbl_Rpts_Driver.py completed successfully.")
        rootLogger.info(f"\nEnded at {TMSTMP}" )


    except Exception as e:
        print (f"Exception occured in STS_MED_INS_Tbl_Rpts_Driver.py\n {e}")

        rootLogger.error("Exception occured in STS_MED_INS_Tbl_Rpts_Driver.py.")
        rootLogger.error("\n%s", str(e))

        ## Send Failure email	
        SUBJECT = f"Exception occured in STS_MED_INS_Tbl_Rpts_Driver.py - Failed ({ENVNAME}{TESTEMAIL})"
        MSG= f"Exception occured in STS_MED_INS_Tbl_Rpts_Driver.py {e}.  Process failed"
