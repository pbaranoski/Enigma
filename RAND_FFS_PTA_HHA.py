...
def main_processing_loop():
    ...
    try:    
        ...
        LOGNAME = f"{LOG_DIR}{TESTLOG}RAND_FFS_PTA_HHA_{TMSTMP}.log"
        ...
        rootLogger.info(f"\nRAND_FFS_PTA_HHA.py started at {TMSTMP}")
        ...
        #############################################################
        rootLogger.info("")
        rootLogger.info("RAND FFS PTA HHA Processing will use the following parameters:")
        ...
        for i in range(len(start_date_parms)):
            ...
            XTR_FILE_NAME = f"RAND_FFS_HHA_Y{EXT_YYYY}_P{RNG}_{TMSTMP}.csv.gz"
            ...
            try:
                ...
                ## Send Failure email	
                SUBJECT=f"RAND_FFS_PTA_HHA.py extract - Failed ({ENVNAME}{TESTEMAIL})"
            ...
            try:
                ...
                ## Send Failure email	
                SUBJECT=f"Combining S3 files in RAND_FFS_PTA_HHA.py - Failed ({ENVNAME}{TESTEMAIL})"
                MSG=f"Combining S3 files in RAND_FFS_PTA_HHA.py has failed."
        ...
        SUBJECT=f"RAND FFS PTA HHA Extract Complete ({ENVNAME}{TESTEMAIL})" 
        MSG=f"RAND FFS PTA HHA Extract completed successfully.\n\nThe following file(s) were created:\n\n{S3Files}"
        ...
        ######################################################################
        rootLogger.info("")
        rootLogger.info(f"Creating manifest files for RAND FFS PTA HHA Extract. ")
        ...
        try:
            ...
            SUBJECT=f"Create Manifest file in RAND_FFS_PTA_HHA.py - Failed ({ENVNAME}{TESTEMAIL})"
            MSG=f"Create Manifest file in RAND_FFS_PTA_HHA.py  has failed. {e}"
        ...
        rootLogger.info("RAND FFS PTA HHA Extract completed successfully.")
        rootLogger.info(f"\nEnded at {TMSTMP}" )


    except Exception as e:
        print (f"Exception occured in RAND_FFS_PTA_HHA.py\n {e}")

        rootLogger.error("Exception occured in RAND_FFS_PTA_HHA.py.")
        rootLogger.error("\n%s", str(e))

        ## Send Failure email	
        SUBJECT=f"RAND_FFS_PTA_HHA.py  - Failed ({ENVNAME}{TESTEMAIL})"
        MSG=f"Exception occured in RAND_FFS_PTA_HHA.py {e}. Process failed. "
