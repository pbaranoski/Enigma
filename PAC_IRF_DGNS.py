#!/usr/bin/env python
########################################################################################################
# Name:  PAC_IRF_DGNS.py
#
# Desc: Script to Extract Inpatient Rehabilitation Facility(IRF) Diagnosis Data
#
...
try:
   ...
   #**************************************
   #   Extract Inpatient Rehabilitation Facility(IRF) Diagnosis Data
   #**************************************   
   snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_PAC_STG/PAC_IRF_DGNS_Y{FYQ}_{TMSTMP}.csv.gz
                                                FROM (
...
LEFT OUTER JOIN IDRC_{ENVNAME}.CMS_FCT_CLM_{ENVNAME}.CLM_PROD CP  
...
AND (C.CLM_THRU_DT between
            dateadd('month',-10, date_trunc('month',CURRENT_DATE()))
             and 
LAST_DAY(ADD_MONTHS(CURRENT_DATE,-8))) 
AND (Substr(C.CLM_BLG_PRVDR_OSCAR_NUM,3,4) BETWEEN '3025' AND '3099'OR (Substr(C.CLM_BLG_PRVDR_OSCAR_NUM,3,1) = 'T')  )  
AND C.CLM_PMT_AMT >0   
