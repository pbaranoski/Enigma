...
CUR_YR=`date +%Y`  >> ${LOGNAME}  2>&1
MNUP_YR=`expr ${CUR_YR} - 1`
...
#############################################################
echo "" >> ${LOGNAME}
echo "Start execution of MNUP_MED_NONUTIL_ext.py program"  >> ${LOGNAME}

# Export environment variables for Python code
export TMSTMP
export MNUP_YR

${PYTHON_COMMAND} ${RUNDIR}MNUP_MED_NONUTIL_ext.py >> ${LOGNAME} 2>&1
...
echo "" >> ${LOGNAME}
echo "Python script MNUP_MED_NONUTIL_ext.py completed successfully. " >> ${LOGNAME}
...
################################################################
echo " " >> ${LOGNAME}
echo "EFT MNUP Extract File " >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT=" MNUP Annaul EFT process  - Failed (${ENVNAME})"
	MSG=" MNUP Annual EFT process has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${MNUP_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
################################################################
echo "" >> ${LOGNAME}
echo "Create Manifest file for SFTP of MNUP Annual Extract.  " >> ${LOGNAME}
