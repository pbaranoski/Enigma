...
COPY INTO @BIA_PRD.CMS_STAGE_XTR_PRD.BIA_PRD_XTR_DOJ_STG/DOJ_SFUI_REQ161_SNF_IP_20260529.120000.txt.gz FROM (
...
WITH SNF_BENES AS (
    ...
    AND C.CLM_FROM_DT BETWEEN TO_DATE('2014-01-01','YYYY-MM-DD') AND TO_DATE('2020-12-31','YYYY-MM-DD')
    AND C.CLM_BLG_PRVDR_OSCAR_NUM IN ('055952', '056079', '056186', '056188', '056313', '056352', '056438', '555368', '555781', '555780')
...
,SNF_N_INP_BENES AS (
    ...
    INNER JOIN SNF_BENES SNF_BENE 
    ON C.BENE_SK    =  SNF_BENE.BENE_SK
    
    INNER JOIN IDRC_PRD.CMS_DIM_BENE_PRD.BENE BENE
    ON C.BENE_SK    =  BENE.BENE_SK

    -- SNF claim
    WHERE C.CLM_TYPE_CD IN ('60')
    AND C.CLM_FROM_DT BETWEEN TO_DATE('2014-01-01','YYYY-MM-DD') AND TO_DATE('2020-12-31','YYYY-MM-DD')
    AND C.CLM_FINL_ACTN_IND = 'Y'
  ...
  SELECT 
	...
	,RPAD(CASE WHEN PROD_MT.CLM_DGNS_25_CD = '~' THEN ' ' ELSE COALESCE(PROD_MT.CLM_DGNS_25_CD,' ') END,7,' ')  AS CLM_DGNS_25_CD 
      ...
      ,'END'
        ...
        -- Bene on claim has to be bene that is in SNF_N_INP_BENES View
        INNER JOIN SNF_N_INP_BENES 
        ON C.BENE_SK    =  SNF_N_INP_BENES.BENE_SK
...
WHERE C.CLM_FROM_DT BETWEEN TO_DATE('2014-01-01','YYYY-MM-DD') AND TO_DATE('2020-12-31','YYYY-MM-DD')
AND C.CLM_FINL_ACTN_IND = 'Y'

-- We want only the claims: 1) for the Benes that have both SNF and INP claims of interest to us
--                          2) And the claims that match the SNF and INP critera (We don't want other claim types OR (INP/SNF claims that  don't match our critera)) 
AND (
        ( C.CLM_TYPE_CD IN ('20','30') AND C.CLM_BLG_PRVDR_OSCAR_NUM IN ('055952', '056079', '056186', '056188', '056313', '056352', '056438', '555368', '555781', '555780')
        )
      OR
        ( C.CLM_TYPE_CD IN ('60') 
        )   
...
ORDER BY C.BENE_SK, C.CLM_FROM_DT, CLM_LINE_NUM
