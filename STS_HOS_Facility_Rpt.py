...
# Viren Khanna 2025-08-05 Updated script with HOS Code.
#
########################################################################################################
# IMPORTS
########################################################################################################
...
try:
   ...
   snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_STS_HOS_FACILITY_STG/STS_HOS_FACILITY_RPT_AA6_{EXT_TO_YYYY}_{RUN_PRD}_{TMSTMP}.csv.gz
    ...
    SELECT '1' AS RPT_CLM_TYPE_CD, 'HOSPITAL'      AS RPT_CLM_TYPE_DESC, '1' AS SORT_ORD_IND FROM DUAL
    UNION
    SELECT '2' AS RPT_CLM_TYPE_CD, 'SKILLED NURSING FACILITY'  AS RPT_CLM_TYPE_DESC, '2' AS SORT_ORD_IND FROM DUAL
    UNION
    SELECT '3' AS RPT_CLM_TYPE_CD, 'INTERMEDIATE CARE FACILITY'      AS RPT_CLM_TYPE_DESC, '3' AS SORT_ORD_IND FROM DUAL
    UNION
    SELECT '4' AS RPT_CLM_TYPE_CD, 'HOME HEALTH AGENCY'                AS RPT_CLM_TYPE_DESC, '4' AS SORT_ORD_IND FROM DUAL
    UNION
    SELECT '5' AS RPT_CLM_TYPE_CD,  'FREESTANDING HOSPICE'               AS RPT_CLM_TYPE_DESC, '5' AS SORT_ORD_IND FROM DUAL
...
,PARTA_DTL_CLMS  AS (
            ...
            SELECT C.GEO_BENE_SK
                ...
                ,CASE WHEN C.CLM_QUERY_CD = '0' 
                  ...
                  , CASE WHEN FF.HOSPICE_TYPE_DESC = 'Hospital' THEN '1'
                  WHEN FF.HOSPICE_TYPE_DESC = 'Skilled Nursing Facility' THEN '2'
                  WHEN FF.HOSPICE_TYPE_DESC = 'INTERMEDIATE CARE FACILITY' THEN '3'
                  WHEN FF.HOSPICE_TYPE_DESC = 'Home Health Agency' THEN '4'
                   WHEN FF.HOSPICE_TYPE_DESC = 'Freestanding Hospice' THEN '5' 
                ELSE '6' END AS RPT_CLM_TYPE_CD 
          ...
          AND  C.CLM_QUERY_CD <> 'C'
            AND C.CLM_TYPE_CD = '50'
            
           AND RPT_CLM_TYPE_CD IN ('1','2','3','4','5') 
          -- AND RPT_CLM_TYPE_CD IN ('2') 
          AND FF.HOSPICE_TYPE_CD = '12'
      ---    AND C.CLM_BLG_PRVDR_OSCAR_NUM = '491553'
...
SELECT CAL_YEAR
     ...
     ,SUM(CASE WHEN RPT_CLM_TYPE_CD = '1' THEN CHRG_AMT  ELSE 0 END) AS HOSPITAL_CHRG_AMT
     ,SUM(CASE WHEN RPT_CLM_TYPE_CD = '1' THEN PMT_AMT   ELSE 0 END) AS HOSPITAL_PMT_AMT
     ,SUM(CASE WHEN RPT_CLM_TYPE_CD = '2' THEN CHRG_AMT  ELSE 0 END) AS SNF_CHRG_AMT
     ,SUM(CASE WHEN RPT_CLM_TYPE_CD = '2' THEN PMT_AMT   ELSE 0 END) AS SNF_PMT_AMT
    
     ,SUM(CASE WHEN RPT_CLM_TYPE_CD = '3' THEN CHRG_AMT  ELSE 0 END) AS ICF_CHRG_AMT
     ,SUM(CASE WHEN RPT_CLM_TYPE_CD = '3' THEN PMT_AMT   ELSE 0 END) AS ICF_PMT_AMT

     
     ,SUM(CASE WHEN RPT_CLM_TYPE_CD = '4' THEN CHRG_AMT  ELSE 0 END) AS HHA_CHRG_AMT
     ,SUM(CASE WHEN RPT_CLM_TYPE_CD = '4' THEN PMT_AMT   ELSE 0 END) AS HHA_PMT_AMT

     
     ,SUM(CASE WHEN RPT_CLM_TYPE_CD = '5' THEN CHRG_AMT  ELSE 0 END) AS FREESTANDING_HOSPICE_CHRG_AMT
     ,SUM(CASE WHEN RPT_CLM_TYPE_CD = '5' THEN PMT_AMT   ELSE 0 END) AS FREESTANDING_HOSPICE_PMT_AMT
