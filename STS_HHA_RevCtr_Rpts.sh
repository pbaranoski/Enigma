...
#
######################################################################################
# Name:  STS_HHA_RevCtr_Rpts.sh
#        
# DESC:   This script extracts data for STS HHA table report - HHA Rev Cntr UNITS/CHRGS	  
#         by period expense (legacy AA5 report)
#
# Created: Paul Baranoski  08/27/2024
# Modified: 
#
# Paul Baranoski 2024-08-27 Create script.
# Paul Baranoski 2024-09-12 Changed script/report to be semi-annual instead of quarterly.
# Paul Baranoski 2025-02-19 Change to use new STS_HHA_REV_CTR Bucket and Email constant.
######################################################################################
set +x
...
LOGNAME=/app/IDRC/XTR/CMS/logs/STS_HHA_RevCtr_Rpts_${TMSTMP}.log
...
echo "################################### " >> ${LOGNAME}
echo "STS_HHA_RevCtr_Rpts.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
S3BUCKET=${STS_HHA_REV_CTR_BUCKET} 
...
#	RUN_PRD=SEP
	...
	# Send Failure email	
	SUBJECT="STS HHA RevCtr Report - Failed (${ENVNAME})"
...
EXT_FROM_YYYY=$((EXT_TO_YYYY-2))  2>> ${LOGNAME}
...
#############################################################
echo "" >> ${LOGNAME}
echo "Run STS_HHA_RevCtr_Rpts.py program" >> ${LOGNAME}

${PYTHON_COMMAND} ${RUNDIR}STS_HHA_RevCtr_Rpts.py  >> ${LOGNAME} 2>&1
...
if [[ $RET_STATUS != 0 ]]; then
	echo "" >> ${LOGNAME}
	echo "Python script STS_HHA_RevCtr_Rpts.py failed" >> ${LOGNAME}
	
	# Send Failure email	
	SUBJECT="STS HHA RevCtr Report - Failed (${ENVNAME})"
	MSG="Python script STS_HHA_RevCtr_Rpts.py failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
echo "" >> ${LOGNAME}
echo "Python script STS_HHA_RevCtr_Rpts.py completed successfully." >> ${LOGNAME}
...
# Send Success email	
SUBJECT="STS HHA RevCtr Report - completed ($ENVNAME)"
MSG="STS HHA RevCtr Report completed. \n\nThe following extract files were created:\n\n${S3Files}"
${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${STS_HHA_REV_CTR_EMAIL_SUCCESS_RECIPIENT}" "${SUBJECT}" "${MSG}"  >> ${LOGNAME} 2>&1
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="Sending Success email in STS_HHA_RevCtr_Rpts.sh  - Failed (${ENVNAME})"
	MSG="Sending Success email in STS_HHA_RevCtr_Rpts.sh has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="Create Manifest file in STS_HHA_RevCtr_Rpts.sh - Failed (${ENVNAME})"
	MSG="Create Manifest file in STS_HHA_RevCtr_Rpts.sh  has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#############################################################
echo "" >> ${LOGNAME}
echo "STS_HHA_RevCtr_Rpts.sh completed successfully." >> ${LOGNAME}
