...
#
# Created: Paul Baranoski  08/09/2024
# Modified: 
#
...
echo "################################### " >> ${LOGNAME}
echo "STS_MED_INS_Tbl_Rpts.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
if [ "${CUR_MM}" = "07" ];then
	...
	# Send Failure email	
	SUBJECT="STS Medical Insurance Table Report - Failed (${ENVNAME})"
...
EXT_FROM_YYYY=$((EXT_TO_YYYY-6))  2>> ${LOGNAME}
...
#############################################################
echo "" >> ${LOGNAME}
echo "Run STS_MED_INS_Tbl_Rpts.py program" >> ${LOGNAME}

${PYTHON_COMMAND} ${RUNDIR}STS_MED_INS_Tbl_Rpts.py  >> ${LOGNAME} 2>&1
...
if [[ $RET_STATUS != 0 ]]; then
	echo "" >> ${LOGNAME}
	echo "Python script STS_MED_INS_Tbl_Rpts.py failed" >> ${LOGNAME}
	
	# Send Failure email	
	SUBJECT="STS Medical Insurance Table Report - Failed (${ENVNAME})"
	MSG="Python script STS_MED_INS_Tbl_Rpts.py failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
echo "" >> ${LOGNAME}
echo "Python script STS_MED_INS_Tbl_Rpts.py completed successfully." >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="Sending Success email in STS_MED_INS_Tbl_Rpts.sh  - Failed (${ENVNAME})"
	MSG="Sending Success email in STS_MED_INS_Tbl_Rpts.sh has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#############################################################
echo "" >> ${LOGNAME}
echo "Create Manifest file for STS Medical Insurance Table Report.  " >> ${LOGNAME}

${RUNDIR}CreateManifestFile.sh ${S3BUCKET} ${TMSTMP} ${STS_MED_INS_BOX_RECIPIENT} 
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="Create Manifest file in STS_MED_INS_Tbl_Rpts.sh - Failed (${ENVNAME})"
	MSG="Create Manifest file in STS_MED_INS_Tbl_Rpts.sh  has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#############################################################
echo "" >> ${LOGNAME}
echo "STS_MED_INS_Tbl_Rpts.sh completed successfully." >> ${LOGNAME}
