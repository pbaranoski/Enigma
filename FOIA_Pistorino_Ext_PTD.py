#!/usr/bin/env python
########################################################################################################
# Name:  DOJ_Pistarino_Ext_PTD.py
#
# Desc: DOJ Pistarino Extract PTD SQL
#
#
# Paul Baranoski 2025-12-23 Created module.
########################################################################################################
import os
import sys
import datetime
...
try:
   ...
   snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_FOIA_STG/FOIA_{REQ}_PISTORINO{PART}_EXT_{CLM_TYPE_LIT}_{FILE_LIT}_{TMSTMP}.txt.gz
            ...
            SELECT DISTINCT
                ...
                CASE WHEN RPAD(C.CLM_PRSBNG_PRVDR_GNRC_ID_NUM,35,' ') > '  ' 
                ...
                --****\/****			
                --OLD: NUMBER(8,2) -9(7)v99 	NEW: NUMBER(11,2) S9(9)v99
                ...
                LPAD(CLR.CLM_PTD_MODEL_CD,2,' ') AS CLM_PTD_MODEL_CD,   
                'END' AS END_REC
