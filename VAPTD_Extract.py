#!/usr/bin/env python
########################################################################################################
# Name:   VAPTD_Extract.py
# DESC:   This python program extracts the VA Part D data from Snowflake and loads the file to S3
#
# Created: Joshua Turner
# Modified: 	12/19/2022
# N.Tinovsky 	11/25/2024	Increase length of fields: CLM_LINE_CALCD_GAP_DSCNT_AMT, CLM_LINE_GRS_ABOVE_THRSHLD_AMT
#			, CLM_LINE_GRS_BLW_THRSHLD_AMT , CLM_LINE_INGRDNT_CST_AMT , CLM_LINE_LIS_AMT , CLM_LINE_NCVRD_PD_AMT
#			, CLM_LINE_PLRO_AMT, CLM_LINE_RPTD_GAP_DSCNT_AMT, CLM_LINE_SLS_TAX_AMT, CLM_LINE_SRVC_CST_AMT
#			, CLM_LINE_TROOP_TOT_AMT, CLM_LINE_VCCN_ADMIN_FEE_AMT, CLM_LTC_DSPNSNG_MTHD_CD , CLM_PRSBNG_PRVDR_GNRC_ID_NUM
########################################################################################################
# IMPORTS
########################################################################################################
import os
...
try:
    ...
    ########################################################################################################
    # Extract VA Part D data and write to S3 as a pipe delimited flat file
    ########################################################################################################
    ...
    SELECT DISTINCT
        ...
        /*  ADD FINDER FILE JOIN  */
        INNER JOIN "BIA_{ENVNAME}"."CMS_TARGET_XTR_{ENVNAME}"."VA_PTD_MOA_FFTAB" FND
           ON FND.SSN          = B.BENE_SSN_NUM
        ...
        WHERE C.CLM_FINL_ACTN_IND = 'Y'
            AND CL.CLM_LINE_FROM_DT BETWEEN '{START_DATE}' AND '{END_DATE}'                        
