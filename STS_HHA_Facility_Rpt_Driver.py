#!/usr/bin/env python
########################################################################################################
# Name:  STS_HHA_Facility_Driver.py
#
...
#
# Execute as python3 STS_HHA_Facility_Driver.py     (processing without override date) 
# Execute as python3 STS_HHA_Facility_Driver.py $1  (processing with override date --> YYYY-MM-DD format) 
#
...
S3BUCKET = rf"{XTR_BUCKET}/{STS_HHA_FACILITY_BUCKET_FLDR}"
...
def main_processing_loop():
    ...
    try:    
        ...
        LOGNAME = f"{LOG_DIR}{TESTLOG}STS_HHA_Facility_Rpt_{TMSTMP}.log"
        ...
        rootLogger.info(f"\nSTS_HHA_Facility_Driver.py started at {TMSTMP}")
        ...
        if CUR_MM == "04":
            ...
            # Send Failure email	
            SUBJECT=f"STS HHA Facility Report - Failed ({ENVNAME}{TESTEMAIL})"
        ...
        #############################################################
        rootLogger.info("")
        rootLogger.info("Start execution of STS_HHA_Facility_Rpt.py program")

        try:
            sp_info = subprocess.run(['python3', 'STS_HHA_Facility_Rpt.py'], capture_output=True, text=True, check=True)
            write_sp_info_2_log(sp_info) 
            
        except subprocess.CalledProcessError as e:
            rootLogger.error(f"Calling STS_HHA_Facility_Rpt.py failed with return code {e.returncode}")
            rootLogger.error("\n%s", e.output)
            
            ## Send Failure email	
            SUBJECT=f"STS HHA Facility Report - Failed ({ENVNAME}{TESTEMAIL})"
            MSG=f"Python script STS_HHA_Facility_Rpt.py failed."
        ...
        rootLogger.info("")
        rootLogger.info("Python script STS_HHA_Facility_Driver.py completed successfully.")
        ...
        # Send Success email	
        SUBJECT=f"STS HHA Facility Report - completed ({ENVNAME}{TESTEMAIL})"
        MSG=f"STS HHA Facility Report completed. \n\nThe following extract files were created:\n\n{S3Files}"
        
        try:
            sp_info = subprocess.run(['python3', 'sendEmail.py', CMS_EMAIL_SENDER, STS_HHA_FACILITY_EMAIL_SUCCESS_RECIPIENT, SUBJECT, MSG], capture_output=True, text=True, check=True)
            write_sp_info_2_log(sp_info)
        ...
        #####################################################
        try:
            CreManDr.createManifestFile(bucket=XTR_BUCKET, s3folder=STS_HHA_FACILITY_BUCKET_FLDR, runToken=TMSTMP, BoxEmails=STS_HHA_FACILITY_BOX_RECIPIENT )

        except Exception as e:

            SUBJECT=f"Create Manifest file in STS_HHA_Facility_Driver.py - Failed ({ENVNAME}{TESTEMAIL})"
            MSG=f"Create Manifest file in STS_HHA_Facility_Driver.py  has failed."
        ...
        rootLogger.info("Script STS_HHA_Facility_Driver.py completed successfully.")
        rootLogger.info(f"\nEnded at {TMSTMP}" )


    except Exception as e:
        print (f"Exception occured in STS_HHA_Facility_Driver.py\n {e}")

        rootLogger.error("Exception occured in STS_HHA_Facility_Driver.py.")
        rootLogger.error("\n%s", str(e))

        ## Send Failure email	
        SUBJECT=f"STS_HHA_Facility_Driver.py  - Failed ({ENVNAME}{TESTEMAIL})"
        MSG=f"Exception occured in STS_HHA_Facility_Driver.py {e}. Process failed. "
