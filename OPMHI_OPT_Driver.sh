#!/usr/bin/bash
############################################################################################################
# Script Name: OPMHI_OPT_Driver.sh
# Description: This script executes the Part A SQL python script for OPM-HI OPT 
#
...
CUR_DT=`date +%Y%m%d`
LOGNAME=/app/IDRC/XTR/CMS/logs/${TESTLOG}_OPMHI_OPT_Driver_${TMSTMP}.log
...
echo "################################### " >> ${LOGNAME}
echo "OPMHI_OPT_Driver.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
S3BUCKET=${OPMHI_OPT_BUCKET}

#################################################################################
# Clean linux data directory of any residual files
#################################################################################
echo "" >> ${LOGNAME}
echo "Remove any residual FEHB_CMS_OUT_* files on linux data directory" >> ${LOGNAME}
rm ${DATADIR}FEHB_CMS_OUT_*   2>> ${LOGNAME}
...
echo "" >> ${LOGNAME}
echo "OPM-HI OPT Extract is starting with the following parameters" >> ${LOGNAME}
echo "CLAIM TYPE: ${CTYP}" >> ${LOGNAME}
echo "START DATE: ${START_DATE}" >> ${LOGNAME}
echo "END DATE: ${END_DATE}" >> ${LOGNAME}
echo "CLAIM TYPE CD: ${CLM_TYPE_CD}" >> ${LOGNAME}
echo "S3 Bucket: ${S3BUCKET}" >> ${LOGNAME}
echo "EXT_FILENAME: ${EXT_FILENAME}" >> ${LOGNAME}


#################################################################################
# Execute Python code to produce OPT file
#################################################################################
echo "" >> ${LOGNAME}
echo "Start execution of OPMHI_PTA_Extract.py program"  >> ${LOGNAME}

${PYTHON_COMMAND} ${RUNDIR}OPMHI_PTA_Extract.py >> ${LOGNAME} 2>&1
...
if [[ $RET_STATUS != 0 ]]; then
        echo "" >> ${LOGNAME}
        echo "Python script OPMHI_PTA_Extract.py failed" >> ${LOGNAME}
		
        # Send Failure email	
        SUBJECT="OPMHI_OPT_Driver.sh - Failed (${ENVNAME})"
        MSG="OPM-HI OPT extract has failed."
...
if [ $RET_STATUS != 0 ]; then
         ...
         # Send Failure email	
         SUBJECT="OPMHI_OPT_Driver.sh - Failed (${ENVNAME})"
         MSG="CombineS3Files.sh for OPM-HI PTB OPT has failed."
         ${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
SUBJECT="OPMHI_OPT_Driver.sh  - Completed (${ENVNAME}${TESTEMAIL})"
MSG="OPM-HI PTA OPT has completed successfully.\n\nThe following file(s) were created:\n\n${S3Files}"
...
if [[ $RET_STATUS != 0 ]]; then
		...
		# Send Failure email	
		SUBJECT="Sending Success email in OPMHI_OPT_Driver.sh - Failed (${ENVNAME})"
		MSG="Sending Success email in OPMHI_OPT_Driver.sh  has failed."
		${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#############################################################
echo "" >> ${LOGNAME}
echo "Create Manifest file for OPMHI OPT Extract.  " >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="Create Manifest file in OPMHI_OPT_Driver.sh - Failed (${ENVNAME})"
	MSG="Create Manifest file in OPMHI_OPT_Driver.sh has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#################################################################################
echo "" >> ${LOGNAME}
echo "OPMHI_OPT_Driver.sh completed successfully." >> ${LOGNAME}
