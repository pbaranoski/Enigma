#!/usr/bin/bash
############################################################################################################
# Name:  SRTR_ENC_OPT_Extract.sh
#
# Desc: SRTR OPT Extract 
#
# Execute as ./SRTR_ENC_OPT_Extract.sh 
#
...
LOGNAME=/app/IDRC/XTR/CMS/logs/SRTR_ENC_OPT_Extract_${TMSTMP}.log
...
echo "################################### " >> ${LOGNAME}
echo "SRTR_ENC_OPT_Extract.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="SRTR Encounter OPT Extract - Failed"
...
do
		...
		EXT_YEAR=${line}
		CLM_TYPE_LIT=OPT
		EXT_CLM_TYPES="4013,4014,4022,4023,4071,4072,4073,4074,4075,4076,4077,4079,4083,4085,4089"
		...
		#############################################################
		echo "" >> ${LOGNAME}
		echo "Start execution of SRTR_ENC_OPT_Extract.py program"  >> ${LOGNAME}
		...
		if [[ $RET_STATUS != 0 ]]; then
				echo "" >> ${LOGNAME}
				echo "Python script SRTR_ENC_OPT_Extract.py failed" >> ${LOGNAME}
				
				# Send Failure email	
				SUBJECT="SRTR_ENC_OPT_Extract.sh  - Failed"
				MSG="SRTR Encounter Outpatient extract has failed."
				${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_ENC_OPT_EMAIL_SENDER}" "${SRTR_ENC_OPT_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1

				exit 12
		fi

		echo "" >> ${LOGNAME}
		echo "Python script SRTR_ENC_OPT_Extract.py completed successfully. " >> ${LOGNAME}
...
SUBJECT="SRTR Encounter OPT extract" 
MSG="The SRTR Encounter OPT extract from Snowflake has completed.\n\nThe following file(s) were created:\n\n${S3Files}"
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="Sending Success email in SRTR_ENC_OPT_Extract.sh  - Failed"
	MSG="Sending Success email in SRTR_ENC_OPT_Extract.sh  has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_ENC_EMAIL_SENDER}" "${SRTR_ENC_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#############################################################
echo " " >> ${LOGNAME}
echo "EFT SRTR Encounter OPT Extract Files " >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="SRTR Encounter OPT EFT process  - Failed"
	MSG="SRTR Encounter OPT EFT process has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_ENC_EMAIL_SENDER}" "${SRTR_ENC_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
