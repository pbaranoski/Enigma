#!/usr/bin/env python
########################################################################################################
# Name:  OPMHI_PTB_Extract.py
# DESC:  This python script executes the Part B SQL for OPM-HI
#
# Created: Joshua Turner
# Modified: 06/12/2023
#
# 10/03/2023   Paul Baranoski       Modified extract filename to use imported EXT_FILENAME.
########################################################################################################
# IMPORTS
########################################################################################################
import os
...
try:
    ...
    ########################################################################################################
    # Extract OPM-HI Part B data and write to S3
    ########################################################################################################
    snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_{STAGE_NAME}_STG/{EXT_FILENAME}
            ...
            SELECT DISTINCT
                ...
                RPAD(REPLACE(COALESCE(C.CDN.CLM_FPS_RSN_CD,''),'~',''),3,' ') AS CLM_FPS_RSN_CD,
                RPAD(REPLACE(COALESCE(C.CDN.CLM_RMTNC_ADVC_NUM,''),'~',''),7,' ') AS CLM_RMTNC_ADVC_NUM,
                */
                ...
                /*RPAD(REPLACE(COALESCE(PRFNL.CLM_FAC_PRVDR_CPO_OSCAR_NUM,''),'~',''),6,' ') AS CLM_FAC_PRVDR_CPO_OSCAR_NUM,*/
                RPAD(REPLACE(COALESCE(C.PRVDR_CPO_FAC_NPI_NUM,''),'~',''),10,' ') AS PRVDR_CPO_FAC_NPI_NUM,
                /*RPAD(REPLACE(COALESCE(C.CLM_FAC_PRVDR_CPO_ORG_NPI_ID,''),'~',''),10,' ') AS CLM_FAC_PRVDR_CPO_ORG_NPI_ID*/
            ...
            WHERE C.CLM_TYPE_CD BETWEEN {CLM_TYPE_CD1} AND {CLM_TYPE_CD2}
              ...
              AND (
                CL.CLM_LINE_HCPCS_CD NOT IN
                (SELECT OPMHI_CPT_EXCL FROM "BIA_{ENVNAME}"."CMS_TARGET_XTR_{ENVNAME}"."OPMHI_CPT_EXCL")
                OR CPM.CLM_PRNCPL_DGNS_CD NOT IN
                (SELECT OPMHI_ICD10DG_EXCL FROM "BIA_{ENVNAME}"."CMS_TARGET_XTR_{ENVNAME}"."OPMHI_ICD10DGS_EXCL")
                OR CL.CLM_LINE_DGNS_CD NOT IN
                (SELECT OPMHI_ICD10DG_EXCL FROM "BIA_{ENVNAME}"."CMS_TARGET_XTR_{ENVNAME}"."OPMHI_ICD10DGS_EXCL")
               )
