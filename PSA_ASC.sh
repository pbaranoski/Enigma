...
# Modified:
#
# Paul Baranoski 2023-12-08 Created script.
#
############################################################################################################
...
LOGNAME=/app/IDRC/XTR/CMS/logs/PSA_ASC_${TMSTMP}.log
RUNDIR=/app/IDRC/XTR/CMS/scripts/run/
DATADIR=/app/IDRC/XTR/CMS/data/

PSA_FF_ASC_PREFIX=PSA_FINDER_FILE_HCPCS_APC_CATEGORIES
...
echo "################################### " >> ${LOGNAME}
echo "PSA_ASC.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
rm ${DATADIR}tempPSAASC.txt  >> ${LOGNAME} 2>&1
...
echo "List ${PSA_FF_ASC_PREFIX}_* finder files in S3 " >> ${LOGNAME}

S3_FINDER_FILES=`aws s3 ls s3://${FINDER_FILE_BUCKET}${PSA_FF_ASC_PREFIX}`  >> ${LOGNAME} 
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="PSA_ASC.sh - Failed (${ENVNAME})"
	MSG="Listing S3 finder files from ${FINDER_FILE_BUCKET}${PSA_FF_ASC_PREFIX} failed."
...
if [[ $RET_STATUS != 0 ]]; then
	echo "" >> ${LOGNAME}
	echo "Shell script PSA_ASC.sh failed." >> ${LOGNAME}
...
# 1) Skip header; 2) Finder file has 4 fields
tail -n +2 ${DATADIR}${FINDER_FILE} | cut -d, -f4- | sort -u > ${DATADIR}tempPSAASC.txt  
...
#############################################################
echo "" >> ${LOGNAME}
echo "Start execution of PSA_ASC.py program"  >> ${LOGNAME}
...
${PYTHON_COMMAND} ${RUNDIR}PSA_ASC.py >> ${LOGNAME} 2>&1
...
if [[ $RET_STATUS != 0 ]]; then
	echo "" >> ${LOGNAME}
	echo "Python script PSA_ASC.py failed" >> ${LOGNAME}
	
	# Send Failure email	
	SUBJECT="PSA_ASC.sh  - Failed ${ENVNAME}"
	MSG="PSA ASC Extract has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${PSA_EMAIL_SENDER}" "${PSA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
echo "" >> ${LOGNAME}
echo "Python script PSA_ASC.py completed successfully. " >> ${LOGNAME}
...
SUBJECT="PSA ASC extract (${ENVNAME})" 
MSG="The Extract for the creation of the PSA ASC extract file from Snowflake has completed.\n\nThe following file(s) were created:\n\n${S3Files}"
...
if [[ $RET_STATUS != 0 ]]; then
		...
		# Send Failure email	
		SUBJECT="Sending Success email in PSA_ASC.sh  - Failed ${ENVNAME}"
		MSG="Sending Success email in PSA_ASC.sh  has failed."
		${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${PSA_EMAIL_SENDER}" "${PSA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#################################################################################
echo "" >> ${LOGNAME}
echo "Move processed S3 PSA ASC Finder File to archive directory in S3." >> ${LOGNAME}
...
#############################################################
echo "" >> ${LOGNAME} 
echo "Remove tempPSAASC.txt from data directory" >> ${LOGNAME} 

rm ${DATADIR}tempPSAASC.txt >> ${LOGNAME} 2>&1
rm ${DATADIR}${PSA_FF_ASC_PREFIX}*  >> ${LOGNAME} 2>&1
...
#############################################################
echo "" >> ${LOGNAME}
echo "PSA_ASC.sh completed successfully." >> ${LOGNAME}
