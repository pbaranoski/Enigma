#!/usr/bin/env python
########################################################################################################
# Name:  STS_PTA_BillsPymts_Rpts_Driver.py
#
# DESC:   This script extracts data for STS PTA Bills Payments by Type of Service report 
#         (legacy A-1 report)
#
# Execute as python3 STS_PTA_BillsPymts_Rpts_Driver.py     (processing without override date) 
# Execute as python3 STS_PTA_BillsPymts_Rpts_Driver.py $1  (processing with override date --> YYYY-MM-DD format) 
#
# 			$1 = From_dt (YYYY-MM-DD)
#
#   
# Paul Baranoski   2025-10-15 Create Module.
# Vijay Mandavilli 2026-05-28 Updated code with common functions and test logic.
########################################################################################################
...
def validate_dt(sDate2Validate):
    ...
    try:
        ...
        ## Send Failure email	
        SUBJECT=f"STS_PTA_BillsPymts_Rpts_Driver.py - Failed ({ENVNAME})"
...
def main_processing_loop():
    ...
    try:    
        ...
        LOGNAME = f"{LOG_DIR}{TESTLOG}STS_PTA_BillsPymts_Rpts_{TMSTMP}.log"
        ...
        rootLogger.info(f"\nSTS_PTA_BillsPymts_Rpts_Driver.py started at {TMSTMP}")
        ...
        #############################################################
        rootLogger.info("")
        rootLogger.info("Start execution of STS_PTA_BillsPymts_Rpts.py program")

        try:
            sp_info = subprocess.run(['python3', 'STS_PTA_BillsPymts_Rpts.py'], capture_output=True, text=True, check=True)
            write_sp_info_2_log(sp_info) 
            
        except subprocess.CalledProcessError as e:
            rootLogger.error(f"Calling STS_PTA_BillsPymts_Rpts.py failed with return code {e.returncode}")
            rootLogger.error("\n%s", e.output)
            
            ## Send Failure email	
            SUBJECT=f"STS PTA Bills Pymts Report - Failed ({ENVNAME})"
            MSG=f"Python script STS_PTA_BillsPymts_Rpts.py failed."            
        ...
        rootLogger.info("")
        rootLogger.info("Python script STS_PTA_BillsPymts_Rpts_Driver.py completed successfully.")
        ...
        # Send Success email	
        SUBJECT=f"STS PTA Bills Pymts Report - completed ({ENVNAME}{TESTEMAIL})"
        MSG=f"STS PTA Bills Pymts Report completed. \n\nThe following extract files were created:\n\n{S3Files}"
        
        try:
            sp_info = subprocess.run(['python3', 'sendEmail.py', CMS_EMAIL_SENDER, STS_PTA_BPYMTS_EMAIL_SUCCESS_RECIPIENT, SUBJECT, MSG], capture_output=True, text=True, check=True)
            write_sp_info_2_log(sp_info)
        ...
        #####################################################
        try:
            CreManDr.createManifestFile(bucket=XTR_BUCKET, s3folder=STS_PTA_BPYMTS_BUCKET_FLDR, runToken=TMSTMP, BoxEmails=STS_PTA_BPYMTS_BOX_RECIPIENT )

        except Exception as e:

            SUBJECT=f"Create Manifest file in STS_PTA_BillsPymts_Rpts_Driver.py - Failed ({ENVNAME})"
            MSG=f"Create Manifest file in STS_PTA_BillsPymts_Rpts_Driver.py  has failed."
        ...
        rootLogger.info("Script STS_PTA_BillsPymts_Rpts_Driver.py completed successfully.")
        rootLogger.info(f"\nEnded at {TMSTMP}" )


    except Exception as e:
        print (f"Exception occured in STS_PTA_BillsPymts_Rpts_Driver.py\n {e}")

        rootLogger.error("Exception occured in STS_PTA_BillsPymts_Rpts_Driver.py.")
        rootLogger.error("\n%s", str(e))

        ## Send Failure email	
        SUBJECT = f"Exception occured in STS_PTA_BillsPymts_Rpts_Driver.py - Failed ({ENVNAME}{TESTEMAIL})"
        MSG= f"Exception occured in STS_PTA_BillsPymts_Rpts_Driver.py {e}.  Process failed"
