...
try:
   ...
   #**************************************   
   snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_PSA_STG/PSA_OUTPATIENT_{YYYY}_{TMSTMP}.csv.gz
                                                FROM (
                    ...
                    SELECT *
                      FROM (
                           ...
                           SELECT BZ5.GEO_ZIP5_CD As Bene_ZIP_CD
                            --       ,APC.CAT_CD AS CAT_CD
                                   ,APC.CAT_DESC AS CAT_DESC
                                   ,SUM(CL.CLM_LINE_PRVDR_PMT_AMT + CL.CLM_LINE_BENE_PMT_AMT) AS PMT_AMT
                            ...
                            INNER JOIN BIA_{ENVNAME}.CMS_TARGET_XTR_{ENVNAME}.PSA_APC_CAT_FF  APC
                            ON APC.APC_CD = CLI.CLM_REV_APC_HIPPS_CD
                            
                            INNER JOIN IDRC_{ENVNAME}.CMS_DIM_GEO_{ENVNAME}.GEO_ZIP5_CD BZ5
                            ON C.GEO_BENE_SK=BZ5.GEO_SK
                            
                            WHERE C.CLM_FINL_ACTN_IND = 'Y' 
                            AND C.CLM_TYPE_CD = '40'
                            AND C.CLM_THRU_DT BETWEEN TO_DATE('{YYYY}-01-01','YYYY-MM-DD') AND TO_DATE('{YYYY}-12-31','YYYY-MM-DD') 
                            AND C.CLM_BILL_FAC_TYPE_CD||C.CLM_BILL_CLSFCTN_CD IN 
                                ('12','13','14','23','24','34','43','71','72','73','74','75','76','77','83','85')
                            -- Not necessary
                            --AND CLI.CLM_REV_APC_HIPPS_CD IS NOT NULL 
                            AND BZ5.GEO_ZIP5_CD IS NOT NULL  
                            
                            GROUP BY BZ5.GEO_ZIP5_CD, APC.CAT_DESC
                            HAVING SUM(CL.CLM_LINE_PRVDR_PMT_AMT + CL.CLM_LINE_BENE_PMT_AMT) > 0
                            --ORDER BY Bene_ZIP_CD, APC.CAT_DESC
                        
                        )

                    -- Example:
                    --PIVOT(MAX(CHRG_AMT) FOR CAT_DESC IN ('Addiction Medicine','Anesthesiology','Cardiac Surgery')) AS P
                    --(Bene_ZIP_CD, "Addiction Medicine", "Anesthesiology","Cardiac Surgery")    

                    PIVOT(MAX(PMT_AMT) FOR CAT_DESC IN ({PIV_COL_DESC_LIST})) AS P
