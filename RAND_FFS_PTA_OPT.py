#!/usr/bin/bash
############################################################################################################
# Name: RAND_FFS_PTA_OPT.py
# Desc: RAND FFS Part A Extract for OPT
#
...
def main_processing_loop():
    ...
    try:    
        ...
        LOGNAME = f"{LOG_DIR}{TESTLOG}RAND_FFS_PTA_OPT_{TMSTMP}.log"
        ...
        rootLogger.info(f"\nRAND_FFS_PTA_OPT.py started at {TMSTMP}")
        ...
        #############################################################
        rootLogger.info("")
        rootLogger.info("RAND FFS PTA OPT Processing will use the following parameters:")
        ...
        for i in range(len(ext_months)):
            ...
            for j in range(len(ext_months[i]["start_dates"])):
                ...
                XTR_FILE_NAME = f"RAND_FFS_OPT_Y{EXT_YYYY}_{MONTH_DESC}P{RNG}_{TMSTMP}.csv.gz"
                ...
                try:
                    ...
                    ## Send Failure email	
                    SUBJECT=f"RAND_FFS_PTA_OPT.py extract - Failed ({ENVNAME}{TESTEMAIL})"
                ...
                try:
                    ...
                    ## Send Failure email	
                    SUBJECT=f"Combining S3 files in RAND_FFS_PTA_OPT.py - Failed ({ENVNAME}{TESTEMAIL})"
                    MSG=f"Combining S3 files in RAND_FFS_PTA_OPT.py has failed."
        ...
        SUBJECT=f"RAND FFS PTA OPT Extract Complete ({ENVNAME}{TESTEMAIL})" 
        MSG=f"RAND FFS PTA OPT Extract completed successfully.\n\nThe following file(s) were created:\n\n{S3Files}"
        ...
        ######################################################################
        rootLogger.info("")
        rootLogger.info(f"Creating manifest files for RAND FFS PTA OPT Extract. ")
        ...
        try:
            ...
            SUBJECT=f"Create Manifest file in RAND_FFS_PTA_OPT.py - Failed ({ENVNAME}{TESTEMAIL})"
            MSG=f"Create Manifest file in RAND_FFS_PTA_OPT.py  has failed. {e}"
        ...
        rootLogger.info("RAND FFS PTA OPT Extract completed successfully.")
        rootLogger.info(f"\nEnded at {TMSTMP}" )


    except Exception as e:
        print (f"Exception occured in RAND_FFS_PTA_OPT.py\n {e}")

        rootLogger.error("Exception occured in RAND_FFS_PTA_OPT.py.")
        rootLogger.error("\n%s", str(e))

        ## Send Failure email	
        SUBJECT=f"RAND_FFS_PTA_OPT.py  - Failed ({ENVNAME}{TESTEMAIL})"
        MSG=f"Exception occured in RAND_FFS_PTA_OPT.py {e}. Process failed. "
