#!/usr/bin/bash
############################################################################################################
# Name: SAFENC_DME_Extract.sh
#
# Desc: SAF ENC DME Extract
#
#
#  
#
...
#
# Modified:
# Viren Khanna 	 2023-06-13 New script.
...
LOGNAME=/app/IDRC/XTR/CMS/logs/SAFENC_DME_Extract_${TMSTMP}.log
...
echo "################################### " >> ${LOGNAME}
echo "SAFENC_DME_Extract.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
S3BUCKET=${SAFENC_DME_BUCKET} 
echo "SAF ENC DME bucket=${S3BUCKET}" >> ${LOGNAME}
...
aws s3 rm s3://${S3BUCKET} --recursive --exclude "*" --include "SAFENC_DME_*"  2>> ${LOGNAME}
...
SAF_ENC_PARM_FILE=SAF_ENC_DME_PARM_FILE.txt
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="SAF Encounter DME Extract - Failed (${ENVNAME})"
...
#################################################################################
# Find SAF ENC DME Extract Years Parameter file in data directory.
#################################################################################
ParmFile2Process=`ls ${DATADIR}${SAF_ENC_PARM_FILE}` 1>> ${LOGNAME}  2>&1

echo "" >> ${LOGNAME}
echo "SAF Encounter DME Extract Parameter file on linux: ${ParmFile2Process}" >> ${LOGNAME}



#################################################################################
# Loop thru SAF Encounter DME Date Ranges in parameter file.
# 
...
do
		...
		export YYYY
                ...
                #############################################################
		echo "" >> ${LOGNAME}
		echo "Start execution of SAFENC_DME_Extract.py program"  >> ${LOGNAME}
		${PYTHON_COMMAND} ${RUNDIR}SAFENC_DME_Extract.py >> ${LOGNAME} 2>&1
		...
		if [[ $RET_STATUS != 0 ]]; then
				echo "" >> ${LOGNAME}
				echo "Python script SAFENC_DME_Extract.py failed" >> ${LOGNAME}
				
				# Send Failure email	
				SUBJECT="SAFENC_DME_Extract.sh - Failed (${ENVNAME})"
				MSG="SAF ENC DME extract has failed."
				${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SAFENC_EMAIL_SENDERR}" "${SAFENC_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1

				exit 12
		fi

		echo "" >> ${LOGNAME}
		echo "Python script SAFENC_DME_Extract.py completed successfully. " >> ${LOGNAME}
		...
		concatFilename=SAFENC_DME_${STUS_LIT}_${RNG_LIT}_${TMSTMP}.txt.gz
		...
		if [[ $RET_STATUS != 0 ]]; then
				...
				# Send Failure email	
				SUBJECT="Combining S3 files in SAF_ENC_DME_Extract.sh - Failed (${ENVNAME})"
				MSG="Combining S3 files in SAF_ENC_DME_Extract.sh has failed."
				${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SAFENC_EMAIL_SENDER}" "${SAFENC_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
SUBJECT="SAF ENC DME extract (${ENVNAME})" 
MSG="The SAF ENC DME extract from Snowflake has completed.\n\nThe following file(s) were created:\n\n${S3Files}"
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="Sending Success email in SAFENC_DME_Extract.sh - Failed (${ENVNAME})"
	MSG="Sending Success email in SAFENC_DME_Extract.sh  has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SAFENC_EMAIL_SENDER}" "${SAFENC_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#############################################################
echo " " >> ${LOGNAME}
echo "EFT SAF Encounter DME Extract Files " >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="SAF Encounter DME EFT process - Failed (${ENVNAME})"
	MSG="SAF Encounter DME EFT process has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SAFENC_EMAIL_SENDER}" "${SAFENC_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#############################################################
echo "" >> ${LOGNAME}
echo "SAFENC_DME_Extract.sh completed successfully." >> ${LOGNAME}
