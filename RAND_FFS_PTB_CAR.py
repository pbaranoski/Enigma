#!/usr/bin/bash
############################################################################################################
# Name: RAND_FFS_PTB_CAR.py
# Desc: RAND FFS Part A Extract for CAR
#
...
def main_processing_loop():
    ...
    try:    
        ...
        LOGNAME = f"{LOG_DIR}{TESTLOG}RAND_FFS_PTB_CAR_{TMSTMP}.log"
        ...
        rootLogger.info(f"\nRAND_FFS_PTB_CAR.py started at {TMSTMP}")
        ...
        #############################################################
        rootLogger.info("")
        rootLogger.info("RAND FFS PTB CAR Processing will use the following parameters:")
        ...
        #############################################################
        rootLogger.info("")
        rootLogger.info("Loop thru date ranges for extract files")
                
        ext_months = [
            {"ext_month":"JAN",
             "start_dates": ["-01-01","-01-08","-01-15", "-01-22"],
             "end_date": ["-01-07","-01-14","-01-21", "-01-31"]
            },
            {"ext_month":"FEB",
             "start_dates": ["-02-01","-02-08","-02-15", "-02-22"],
             "end_date": ["-02-07","-02-14","-02-21", "-02-28"]
            },
            {"ext_month":"MAR",
             "start_dates": ["-03-01","-03-08","-03-15", "-03-22"],
             "end_date": ["-03-07","-03-14","-03-21", "-03-31"]
            },
            {"ext_month":"APR",
             "start_dates": ["-04-01","-04-08","-04-15", "-04-22"],
             "end_date": ["-04-07","-04-14","-04-21", "-04-30"]
            },
            {"ext_month":"MAY",
             "start_dates": ["-05-01","-05-08","-05-15", "-05-22"],
             "end_date": ["-05-07","-05-14","-05-21", "-05-31"]
            },
            {"ext_month":"JUN",
             "start_dates": ["-06-01","-06-08","-06-15", "-06-22"],
             "end_date": ["-06-07","-06-14","-06-21", "-06-30"]
            },
            {"ext_month":"JUL",
             "start_dates": ["-07-01","-07-08","-07-15", "-07-22"],
             "end_date": ["-07-07","-07-14","-07-21", "-07-31"]
            },
            {"ext_month":"AUG",
             "start_dates": ["-08-01","-08-08","-08-15", "-08-22"],
             "end_date": ["-08-07","-08-14","-08-21", "-08-31"]
            },
            {"ext_month":"SEP",
             "start_dates": ["-09-01","-09-08","-09-15", "-09-22"],
             "end_date": ["-09-07","-09-14","-09-21", "-09-30"]
            },
            {"ext_month":"OCT",
             "start_dates": ["-10-01","-10-08","-10-15", "-10-22"],
             "end_date": ["-10-07","-10-14","-10-21", "-10-31"]
            },
            {"ext_month":"NOV",
             "start_dates": ["-11-01","-11-08","-11-15", "-11-22"],
             "end_date": ["-11-07","-11-14","-11-21", "-11-30"]
            },
            {"ext_month":"DEC",
             "start_dates": ["-12-01","-12-08","-12-15", "-12-22"],
             "end_date": ["-12-07","-12-14","-12-21", "-12-31"]
            }  
        ]
        ...
        for i in range(len(ext_months)):
            ...
            for j in range(len(ext_months[i]["start_dates"])):
                ...
                XTR_FILE_NAME = f"RAND_FFS_CAR_Y{EXT_YYYY}_{MONTH_DESC}P{RNG}_{TMSTMP}.csv.gz"
                ...
                #############################################################
                rootLogger.info("")
                rootLogger.info("Start execution of RAND_FFS_PartB_Extract_v2.py program")

                try:
                    sp_info = subprocess.run(['python3', 'RAND_FFS_PartB_Extract_v2.py'], capture_output=True, text=True, check=True)
                    write_sp_info_2_log(sp_info) 
                    
                except subprocess.CalledProcessError as e:
                    rootLogger.error(f"Calling RAND_FFS_PartB_Extract_v2.py failed with return code {e.returncode}")
                    rootLogger.error(e.output)
                    
                    ## Send Failure email	
                    SUBJECT=f"RAND_FFS_PTB_CAR.py extract - Failed ({ENVNAME}{TESTEMAIL})"
                    MSG=f"Calling RAND_FFS_PartB_Extract_v2.py failed with return code {e} "
                ...
                # call was successful
                rootLogger.info("Python script RAND_FFS_PartB_Extract_v2.py completed successfully. ")
                ...
                try:
                    ...
                    ## Send Failure email	
                    SUBJECT=f"Combining S3 files in RAND_FFS_PTB_CAR.py - Failed ({ENVNAME}{TESTEMAIL})"
                    MSG=f"Combining S3 files in RAND_FFS_PTB_CAR.py has failed."
        ...
        SUBJECT=f"RAND FFS PTB CAR Extract Complete ({ENVNAME}{TESTEMAIL})" 
        MSG=f"RAND FFS PTB CAR Extract completed successfully.\n\nThe following file(s) were created:\n\n{S3Files}"
        ...
        ######################################################################
        rootLogger.info("")
        rootLogger.info(f"Creating manifest files for RAND FFS PTB CAR Extract. ")
        ...
        try:
            ...
            SUBJECT=f"Create Manifest file in RAND_FFS_PTB_CAR.py - Failed ({ENVNAME}{TESTEMAIL})"
            MSG=f"Create Manifest file in RAND_FFS_PTB_CAR.py  has failed. {e}"
        ...
        rootLogger.info("RAND FFS PTB CAR Extract completed successfully.")
        rootLogger.info(f"\nEnded at {TMSTMP}" )


    except Exception as e:
        print (f"Exception occured in RAND_FFS_PTB_CAR.py\n {e}")

        rootLogger.error("Exception occured in RAND_FFS_PTB_CAR.py.")
        rootLogger.error("\n%s", str(e))

        ## Send Failure email	
        SUBJECT=f"RAND_FFS_PTB_CAR.py  - Failed ({ENVNAME}{TESTEMAIL})"
        MSG=f"Exception occured in RAND_FFS_PTB_CAR.py {e}. Process failed. "
