#!/usr/bin/env python
########################################################################################################
# Name:  RAND_FFS_PartB_Extract.py
# DESC:  This python script extracts data from IDRC for the RAND FFS Part B (CAR, DME) extracts
#
...
def execute_partb_extract():
    ...
    try:
        ...
        ########################################################################################################
        # Extract RAND Part B data and write to S3 as a flat file
        ########################################################################################################
        snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_RANDFFSPTAB_STG/{XTR_FILE_NAME}
                ...
                SELECT DISTINCT
                    ...
                    RPAD(COALESCE(B.BENE_EQTBL_BIC_HICN_NUM,''),11,' ') AS BENE_EQTBL_BIC_HICN_NUM,
                    'END' AS END_OF_FILE
...
########################################################################################################
# RUN
# For CTYP CAR, each month has 4 ranges (1 for each week of the month). 
#     Loop through the list of objects to establish date parameters and call the extract function
# For CTYP DME, ranges are established as bi-annual
########################################################################################################
bErrorOccurred = False
FILE_LIST = open(f"{DATADIR}temp_RAND_PARTB_Files_{TMSTMP}.txt", "a")
print('')
