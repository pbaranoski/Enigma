#!/usr/bin/bash
############################################################################################################
# Name: SRTR_FFS_PTB_DME.sh
#
# Desc: SRTR FFS Part B Extract for DME
#
...
LOGNAME=/app/IDRC/XTR/CMS/logs/SRTR_FFS_PTB_DME_${TMSTMP}.log
touch ${LOGNAME}
...
echo "################################### " >> ${LOGNAME}
echo "SRTR_FFS_PTB_DME.sh started at ${TMSTMP} " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="SRTR FFS Part B DME Extract - Failed"
...
do
	...
	if [[ $RET_STATUS != 0 ]]; then
		...
		# Send Failure email	
		SUBJECT="SRTR_FFS_PTB_DME.sh  - Failed"
		MSG="SRTR FFS Part B DME extract has failed in extract step."
		${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_FFS_EMAIL_SENDER}" "${SRTR_FFS_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
		exit 12
	fi

	echo "" >> ${LOGNAME}
	echo "Python script SRTR_FFS_PartA_Extract.py completed successfully for DME. " >> ${LOGNAME}
done
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="SRTR_FFS_PTB_DME.sh  - Failed"
	MSG="EFT SRTR FFS Part B DME Extract Files process has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_FFS_EMAIL_SENDER}" "${SRTR_FFS_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
SUBJECT="SRTR FFS Part B DME Extract" 
MSG="The SRTR FFS Part B DME extract from Snowflake has completed.\n\nThe following file(s) were created:\n\n${S3Files}"
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="Sending Success email in SRTR_FFS_PTB_DME.sh - Failed"
	MSG="Sending Success email in SRTR_FFS_PTB_DME.sh  has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_FFS_EMAIL_SENDER}" "${SRTR_FFS_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
###########################################################################################
echo "" >> ${LOGNAME}
echo "SRTR_FFS_PTB_DME.sh completed successfully." >> ${LOGNAME}
