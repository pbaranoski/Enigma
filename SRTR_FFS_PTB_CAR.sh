#!/usr/bin/bash
############################################################################################################
# Name: SRTR_FFS_PTB_CAR.sh
#
# Desc: SRTR FFS Part B Extract for CAR
#
...
# Joshua Turner         2023-05-17      Added EFT functionality and updated to use getExtractFilenamesAndCounts script
############################################################################################################
set +x
...
LOGNAME=/app/IDRC/XTR/CMS/logs/SRTR_FFS_PTB_CAR_${TMSTMP}.log
touch ${LOGNAME}
...
echo "################################### " >> ${LOGNAME}
echo "SRTR_FFS_PTB_CAR.sh started at ${TMSTMP} " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="SRTR FFS Part B CAR Extract - Failed"
...
###########################################################################################
# Loop through the years parameter file and call SRTR_FFS_PartB_Extract.py for each 
...
do
	...
	#############################################################
	echo "" >> ${LOGNAME}
	echo "Start execution of SRTR_FFS_PartB_Extract.py program"  >> ${LOGNAME}
	${PYTHON_COMMAND} ${RUNDIR}SRTR_FFS_PartB_Extract.py >> ${LOGNAME} 2>&1
	...
	if [[ $RET_STATUS != 0 ]]; then
		echo "" >> ${LOGNAME}
		echo "Python script SRTR_FFS_PartB_Extract.py failed" >> ${LOGNAME}
		
		# Send Failure email	
		SUBJECT="SRTR_FFS_PTB_CAR.sh  - Failed"
		MSG="SRTR FFS Part B CAR extract has failed in extract step."
		${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_FFS_EMAIL_SENDER}" "${SRTR_FFS_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
		exit 12
	fi

	echo "" >> ${LOGNAME}
	echo "Python script SRTR_FFS_PartA_Extract.py completed successfully for CAR. " >> ${LOGNAME}
done
...
###########################################################################################
echo " " >> ${LOGNAME}
echo "EFT SRTR FFS Part B DME Extract Files " >> ${LOGNAME}
...
if [ $RET_STATUS != 0 ]; then
	...
	# Send Failure email	
	SUBJECT="Get S3 Extract file list in SRTR_FFS_PTB_CAR.sh - Failed"
	MSG="S3 Extract file list in SRTR_FFS_PTB_CAR.sh has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_FFS_EMAIL_SENDER}" "${SRTR_FFS_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
SUBJECT="SRTR FFS Part B CAR Extract" 
MSG="The SRTR FFS Part B CAR extract from Snowflake has completed.\n\nThe following file(s) were created:\n\n${S3Files}"
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="Sending Success email in SRTR_FFS_PTB_CAR.sh - Failed"
	MSG="Sending Success email in SRTR_FFS_PTB_CAR.sh  has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_FFS_EMAIL_SENDER}" "${SRTR_FFS_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
###########################################################################################
echo "" >> ${LOGNAME}
echo "SRTR_FFS_PTB_CAR.sh completed successfully." >> ${LOGNAME}
