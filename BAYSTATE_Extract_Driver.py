#!/usr/bin/env python
########################################################################################################
# Name: BAYSTATE_Extract_Driver.py
# DESC:   This script executes the full SSN extraction python script for MEDPAR Baystate
#
# Vijay Mandavilli   2026-01-21 Create Module.
...
def main_processing_loop():
    ...
    try:    
        ...
        LOGNAME = f"{LOG_DIR}{TESTLOG}BAYSTATE_Extract_Driver_{TMSTMP}.log"
        ...
        rootLogger.info(f"\nBAYSTATE_Extract_Driver.py started at {TMSTMP}")
        ...
        ###########################################
        # Section 1: BAYSTATE_EXTRACT_Driver.py logic
        ############################################    

        rootLogger.info("--- Starting BAYSTATE_Extract_Driver.py logic ---")
        ...
        #############################################################
        # Execute Python code to produce Baystate SSN extract
        #############################################################
        rootLogger.info("")
        rootLogger.info("Start execution of BAYSTATE_Extract.py program")

        try:
            sp_info = subprocess.run(['python3', 'BAYSTATE_Extract.py'], capture_output=True, text=True, check=True)
            write_sp_info_2_log(sp_info) 
            
        except subprocess.CalledProcessError as e:
            rootLogger.error(f"Calling Python script BAYSTATE_Extract.py failed with return code {e.returncode}")
            rootLogger.error("\n%s", e.output)
            
            ## Send Failure email	
            SUBJECT=f"BAYSTATE_Extract_Driver.py  - Failed ({ENVNAME})"
            MSG=f"MEDPBAR Baystate SSN extract has failed."
        ...
        rootLogger.info("")
        rootLogger.info("Python script BAYSTATE_Extract.py completed successfully.")
        ...
        # Send Success email	
        SUBJECT=f"BAYSTATE_Extract_Driver.py  - Completed ({ENVNAME}{TESTEMAIL})"
        MSG=f"MEDPAR Baystate SSN Extract has completed successfully.\n\nThe following file(s) were created:\n\n{S3Files}"
        ...
        #############################################################
        rootLogger.info("")
        rootLogger.info("EFT MEDPAR Baystate SSN Extract File")
        ...
        try:
            ...
            ## Send Failure email	
            SUBJECT = f"MEDPAR BAYSTATE Extract SSN EFT process  - Failed ({ENVNAME})"
            MSG= f"MEDPAR BAYSTATE Extract SSN EFT process has failed."
        ...
        rootLogger.info("Script BAYSTATE_Extract_Driver.py completed successfully.")
        rootLogger.info(f"\nEnded at {datetime.now().strftime('%Y%m%d.%H%M%S')}" )


    except Exception as e:
        print (f"Exception occured in BAYSTATE_Extract_Driver.py\n {e}")

        rootLogger.error("Exception occured in BAYSTATE_Extract_Driver.py.")
        rootLogger.error("\n%s", str(e))
