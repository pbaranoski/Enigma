...
MANIFEST_EMAIL_SENDER=${EFT_EMAIL_SENDER}
MANIFEST_EMAIL_SUCCESS_RECIPIENT=${EFT_EMAIL_SUCCESS_RECIPIENT}
MANIFEST_EMAIL_FAILURE_RECIPIENT=${EFT_EMAIL_FAILURE_RECIPIENT}
...
#############################################################
# Get S3 folder name (w/o Path) 
#
# NonProdFldr value = 1 or 0 Ex. xtr/DEV/Blbtn --> 1  xtr/Blbtn --> 0   
#
# if nonprod folder = 1 --> get first 3 tokens (bucket/xtr/DEV) 
# else                  --> get first 2 tokens (bucket/xtr)
#############################################################
NonProdFldr=`echo ${S3Folder} | awk '/DEV|TST|IMPL/{print}' | wc -l` 2>> ${LOGNAME}
echo "NonProdFldr=${NonProdFldr}" >> ${LOGNAME}

if [ "${ExtractTypeOverride}" = "" ];then
	if [ ${NonProdFldr} -eq 1 ]; then
		S3XTRFolder=`echo ${S3BucketAndFldr} | cut -d/ -f1-3 ` 2>> ${LOGNAME}
		ExtractType=`echo ${S3BucketAndFldr} | cut -d/ -f4 ` 2>> ${LOGNAME}
		#ExtractType=`echo ${S3BucketAndFldr} | awk -F/ '{print $NF}'  `
	 else
		S3XTRFolder=`echo ${S3BucketAndFldr} | cut -d/ -f1-2 `   2>> ${LOGNAME}
		ExtractType=`echo ${S3BucketAndFldr} | cut -d/ -f3 ` 2>> ${LOGNAME}	
	fi
else
...
#echo "S3XTRFolder=${S3XTRFolder}" >> ${LOGNAME}
...
if [[ $RET_STATUS = 0 ]]; then
	...
	if [[ $RET_STATUS != 0 ]]; then
		...
		#${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${MANIFEST_EMAIL_SENDER}" "${MANIFEST_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#############################################################
echo "" >> ${LOGNAME}
${PYTHON_COMMAND} ${RUNDIR}CreateManifestFile.py --bucket ${S3Bucket} --folder ${S3Folder} --files ${S3Files} --REmails ${RecipientEmails} --outfile ${ManifestPathAndFilename} --jiraURL ${jiraURL}  >> ${LOGNAME} 2>&1
...
if [ "${HoldManifestFile}" = "Y" ];then
	aws s3 cp ${DATADIR}${ManifestFilename} s3://${MANIFEST_HOLD_BUCKET}${ManifestFilename}  1>> ${LOGNAME} 2>&1
else
