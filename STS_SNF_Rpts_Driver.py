#!/usr/bin/env python
########################################################################################################
# Name:  STS_SNF_Rpts_Driver.py
#
# DESC:   This script extracts data for STS HHA table report - HHA Rev Cntr UNITS/CHRGS	  
#         by period expense (legacy AA5 report)
#
# Execute as python3 STS_SNF_Rpts_Driver.py     (processing without override date) 
# Execute as python3 STS_SNF_Rpts_Driver.py $1  (processing with override date --> YYYY-MM-DD format) 
#
...
def validate_dt(sDate2Validate):
    ...
    try:
        ...
        ## Send Failure email	
        SUBJECT=f"STS_SNF_Rpts_Driver.py - Failed ({ENVNAME})"
...
def main_processing_loop():
    ...
    try:    
        ...
        LOGNAME = f"{LOG_DIR}STS_SNF_RPTS_{TMSTMP}.log"
        ...
        rootLogger.info(f"\nSTS_SNF_Rpts_Driver.py started at {TMSTMP}")
        ...
        #############################################################
        rootLogger.info("")
        rootLogger.info("Start execution of STS_SNF_RPT.py program")

        try:
            sp_info = subprocess.run(['python3', 'STS_SNF_RPT.py'], capture_output=True, text=True, check=True)
            write_sp_info_2_log(sp_info) 
            
        except subprocess.CalledProcessError as e:
            rootLogger.error(f"Calling STS_SNF_RPT.py failed with return code {e.returncode}")
            rootLogger.error("\n%s", e.output)
            
            ## Send Failure email	
            SUBJECT=f"STS SNF Report - Failed ({ENVNAME})"
            MSG=f"Python script STS_SNF_RPT.py failed."
        ...
        rootLogger.info("")
        rootLogger.info("Python script STS_SNF_Rpts_Driver.py completed successfully.")
        ...
        # Send Success email	
        SUBJECT=f"STS SNF Report - completed ({ENVNAME})"
        MSG=f"STS SNF Report completed. \n\nThe following extract files were created:\n\n{S3Files}"
        
        try:
            sp_info = subprocess.run(['python3', 'sendEmail.py', CMS_EMAIL_SENDER, STS_SNF_EMAIL_SUCCESS_RECIPIENT, SUBJECT, MSG], capture_output=True, text=True, check=True)
            write_sp_info_2_log(sp_info)
        ...
        #####################################################
        try:
            CreManDr.createManifestFile(bucket=XTR_BUCKET, s3folder=STS_SNF_BUCKET_FLDR, runToken=TMSTMP, BoxEmails=STS_SNF_BOX_RECIPIENT )

        except Exception as e:

            SUBJECT=f"Create Manifest file in STS_SNF_Rpts_Driver.py - Failed ({ENVNAME})"
            MSG=f"Create Manifest file in STS_SNF_Rpts_Driver.py  has failed."
        ...
        rootLogger.info("Script STS_SNF_Rpts_Driver.py completed successfully.")
        rootLogger.info(f"\nEnded at {TMSTMP}" )


    except Exception as e:
        print (f"Exception occured in STS_SNF_Rpts_Driver.py\n {e}")

        rootLogger.error("Exception occured in STS_SNF_Rpts_Driver.py.")
        rootLogger.error("\n%s", str(e))
