...
# Paul Baranoski 2025-09-22 Change Key column names.
...
PSPS_SRVC_YR=os.getenv('PSPS_SRVC_YR')
PSPS_SRVC_QTR=os.getenv('PSPS_SRVC_QTR')
...
try:
   ...
   snowconvert_helpers.execute_sql_statement(f"""DELETE FROM BIA_{ENVNAME}.CMS_EXTNL_XTR_{ENVNAME}.PSPS_EXT_DATA 
                                                 WHERE SRVC_YR = '{PSPS_SRVC_YR}' 
                                                   AND SRVC_QTR     = '{PSPS_SRVC_QTR}'    
   ...
   snowconvert_helpers.execute_sql_statement(f""" COPY INTO BIA_{ENVNAME}.CMS_EXTNL_XTR_{ENVNAME}.PSPS_EXT_DATA
                                                ...
                                                FROM (SELECT '{PSPS_SRVC_YR}' AS SRVC_YR,
                                                            ,'{PSPS_SRVC_QTR}'     AS SRVC_QTR
