...
try:
   ...
   snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_SAFENC{CLM_TYPE_LIT}_STG/SAFENC_{CLM_TYPE_LIT}_{STUS_LIT}_{RNG_LIT}_{TMSTMP}.txt.gz
    ...
    ,TMP_RLT_OCRNC_SGNTR_MBR_OPT_TMP AS (
    ...
    SELECT DISTINCT
        ...
        ,CASE WHEN C.GEO_BLG_SSA_STATE_CD = '~' OR C.GEO_BLG_SSA_STATE_CD IS NULL 
              THEN ( CASE WHEN COALESCE(C.GEO_FAC_SSA_STATE_CD,' ') = '~' 	
                          THEN '  ' 
                          ELSE COALESCE(C.GEO_FAC_SSA_STATE_CD,'  ') END ) 
              ELSE C.GEO_BLG_SSA_STATE_CD 
        END AS  GEO_BLG_SSA_STATE_CD
        ...
        ,CASE WHEN PROD.CLM_DGNS_PRCDR_ICD_IND = '~'  
              THEN ' '
              ELSE RPAD(COALESCE(PROD.CLM_DGNS_PRCDR_ICD_IND,' '),1,' ') END  AS  CLM_DGNS_PRCDR_ICD_IND

        -- (FORMAT '9(13)V9(2)S') (CHAR(15))
        ,REPLACE(TO_CHAR(CASE WHEN CEOPD.CLM_OTHR_PYR_RSPNSBLTY_SQNC_CD = 'P'
                              THEN COALESCE(CEOPD.CLM_ENCTR_OTHR_PYR_PD_AMT,0)  
                              ELSE 0 END,'000000000000.00S'),'.','')  AS CLM_ENCTR_OTHR_PYR_PD_AMT 
        ...
        ,RPAD(CASE WHEN C.CLM_FAC_PRVDR_NPI_NUM = '~' THEN ' ' ELSE COALESCE(C.CLM_FAC_PRVDR_NPI_NUM,' ') END,10,' ')  AS CLM_FAC_PRVDR_NPI_NUM
        ...
        ,RPAD(COALESCE(POVT.VAL_CD_1, ' '),2,' ')  AS  VAL_CD_1
        ,RPAD(COALESCE(POVT.VAL_CD_2, ' '),2,' ')  AS  VAL_CD_2
        ,RPAD(COALESCE(POVT.VAL_CD_3, ' '),2,' ')  AS  VAL_CD_3
        ,RPAD(COALESCE(POVT.VAL_CD_4, ' '),2,' ')  AS  VAL_CD_4
        ,RPAD(COALESCE(POVT.VAL_CD_5, ' '),2,' ')  AS  VAL_CD_5
        ,RPAD(COALESCE(POVT.VAL_CD_6, ' '),2,' ')  AS  VAL_CD_6
        ,RPAD(COALESCE(POVT.VAL_CD_7, ' '),2,' ')  AS  VAL_CD_7
        ,RPAD(COALESCE(POVT.VAL_CD_8, ' '),2,' ')  AS  VAL_CD_8
        ,RPAD(COALESCE(POVT.VAL_CD_9, ' '),2,' ')  AS  VAL_CD_9
        ,RPAD(COALESCE(POVT.VAL_CD_10, ' '),2,' ') AS  VAL_CD_10
        ,RPAD(COALESCE(POVT.VAL_CD_11, ' '),2,' ') AS  VAL_CD_11
        ,RPAD(COALESCE(POVT.VAL_CD_12, ' '),2,' ') AS  VAL_CD_12
        ,RPAD(COALESCE(POVT.VAL_CD_13, ' '),2,' ') AS  VAL_CD_13
        ,RPAD(COALESCE(POVT.VAL_CD_14, ' '),2,' ') AS  VAL_CD_14
        ,RPAD(COALESCE(POVT.VAL_CD_15, ' '),2,' ') AS  VAL_CD_15
        ,RPAD(COALESCE(POVT.VAL_CD_16, ' '),2,' ') AS  VAL_CD_16
        ,RPAD(COALESCE(POVT.VAL_CD_17, ' '),2,' ') AS  VAL_CD_17
        ,RPAD(COALESCE(POVT.VAL_CD_18, ' '),2,' ') AS  VAL_CD_18
        ,RPAD(COALESCE(POVT.VAL_CD_19, ' '),2,' ') AS  VAL_CD_19
        ,RPAD(COALESCE(POVT.VAL_CD_20, ' '),2,' ') AS  VAL_CD_20
        ,RPAD(COALESCE(POVT.VAL_CD_21, ' '),2,' ') AS  VAL_CD_21
        ,RPAD(COALESCE(POVT.VAL_CD_22, ' '),2,' ') AS  VAL_CD_22
        ,RPAD(COALESCE(POVT.VAL_CD_23, ' '),2,' ') AS  VAL_CD_23
        ,RPAD(COALESCE(POVT.VAL_CD_24, ' '),2,' ') AS  VAL_CD_24
        ,RPAD(COALESCE(POVT.VAL_CD_25, ' '),2,' ') AS  VAL_CD_25

        -- WARNING!! (FORMAT '9(13)V9(2)S') (CHAR(15)) Truncation of sign
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_1,0), '000000000000.00S'),'.','')  AS  VAL_AMT_1 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_2,0), '000000000000.00S'),'.','')  AS  VAL_AMT_2 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_3,0), '000000000000.00S'),'.','')  AS  VAL_AMT_3 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_4,0), '000000000000.00S'),'.','')  AS  VAL_AMT_4 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_5,0), '000000000000.00S'),'.','')  AS  VAL_AMT_5 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_6,0), '000000000000.00S'),'.','')  AS  VAL_AMT_6 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_7,0), '000000000000.00S'),'.','')  AS  VAL_AMT_7 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_8,0), '000000000000.00S'),'.','')  AS  VAL_AMT_8 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_9,0), '000000000000.00S'),'.','')  AS  VAL_AMT_9 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_10,0),'000000000000.00S'),'.','') AS  VAL_AMT_10 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_11,0),'000000000000.00S'),'.','') AS  VAL_AMT_11 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_12,0),'000000000000.00S'),'.','') AS  VAL_AMT_12 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_13,0),'000000000000.00S'),'.','') AS  VAL_AMT_13 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_14,0),'000000000000.00S'),'.','') AS  VAL_AMT_14 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_15,0),'000000000000.00S'),'.','') AS  VAL_AMT_15 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_16,0),'000000000000.00S'),'.','') AS  VAL_AMT_16 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_17,0),'000000000000.00S'),'.','') AS  VAL_AMT_17 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_18,0),'000000000000.00S'),'.','') AS  VAL_AMT_18 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_19,0),'000000000000.00S'),'.','') AS  VAL_AMT_19 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_20,0),'000000000000.00S'),'.','') AS  VAL_AMT_20 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_21,0),'000000000000.00S'),'.','') AS  VAL_AMT_21 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_22,0),'000000000000.00S'),'.','') AS  VAL_AMT_22 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_23,0),'000000000000.00S'),'.','') AS  VAL_AMT_23 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_24,0),'000000000000.00S'),'.','') AS  VAL_AMT_24 
        ,REPLACE(TO_CHAR(COALESCE(POVT.VAL_AMT_25,0),'000000000000.00S'),'.','') AS  VAL_AMT_25 
    ...
    LEFT OUTER JOIN TMP_DGNS_E_CD_OPT DECIT
    ...
    LEFT OUTER JOIN TMP_OCRNC_SGNTR_MBR_OPT COSM
    ON COSM.CLM_OCRNC_SGNTR_SK  = C.CLM_OCRNC_SGNTR_SK

    LEFT OUTER JOIN TMP_RLT_OCRNC_SGNTR_MBR_OPT_TMP CROSM
    ON CROSM.CLM_RLT_OCRNC_SGNTR_SK = C.CLM_RLT_OCRNC_SGNTR_SK

    LEFT OUTER JOIN TMP_RLT_COND_SGNTR_MBR_OPT CRCSM
    ON C.CLM_RLT_COND_SGNTR_SK  = CRCSM.CLM_RLT_COND_SGNTR_SK

    LEFT OUTER JOIN TMP_PROD_OPT_VAL POVT
    ON  C.GEO_BENE_SK      = POVT.GEO_BENE_SK
    AND C.CLM_DT_SGNTR_SK  = POVT.CLM_DT_SGNTR_SK
    AND C.CLM_TYPE_CD      = POVT.CLM_TYPE_CD
    AND C.CLM_NUM_SK       = POVT.CLM_NUM_SK

    LEFT OUTER JOIN TMP_OPT_ADR ADRTEMP
