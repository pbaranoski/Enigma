#!/usr/bin/bash
############################################################################################################
# Script Name: OPMHI_HHA_Driver.sh
...
#
#
# 10/02/2023   Paul Baranoski       Modified extract to use BOX delivery. Added logic to build
...
CUR_DT=`date +%Y%m%d`
LOGNAME=/app/IDRC/XTR/CMS/logs/${TESTLOG}OPMHI_HHA_Driver_${TMSTMP}.log
...
echo "################################### " >> ${LOGNAME}
echo "OPMHI_HHA_Driver.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}

#################################################################################
# THIS ONE SCRIPT SETS ALL DATABASE NAMES VARIABLES 
#################################################################################
S3BUCKET=${OPMHI_HHA_BUCKET}

#################################################################################
# Clean linux data directory of any residual files
#################################################################################
echo "" >> ${LOGNAME}
echo "Remove any residual FEHB_CMS_HHA_* files on linux data directory" >> ${LOGNAME}
rm ${DATADIR}FEHB_CMS_HHA_*   2>> ${LOGNAME}
...
echo "" >> ${LOGNAME}
echo "OPM-HI HHA Extract is starting with the following parameters:" >> ${LOGNAME}
...
#################################################################################
# Execute Python code to produce HHA file
#################################################################################
echo "" >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
        ...
        # Send Failure email	
        SUBJECT="OPMHI_HHA_Driver.sh - Failed (${ENVNAME})"
        MSG="OPM-HI HHA extract has failed."
...
if [ $RET_STATUS != 0 ]; then
         ...
         # Send Failure email	
         SUBJECT="OPMHI_HHA_Driver.sh - Failed (${ENVNAME})"
         MSG="CombineS3Files.sh for OPM-HI PTA HHA has failed."
         ${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
SUBJECT="OPMHI_HHA_Driver.sh  - Completed (${ENVNAME}${TESTEMAIL})"
MSG="OPM-HI PTA HHA has completed successfully.\n\nThe following file(s) were created:\n\n${S3Files}"
...
if [[ $RET_STATUS != 0 ]]; then
		...
		# Send Failure email	
		SUBJECT="Sending Success email in OPMHI_HHA_Driver.sh - Failed (${ENVNAME})"
		MSG="Sending Success email in OPMHI_HHA_Driver.sh  has failed."
		${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${CMS_EMAIL_SENDER}" "${ENIGMA_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#################################################################################
echo "" >> ${LOGNAME}
echo "OPMHI_HHA_Driver.sh completed successfully." >> ${LOGNAME}
