...
#
# Created: Paul Baranoski 9/19/2024
# Modified: 
#
...
try:
   ...
   snowconvert_helpers.execute_sql_statement(f"""COPY INTO @BIA_{ENVNAME}.CMS_STAGE_XTR_{ENVNAME}.BIA_{ENVNAME}_XTR_STS_MED_INS_MN_STG/STS_MED_INS_RPT_BB2A_MN_{EXT_TO_YYYY}_{RUN_PRD}_{TMSTMP}.csv.gz
                    FROM (
            ...
            ,RPT_DATA_FORMATTED  AS  (
                ...
                SELECT 
                     ...
                     ,SORT_ORD_IND 
                     ,CAL_YEAR
                     ,COALESCE(MAX(CASE WHEN SERVICE_CD = 0 THEN REIMBURSEMENT_AMT END),0) AS TOT_AMT
                     ,COALESCE(MAX(CASE WHEN SERVICE_CD = 1 THEN NOF_BILLS         END),0) AS PHYS_NOF_BILLS 
                     ,COALESCE(MAX(CASE WHEN SERVICE_CD = 1 THEN REIMBURSEMENT_AMT END),0) AS PHYS_AMT
                     ,COALESCE(MAX(CASE WHEN SERVICE_CD = 1 THEN PER_BILLS         END),0) AS PHYS_PER_BILLS  
                     ,COALESCE(MAX(CASE WHEN SERVICE_CD = 2 THEN NOF_BILLS         END),0) AS OPT_NOF_BILLS 
                     ,COALESCE(MAX(CASE WHEN SERVICE_CD = 2 THEN REIMBURSEMENT_AMT END),0) AS OPT_AMT
                     ,COALESCE(MAX(CASE WHEN SERVICE_CD = 2 THEN PER_BILLS         END),0) AS OPT_PER_BILLS 
                     ,COALESCE(MAX(CASE WHEN SERVICE_CD = 3 THEN NOF_BILLS         END),0) AS IND_LAB_NOF_BILLS 
                     ,COALESCE(MAX(CASE WHEN SERVICE_CD = 3 THEN REIMBURSEMENT_AMT END),0) AS IND_LAB_AMT
                     ,COALESCE(MAX(CASE WHEN SERVICE_CD = 3 THEN PER_BILLS         END),0) AS IND_LAB_PER_BILLS  
                     ,COALESCE(MAX(CASE WHEN SERVICE_CD = 4 THEN NOF_BILLS         END),0) AS HHA_NOF_BILLS 
                     ,COALESCE(MAX(CASE WHEN SERVICE_CD = 4 THEN REIMBURSEMENT_AMT END),0) AS HHA_AMT
                     ,COALESCE(MAX(CASE WHEN SERVICE_CD = 4 THEN PER_BILLS         END),0) AS HHA_PER_BILLS 
                     ,COALESCE(MAX(CASE WHEN SERVICE_CD = 5 THEN REIMBURSEMENT_AMT END),0) AS OTH_AMT
