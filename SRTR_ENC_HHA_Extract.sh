#!/usr/bin/bash
############################################################################################################
# Name:  SRTR_ENC_HHA_Extract.sh
#
# Desc: SRTR HHA Extract 
#
# Execute as ./SRTR_ENC_HHA_Extract.sh 
#
...
# Modified:
#
# Paul Baranoski 2023-02-09 Created script.
...
LOGNAME=/app/IDRC/XTR/CMS/logs/SRTR_ENC_HHA_Extract_${TMSTMP}.log
...
echo "################################### " >> ${LOGNAME}
echo "SRTR_ENC_HHA_Extract.sh started at `date` " >> ${LOGNAME}
echo "" >> ${LOGNAME}
...
do
		...
		#############################################################
		echo "" >> ${LOGNAME}
		echo "Start execution of SRTR_ENC_HHA_Extract.py program"  >> ${LOGNAME}
		...
		if [[ $RET_STATUS != 0 ]]; then
				echo "" >> ${LOGNAME}
				echo "Python script SRTR_ENC_HHA_Extract.py failed" >> ${LOGNAME}
				
				# Send Failure email	
				SUBJECT="SRTR_ENC_HHA_Extract.sh  - Failed"
				MSG="SRTR Encounter HHA extract has failed."
				${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_ENC_EMAIL_SENDER}" "${SRTR_ENC_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
		...
		echo "" >> ${LOGNAME}
		echo "Python script SRTR_ENC_HHA_Extract.py completed successfully. " >> ${LOGNAME}
...
SUBJECT="SRTR Encounter HHA extract" 
MSG="The SRTR Encounter HHA extract from Snowflake has completed.\n\nThe following file(s) were created:\n\n${S3Files}"
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="Sending Success email in SRTR_ENC_HHA_Extract.sh  - Failed"
	MSG="Sending Success email in SRTR_ENC_HHA_Extract.sh  has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_ENC_EMAIL_SENDER}" "${SRTR_ENC_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
...
#############################################################
echo " " >> ${LOGNAME}
echo "EFT SRTR Encounter HHA Extract Files " >> ${LOGNAME}
...
if [[ $RET_STATUS != 0 ]]; then
	...
	# Send Failure email	
	SUBJECT="SRTR Encounter HHA EFT process  - Failed"
	MSG="SRTR Encounter HHA EFT process has failed."
	${PYTHON_COMMAND} ${RUNDIR}sendEmail.py "${SRTR_ENC_EMAIL_SENDER}" "${SRTR_ENC_EMAIL_FAILURE_RECIPIENT}" "${SUBJECT}" "${MSG}" >> ${LOGNAME} 2>&1
